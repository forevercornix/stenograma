...
`REQUIRE_S3=1 S3_ENDPOINT=http://localhost:8333 npm run test:s3` —
komanda įvardyta PR aprašyme, rezultatas — CI.
*(#405: vardai atnaujinti. `S3_ACCESS_KEY`/`S3_SECRET_KEY` neprivalomi — sargas turi numatytuosius ≥16 simbolių raktus; trumpesni raktai kai kuriuose serveriuose tyliai palieka užklausas be autorizacijos.)*
