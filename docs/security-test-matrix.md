...
| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **Ištrynimas IŠGYVENA atkūrimą iš senesnės kopijos** | `drRestore.integration` | ✅ CI PASS (run 33782254813; 10 subtestų, `skipped 0`). Žingsniai: pripildymas → kopija → ištrynimas PO kopijos → žurnalo eksportas → restore → **tarpinė asercija** → koordinatorius → galinė būsena → idempotentiškumas |
...
| ⚠️ **Nepašalintas audio registruojamas, bet nėra nei nesėkmė, nei revive** | `erasureReplayContract`, `jobErasure` | Mutacijos M12 (`storageRemoved` vėl besąlyginis) ir M13 (registravimas pašalintas) → krinta. ⚠️ **Du radiniai:** mano šaka tikrino `outcome.audioCleanupPending` — lauko, kurio `eraseJob()` NIEKADA nenustato (negyvas kelias su garantiją skelbiančiu komentaru); o `eraseJob` `storageRemoved` statė besąlygiškai, ignoruodamas `fileStorage.del()` grąžinamą reikšmę, tad `DATA_ERASED` rašė `storage=deleted` ir tada, kai objekto nebuvo. Testų dublis grąžindavo `undefined` ir tai dengė |
...
| ⚠️ **Teigiamas pasenusio žurnalo kelias eina iki galo** | `drCoordinatorContract` | Mutacija M15 (`patvirtinimas` vėl numetamas `paleisti()` viduje) → krinta. ⚠️ **Codex radinys (#288):** `paleisti()` NEPRIIMDAVO `patvirtinimas`, o CLI jį perduodavo — `PRIVACY_MODE` diegime teisėto atsigavimo su pasenusiu žurnalu NEBUVO IŠVIS. Nepagavo todėl, kad padengtos buvo tik NEIGIAMOS šakos ir tik `_uzfiksuotiOverride()` lygyje: teigiamas kelias per visą seką liko neįrodytas — „patikra be teigiamos kontrolės", tik viena pakopa aukščiau |
