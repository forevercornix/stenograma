...
| ⚠️ **Pasirinkus PostgreSQL, neprieinama DB NUTRAUKIA startą — nenusileidžia į atmintį** | CI žingsnis „Fail-closed startas" (`ci.yml`, `backend`) | ⚠️ **Ne unit testas sąmoningai.** `_initializePostgresForTests` tikrino funkciją, kurios produkcinis startas tiesiogiai nekviečia; tarp jos ir tikro `node server.js` guli visa parinkimo politika, kurią šis PR ir pakeitė. ⚠️ **EXIT KODO NEPAKANKA — išmatuota:** su uždarytu barjeru tas pats startas krenta kodu 1 dėl kitos priežasties, tad kodą tikrinanti patikra būtų žalia abiem atvejais. **Mutacija (produkcijos pusėje):** `initializePostgres()` `catch` bloke `throw` pakeitus į `store = memoryStore; return store` → žingsnis krenta. ⚠️ **Ir ta mutacija atskleidė TREČIĄ sluoksnį:** procesas vis tiek nekyla — jį sustabdo ištrynimo žymų lentelės patikra, irgi fail-closed. Todėl asercija, tikrinanti tik „job store neinicijuotas", mutacijos NEBŪTŲ pagavusi; pagauna būtent priežasties asercija |
