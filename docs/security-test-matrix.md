...
| Audito pool'as turi `error` klausytoją — neveiklios jungties klaida (DB restartas, tinklo trūkis) NENUŽUDO proceso | `auditStoreFields` | Pašalinus klausytoją → `EventEmitter` neapdorotą `error` meta, Node nutraukia HTTP serverį arba worker'į, ir nei blokuojantis atmetimas, nei neblokuojantis skaitiklis nebesuveikia — proceso nebelieka |
