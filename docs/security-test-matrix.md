...
| Nepavykęs įrašymas patenka į logą su `jobId` ir kodu | `deletionRetryPersistence` (`MATOMUMAS`) | Grąžinus tylų `.catch(() => {})` → krinta 4 |
| ⚠️ **Logas NEŠA korelaciją, bet NE job duomenų ar viso `patch`'o** | `deletionRetryPersistence` (`#197 LOGAS`) | Įdėjus `JSON.stringify(patch)` į klaidos logą → krinta 1. ⚠️ Iki #197 closure buvo padengta tik teigiama pusė („yra `jobId` ir kodas"), o ko loge NETURI būti - niekas netikrino |
| Matricos eilutės rodo į EGZISTUOJANČIUS testų failus; mutacijų stulpelis užpildytas | `check-matrix-rows.mjs` (**tripwire**, su savipatikra) | Ištrynus testų failą ar suklydus varde → krinta `npm run test:evidence`. ⚠️ **KO ŠI PATIKRA NEGAUDO: eilutės, rodančios į esamą failą, kuriame to testo NĖRA.** Būtent tokia buvo #197 spraga (G5) - eilutė rodė į `deletionRetryPersistence`, failas egzistavo, testo jame nebuvo. Patikra tikrina eilutės FORMĄ, ne turinį |
| ⚠️ **Pavykus įrašymui atsarginė būsena IŠVALOMA** | `deletionRetryPersistence` (`#197 REKONCILIACIJA`) | Pašalinus `atsarginiaiBandymai.delete()` → krinta 1. ⚠️ **Iki #197 closure ši eilutė rodė į rinkinį, kuriame tokio testo NEBUVO** - mutacija nesulaužydavo nieko visoje 1582 testų suitėje. Tikrinama per ELGESĮ (bandymo numeris po reconciliacijos), ne per privatų `Map` |
| ⚠️ **Atsarginė būsena izoliuota tarp `jobId`** | `deletionRetryPersistence` (`#197 IZOLIACIJA`) | Pašalinus `jobId` iš rakto → krinta 1. Vieno jobo testuose kryžminė tarša nematoma: raktas be `jobId` praeidavo visą suitę, o vieno jobo nesėkmė keltų svetimo jobo bandymų numerį |
| ⚠️ **Fallback naudoja TĄ PAČIĄ autoritetingą backoff formulę** | `deletionRetryPersistence` (`#197 FORMULĖ`) | Įvedus antrą nepriklausomą formulę (fiksuota 1 s) → krinta 1. Esami testai stūmė laiką valanda, tad trumpesnės atgalos nepastebėdavo; riba imama iš `_backoffMs()`, kad testas netaptų trečia formule |
...
| ⚠️ **Python grupė priima TIK deklaruotus serviso paketus, be wildcard** (#367 Codex A) | `dependabotPolicy` | Įrašius `torch*`, `nvidia-*`, `onnxruntime` ar net `starlette` → krinta. ⚠️ Senoji redakcija tikrino `includes()` prieš ML sąrašą ir `torch*` **praleisdavo** — vienintelis politikos liudytojas buvo nukaunamas viena eilute. Dabar taisyklė išvesta: pažodiniai vardai plius uždara aibė, tad ML paketų sąrašo nebereikia |
...
| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| **`DATABASE_URL` NEPERJUNGIA job metaduomenų** | `jobStoreBackendSelection` | ⚠️ **SAVYBĖ IŠLIKO, PRIEŽASTIS PASIKEITĖ (#155).** Buvo: aktyvavimo barjeras, o mutacija `POSTGRES_AKTYVAVIMAS_LEISTAS = true` numušdavo 7 testus. Barjeras ATIDARYTAS — ir `DATABASE_URL` toliau neperjungia, nes `postgres` renkamas TIK eksplicitiniu `JOB_STORE_BACKEND`. **Nauja mutacija:** grąžinus numanomą išvedimą (`DATABASE_URL` → `postgres`) → krenta |
...
| **Prisijungimo klaida NEGRĮŽTA į memory (fail-closed)** | `jobStoreBackendSelection` | Fallback reikštų split-brain: nauji job'ai atmintyje, autoritetingi — DB. ⚠️ **NEBE PARTIAL (#155):** unit lygmuo (`_initializePostgresForTests`) liko kaip pigus sluoksnis, o pilną produkcinį kelią dengia CI žingsnis „Fail-closed startas" — tikras `node server.js` su `JOB_STORE_BACKEND=postgres` ir uždaru prievadu. Acceptance, perkeltas į aktyvavimo etapą, tame etape ir įvykdytas (žr. #155 skyrių žemiau) |
...
| ⚠️ **Versiją skaičiuoja SAUGYKLA, ne JS snapshot'as** | `jobVersionParity` („Redis versiją didina SERVERIS"), `postgresStore.integration` | PostgreSQL — `VERSIJOS_ZYMA` (`version = jobs.version + 1`, kaip `LAIKO_ZYMA`); Redis — `HINCRBY`. Anksčiau du kvietėjai, perskaitę `N`, abu rašydavo `N + 1`: dvi mutacijos, vienas padidinimas, ir pasenęs snapshot'as CAS'ą praeidavo. Mutacija: grąžinus `serialize(next)` su versija → krinta |
| ⚠️ Du kvietėjai nuo **TO PATIES snapshot'o** duoda +2, ne +1 | `jobVersionParity` | ⚠️ Pirmoji eilutės redakcija rodė į testą, kuris `update()` kvietė NUOSEKLIAI — antrasis skaitė pirmojo įrašytą reikšmę, tad snapshot'o realizacija būtų davusi tą patį (Codex F4). Dabar `hgetall` abiem grąžina tą patį `N`. Mutacija: grįžus prie `hset` su versija → krinta |
| ⚠️ Besąlyginis Redis rašymas — **viena transakcija** (`MULTI`/`EXEC`) | `jobVersionParity` | `HSET` pavykus, o `HINCRBY` kritus, liktų nauji laukai su SENA versija, ir tą versiją turintis klientas praeitų CAS (Codex F1). Pirmoji redakcija riziką tik APRAŠĖ komentare |
| ⚠️ Atkurta `version` ribojama **bendriausiu** PG `integer` diapazonu | `jobVersionParity` | `"2147483648"` praeitų vien „skaitmenys ir ≥ 1" patikrą: memory/Redis atkurtų, PG atmestų — tas pats backup'as elgtųsi skirtingai (Codex F3) |
| ⚠️ Redis **progreso** CAS irgi didina `version` | `ownershipCasRedis.integration` | ⚠️ NOT RUN vietoje (`FakeRedis` `eval` neturi). `CAS_PROGRESS_LUA` versijos neliesdavo, tad po priimto įvykio senas snapshot'as likdavo galiojantis CAS'ui |
| ⚠️ Redis `finishAtomic` po pralaimėto CAS **perklasifikuoja** į `RESULT_CONFLICT` | `ownershipCasRedis.integration` | ⚠️ NOT RUN vietoje. Be to pralaimėtojo retry gautų NUGALĖTOJO rezultatą kaip idempotentišką sėkmę, ir trys backend'ai atsakytų skirtingai į tą pačią lenktynę |
| ⚠️ Netinkama **atkurta** `version` (`0`, neigiama, `"1x"`) atmetama garsiai | `jobVersionParity` | Bendra `parseInt(x) \|\| 0` taisyklė jas priimtų; PG atmestų per `jobs_version_positive`, o memory/Redis atkurtų — atkūrimas taptų backend-priklausomas. Tikrinama ŽALIA reikšmė (`parseInt("1x")` yra `1`). Mutacija: validaciją pašalinus → krinta |
...
| ⚠️ Po `RESULT_CONFLICT` audio **pašalinamas**, ir tai teisinga | — (elgesio sargo nėra) | Autoritetinga būsena yra `completed` su NUGALĖTOJO rezultatu; nugalėtojas šaltinį būtų ištrynęs pats. Ankstesnė 7.5b redakcija kode teigė priešingai — tai buvo §12.1 perdėjimas, ne kodo klaida |
...
| ⚠️ Pralaimėtojas gauna `RESULT_CONFLICT`, **ne** `COMPLETED_WITHOUT_RESULT` | `postgresStore.integration` | `EvalPlanQual` iš naujo skaito TIK užrakintą lentelę; prijungta `job_results` lieka sena. Rezultatas skaitomas ATSKIRU sakiniu po užrakto — mutacija (grįžimas prie `job.result`) šią eilutę nužudo |
| ⚠️ **Retry įėjimo TAISYKLĖ:** `completed` + rezultatas → idempotentiška sėkmė; `completed` be rezultato → remontuotina; `failed`/`cancelled` → jau terminalus; **nėra įrašo / `queued` / `processing`** → vykdyti | `jobFinishIdempotency` (gryna `sprendimasPriesRestart()`) | ⚠️ Ankstesnė redakcija sakė „kita → vykdyti", o tai apėmė ir `failed`/`cancelled` — prieštaravo eilutei žemiau (Codex G2). Mutacijos: `REMONTUOTINA` šakos pašalinimas → krinta 2; „visada `VYKDYTI`" → krinta 2 |
...
| ⚠️ Barjeras **reikalauja TERMINALAUS statuso** — `queued`/`processing` blokuojami | `audioCleanup` | Numatytoji reikšmė apversta (Codex E2): anksčiau kiekvienas kvietėjas turėjo ATSKIRAI tikrinti, ar perėjimas įvyko, ir per vieną PR tai buvo pamiršta keturiose vietose. Mutacija: statuso patikrą pašalinus → krinta |
| ⚠️ Barjeras yra **vienas autoritetas ABIEM** vykdymo keliams (`worker` ir `inline`) | `jobFinishIdempotency` | Mutacija: inline `finally` grąžinus tiesioginį `releaseAudio()` → krinta. 7.5b barjeras buvo tik worker'yje; inline turi savo valymą ir liko be jo |
| ⚠️ **GDPR ištrynimo kelias barjero NETURI** — `jobErasure` šalina per `fileStorage.del()` | `jobFinishIdempotency`, `audioCleanup` | Barjeras ten blokuotų įstatymo reikalaujamą veiksmą. ⚠️ Pirmoji šios eilutės redakcija teigė, kad ištrynimas kviečia `releaseAudio()` — NEKVIEČIA; testas krito ir tai atskleidė. Mutacija: barjerą nukėlus į `releaseAudio()` → krinta |
| ⚠️ **Techninis** valymo pakartojimas eina PER barjerą | `audioCleanup` | Barjerui nepavykus perskaityti būsenos jis žymi `audio_cleanup_pending`; sweep'as tą vėliavą apdorodavo tiesiogiai ir ištrindavo būtent tai, ką barjeras saugojo (Codex E1). Mutacija: grąžinus tiesioginį `releaseAudio()` → krinta |
| ⚠️ `finishFailed` slopina **tik** `JOB_ALREADY_TERMINAL`, ne bet kokį `JobPhaseError` | `jobFinishIdempotency` | `UNKNOWN_SOURCE_STATUS` metamas ta pačia klase; nuslopintas jis duotų melagingą „no-op sėkmę", po kurios eina audio valymas. Mutacija: grįžus prie `err.name` patikros → krinta |
| Audio valomas **tik po įsipareigoto** terminalaus perėjimo (`_handleFailure`) | `workerIdempotency.integration` („`finishFailed` NEĮSIPAREIGOJUS") | ⚠️ Eilutė iš pradžių rodė į testą, kuris šios šakos NEVYKDĖ (jis pradeda nuo `completed`, tad `finishFailed()` grąžina job objektą, o failą saugo ATSKIRAS barjeras). Codex tai pagavo; pridėtas testas, deterministiškai grąžinantis `CONCURRENCY_CONFLICT` |
| Nesėkmės tvarkytojo atmetimas **negriauna worker'io** | `workerIdempotency.integration` („atmetimas gaudomas klausytojo riboje") | ⚠️ Eilutė iš pradžių rodė į testą, kuris `finishFailed()` niekada nepriverčia ATMESTI — ištrynus `.catch(...)` rinkinys būtų likęs žalias. Codex tai pagavo; pridėtas testas stebi `unhandledRejection` IR `failure_handler_error` |
| ⚠️ **Inline** valymas taip pat laukia įsipareigoto perėjimo | `audioCleanup` („inline: NEĮSIPAREIGOJUS…") | Mutacija: `finally` padarius besąlyginį → krinta. Abiem `finish` grąžinus konfliktą, įrašas lieka `processing`, o barjeras ne-terminalį praleidžia |
| ⚠️ Barjero paieškos gedimas **pažymi** valymo pakartojimą | `audioCleanup` („barjero paieškos gedimas PAŽYMI…") | Mutacija: žymėjimą pašalinus → krinta. Be jo trikdis paliktų failą be `audio_cleanup_pending`, o `retryPendingAudioCleanups()` ieško TIK tos vėliavos |
| Barjero kvietimas tikrinamas **kiekvienoje** valymo funkcijoje atskirai | `jobFinishIdempotency` | ⚠️ `jobRunner.js` turi DU nepriklausomus valymo taškus; failo lygio regex praeidavo ištrynus vieną iš jų. Mutacija: pašalinus `_executeInline` kvietimą → krinta |
| Idempotentiškas retry **išvalo** audio prieš grąžindamas rezultatą | `workerIdempotency.integration` | ⚠️ vietoje NEPAGAUNAMA. Be jo failas liktų amžiams būtent tame kritimo lange, dėl kurio šaka egzistuoja |
...
| ⚠️ **`PG*` forma palaikoma SĄLYGINIAI; neperduodami kintamieji → GARSUS atsisakymas** (#264) | `pgBackupPgForma` | Pašalinus ribą (`vaikinioProcesoKliutys()` → `[]`) krinta 2 testai: `PGOPTIONS` ir `PGPASSWORD` nebesustabdo, ir `pg_dump` pasileidžia. Išmatuota: be ribos kopija ateitų iš kitos schemos arba per nešifruotą jungtį, ir abu atrodytų kaip sėkmė |
| ⚠️ **Atsisakymo sąlyga IŠVEDAMA iš `PG_ATITIKMENYS`, ne surašyta** | `pgBackupPgForma` (išvedimo savipatikra) | Fiktyvus `PGNAUJAS` blokuoja, nors jokiame sąraše jo nėra. Be šio atvejo būtų tikrinami penki pavyzdžiai, ne taisyklė — ir naujas libpq kintamasis praeitų tyliai |
| ⚠️ **Vaikinis `pg_dump` gauna TAIKINĮ, o `argv` neturi slaptažodžio** | `pgBackupPgForma` (`PATH` stub'as) | Stub'as įrašo savo `argv` ir `PG*` kintamuosius: matuojama, KĄ procesas gavo, o ne ar jis praėjo. Vėliavos `-h/-p/-U/-d` vietoj DSN reiškia, kad slaptažodžio `argv` eilutėje nėra pagal konstrukciją |
| ⚠️ **`PGDATABASE` su conninfo ATMETAMAS** — libpq perrašytų vėliavas (#264 A1) | `pgBackupPgForma` | Pašalinus patikrą → krinta. Ta pati klasė kaip `PGHOSTADDR` (#245 P1): modelis sako `safe.invalid`, `pg_dump` eitų į `other.invalid` |
| ⚠️ **`DATABASE_URL` be `--url` pasiekia `pg_dump`** (#264 A2, regresija) | `pgBackupPgForma` | Pašalinus `connectionString` šaką → krinta: vėliavų konstruktorius jo nemoka, ir `pg_dump` liktų **be taikinio**, jungdamasis prie numatytosios lokalios bazės |
| ⚠️ **`PGPORT=""` nereiškia `-p 0`** — ta pati „nenustatyta" semantika kaip `pg` (#264 B1) | `pgBackupPgForma` | Grąžinus seną sąlygą → krinta. Preflight tapatybė nesutapimo nepagaudavo: abi jos pusės naudoja tą pačią falsy taisyklę |
| ⚠️ **`PG_` yra aplikacijos erdvė** — repo savas `PG_CONNECT_TIMEOUT_MS` nestabdo (#264 B2) | `pgBackupPgForma` (+ liudytojas) | Pašalinus taisyklę → krinta. **Liudytojas** tvirtina, kad nė vienas žinomas libpq kintamasis neprasideda `PG_`; prielaidai lūžus kris jis, ne produkcija |
| ⚠️ **Sprendimas prieš konfigūraciją**: `BACKUP_DISABLED` pirmiau nei aplinkos klaida (#264 C1) | `pgBackupPgForma` | Sukeitus tvarką → krinta. Operatorius taisytų kredencialus, nors priežastis — administracinis jungiklis |
| ⚠️ **`PG*` forma prieš TIKRĄ `pg_dump`, kredencialai per `~/.pgpass`** (#264 D) | `pgDumpBackup.integration` | ⏳ CI. Uždaro visus tris `UNVERIFIED`: `PATH` stub'as matuoja, ką procesas GAVO, šis — ką `pg_dump` su tuo DARO. Sentinelis kopijoje įrodo, kad vėliavos nuvedė į TĄ klasterį |
| `--url` kelias lieka **pozicinis ir ribos negauna** (regresija) | `pgBackupPgForma` | URL ardymas į vėliavas prarastų query eilutę (`sslmode`, `options`) — tylus praradimas ten, kur jo šiandien nėra |
| ⚠️ **`verify()` biudžetas imamas iš IŠMATUOTO dydžio, ne iš persistinto lūkesčio** (#292) | `artifactVerifyRiba` | Grąžinus biudžetą į `expected.bytes` → krinta. Stebimas tik vienu atveju: saugykla praneša vieną dydį, o atiduoda didesnį — visus kitus pašalina metaduomenų vartai ir dydžių palyginimas |
| ⚠️ **Nevalidus `expected.bytes` → METADUOMENŲ verdiktas, payload neatidaromas** (#292) | `artifactVerifyRiba` | Pašalinus patikrą → krinta 2 testai. `s3` pusėje tikrinama, kad `GetObject` **nesiunčiamas**: sprendimas priimamas prieš tinklo I/O |
| ⚠️ **`head().bytes` virš rėmo → OBJEKTO verdiktas, ne metaduomenų** (#292) | `artifactVerifyRiba` | Pašalinus patikrą → krinta 2 testai. Kilmė skirtinga → operatoriaus išvada skirtinga: tirti saugyklą, ne DB eilutę |
| ⚠️ **Priežastys pasiekia operatoriaus SANTRAUKĄ, ne tik objektą** (#292 Codex A) | `artifactVerifyRiba` | Pašalinus jas iš `santrauka` → krinta. Runbook'o §9d spausdina **tik** `santrauka`; ankstesnė patikra matavo, ar `detale` išlieka **objekte** — ne ar operatorius ją **mato** |
| ⚠️ **Metaduomenų defektas NUSVERIA politiką; pirmenybė užrašyta kaip duomenys** (#292 Codex B) | `artifactVerifyRiba` (6 deriniai) | Apvertus `PIRMENYBE` → krinta. Tikrinami **visi** 3×2 deriniai, ne tas, kurį kas nors pastebėjo — daliniai patikrinimai davė **tris raundus iš eilės** dėl tos pačios tvarkos |
| ⚠️ **Srautas viršija `head()`, bet telpa į ribą → SAUGYKLOS anomalija** (#292 Codex C) | `artifactVerifyRiba` | Grąžinus `VIRSIJA_RIBA` → krinta 2 testai. Klasė, kurios **niekada nebuvo**: pasimatė tik atsiradus gretimai, panaudojus naują verdiktą ne pagal paskirtį |
| ⚠️ **`bytes` virš ribos — POLITIKOS, ne metaduomenų klausimas** (#292 Codex A) | `artifactVerifyRiba` | Grąžinus į metaduomenų defektą → krinta. Sumažinus `MAX_RESULT_BYTES`, anksčiau teisėtai įrašyti artefaktai ją viršija, nors **eilutė ir objektas sveiki**; DB `CHECK` maksimumo neturi. Verdiktų skaičius **sumažėjo** nuo 3 iki 2 |
| ⚠️ **`checksum` validuojamas ŽALIAS, prieš kanonizavimą** (#292 Codex B) | `artifactVerifyRiba` | Perkėlus patikrą po `trim().toLowerCase()` → krinta. Kanonizavimas yra **palyginimo patogumas, ne validumo šaltinis**: didžiosiomis persistinta suma praeidavo ir sutapus objektui duodavo `ok: true` |
| ⚠️ **`bytes = 0` yra metaduomenų defektas — taip sako DB `CHECK`** (#292 Codex A) | `artifactVerifyRiba` | Grąžinus ribą į `< 0` → krinta. Be jos eilutė su `bytes = 0`, tuščio payload SHA-256 ir nupjautu objektu duotų `ok: true` — verifikacija **patvirtintų neįmanomą būseną** |
| ⚠️ **`checksum` ne pagal `CHECK` formatą irgi metaduomenų defektas** (#292 Codex A) | `artifactVerifyRiba` | Pašalinus patikrą → krinta. `CHECK` garantuoja `^[0-9a-f]{64}$`; neatitinkanti suma yra neįmanoma būsena, ne „turinys nesutampa" |
| ⚠️ **`fs`/`s3` PARITETAS: nevalidus lūkestis dingusiam objektui → `exists: false`** (#292 Codex B) | `artifactStoreContract` (visi backend'ai) + `artifactVerifyRiba` | Grąžinus vartus prieš `head()` → krinta 2 testai. Atvejis dedamas į **kontraktą**, ne į vieno backend'o failą: trečias kartas šioje sekoje, kai `fs`/`s3` asimetrija duoda defektą |
| **KONTROLĖ: teisėtas objektas ir teisėtas lūkestis praeina** (fs ir s3) | `artifactVerifyRiba` | Be jos trys mutacijos suderinamos su `verify()`, kuris atmeta **viską**; ta klasė šioje sekoje pasirodė keturis kartus |
...
| ⚠️ **`utils/auditStore/` išimtis `getAll` sarge veikia abiem formomis** | `auditStoreFields` | Mutacija OA (kanonizavimas pašalintas TIKRAJAME skeneryje) → krinta. ⚠️ **Ankstesnė šios eilutės redakcija skelbė kritimą, kurio nebuvo:** testas turėjo predikato KOPIJĄ, tad mutacija tikrajame skeneryje jo nelaužė (#285 peržiūra). Dabar testas kviečia tą patį skenerį. Mutacija OB (bendras helperis nieko nekeičia) → krinta. ⚠️ Tas pats defektas, kurio issue inventorizacija neturėjo: prefiksas `utils/auditStore/` su baigiamuoju `/` Windows'e nesutampa. Šiandien nekrisdavo tik todėl, kad išimtis NEPANAUDOJAMA (0 `auditLog.getAll(` saugyklos sluoksnyje) |
...
| ⚠️ **Turinys skaitomas per NEAPDOROTĄ įrašą** | `auditAsyncCutover` | Mutacija OC (skaitoma per kanoninį kelią) → krinta. POSIX'e `foo\bar.js` yra VIENAS failas tokiu vardu; kanoninis kelias grąžintų svetimą turinį arba nieko, ir tikras producer'is tyliai iškristų iš validacijos — fail-open fail-closed validatoriuje |
| ⚠️ **Fail-closed įrodoma per PRODUKCINES grandis** | `auditAsyncCutover` | Mutacijos OD (`startupChecks` nebekviečia `validateAuditEvents`) ir OE (validatorius nebekviečia `producerIvykiai`) → krinta. ⚠️ Ankstesnė redakcija paduodavo paruoštus rinkinius tiesiai į helperį ir apeidavo ABI grandis; 7.4a garantija yra būtent jų vientisumas. Kontrolė: be nežinomo šaltinio ta pati konfigūracija tos klaidos neturi |
...
| ⚠️ **`options`/`search_path` yra taikinio dalis** | `postRestoreReconcileContract` | Mutacija NT (`options` neįtraukiamas) → krinta. `pg` jį siunčia startup pakete, o visos suderinimo užklausos naudoja nekvalifikuotus vardus — `-csearch_path=prod` vs `…=restore` yra skirtingos lentelės. Kontrolė: vienodas `options` privalo praeiti |
| ⚠️ **Socket kelio raidės reikšmingos, DNS vardo — ne** | `postRestoreReconcileContract` | Mutacija NU (`toLowerCase()` visam host'ui) → krinta. `?host=/Prod` yra failų sistemos kelias; normalizavimas gyvena VIENAME helperyje, nes anksčiau buvo dvi kopijos |
| ⚠️ **Nė vienos PostgreSQL ašies → komanda krenta** (D7) | `postRestoreReconcileContract`, `postRestoreReconcile.integration` | Mutacijos NV (`arSaugu` be „bent vienos") ir NW (ašių sargas išjungtas) → krinta. Integraciniame teste tikrinama BŪSENA: po kritimo sesijos aktyvios, job'ai nepakitę, audito eilutės nėra. Kontrolė: su sesijų ašimi ta pati komanda praeina |
...
| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **Ištrynimas IŠGYVENA atkūrimą iš senesnės kopijos** | `drRestore.integration` | ✅ CI PASS **10 subtestų redakcijai** (run 33782254813, `skipped 0`). ⚠️ **ŠI revizija prideda 6b (gedimo sklidimas + atstatymas), tad subtestų yra 11 ir jiems CI įrodymo DAR NĖRA** — įrašomas po žalio šios šakos paleidimo (Codex, #288). Žingsniai: pripildymas → kopija → ištrynimas PO kopijos → žurnalo eksportas → restore → **tarpinė asercija** → koordinatorius → galinė būsena → idempotentiškumas |
| ⚠️ **Ištrynimas išgyvena atkūrimą IR pasiekia EXTERNAL OBJEKTĄ, ne tik eilutę** | `drRestore.integration` | ⚠️ **R1 (#155): iki šio pakeitimo DR pratyba naudojo TIK inline duomenis** — faile nebuvo nė vienos nuorodos į saugyklą (išmatuota: 0 sutapimų). Vadinasi „ištrynimas išgyvena atkūrimą" buvo įrodyta `jobs` ir `job_results` EILUTĖMS, o po #157 rezultatas gali gulėti failų sistemoje ar S3, ir jo **vienintelis adresas** yra tos pačios lentelės. ⚠️ **Ir sena asercija buvo TUŠČIA:** „rezultato eilutės taip pat nebėra" tikrino `zymetas`, kuris rezultato **niekada neturėjo** — skaičius visada buvo 0, tad ji negalėjo kristi. Dabar pažymėtas job'as užbaigiamas per `rasymoSaugykla`, ir tikrinama: (a) objektas realiai guli saugykloje prieš ištrynimą; (b) po `eraseJob()` jo **nebėra failų sistemoje**; (c) koordinatorius gauna `artifactStores` ir replay atkurtoje bazėje **nekrenta** ties jau ištrintu objektu. ⚠️ **Saugykla VIENA abiem bazėms** — objektai gyvena už DB ribų, tad senesnė kopija kitoje bazėje rodo į TĄ PAČIĄ saugyklą; būtent tokia yra tikrovė |
| ⚠️ **Nepažymėto job'o objektas saugykloje IŠLIEKA** | `drRestore.integration` | ⚠️ **C2 — MUTACIJAI ATSPARI** (raundas `34701354381`, 5 žingsnis, `expected: 'fs'`): `pg_dump` nebeatkūrus `job_results` eilučių, tarpinė asercija krenta tiksliai, o 8 žingsnis vis tiek praeitų — tad mutacija atskiria BŪTENT ją. ⚠️ **C1 — GILUMOS GYNYBA; NEPRIKLAUSOMO MUTACIJOS ĮRODYMO NĖRA.** Dvi realios mutacijos perimtos ankstesnių sluoksnių: DB lygmens (`listResultArtifacts` be `job_id` filtro) atsimušė į `svetimiAdresai`/`NESAUGU`, tad ištrynimas nutrūko visiškai ir kryžminio trynimo NEĮVYKO — C1 asercija buvo įvykdyta ir PRAĖJO, bet ne dėl to, ką gina; `fsStore.del()` lygmens (trynimas išplinta į tėvinį katalogą) atsimušė į `artifactStoreContract: fs` funkciniame rinkinyje (3 subtestai, tarp jų kontrolė), ir `postgres` rinkinys nepasileido. ⚠️ **IŠMATUOTA SLUOKSNIŲ TVARKA** (architektūrinis faktas, kurio nė vienas dokumentas neturėjo): `artifactStoreContract: fs` (funkcinis, pigiausias, pirmas) → `svetimiAdresai`/`NESAUGU` (DB) → C1 (DR pratyba). ⚠️ **KODĖL DVIEJŲ NEPAVYKUSIŲ BANDYMŲ PAKANKA:** izoliuoti C1 reikėtų SUDĖTINĖS mutacijos, išjungiančios abu ankstesnius sluoksnius vienu metu — tai matuotų konfigūraciją, kuri negali egzistuoti, ir toks įrodymas būtų SILPNESNIS, ne stipresnis. Užrašoma pažodžiui, kad po metų „C1 be mutacijos įrodymo" nebūtų skaitoma kaip praleidimas (§14.1, pritaikytas sau) |
...
| ⚠️ **Replay šalina iš ATKURTOS bazės, ne iš fasado** | `erasureReplayContract`, `drRestore.integration` (6a) | Mutacijos M8 (`store` ignoruojamas skaitant) ir M9 (`eraseJob` gauna fasadą) → krinta. ⚠️ **Radinį atidengė kritęs E2E, ne peržiūra ir ne mutacija:** pratybų aplinkoje job'ų autoritetas yra atmintis, tad replay per fasadą atkurtų eilučių NELIESTŲ — ir vis tiek uždarytų žymą bei įrašytų kvitą. ⚠️ **Po barjero atidarymo (#155) riba SVARBESNĖ, ne mažiau:** fasadas gali būti `postgres`, ir tada replay per jį eitų į PRODUKCINĘ bazę — žala iš „nieko neįvyko" tampa „ištrinta ne toje bazėje" |
| ⚠️ **KONTROLĖ: fasadu vykdytas replay eilutės NEPAŠALINA** | `drRestore.integration` (6a), `erasureReplayContract` | Be šios pusės testai įrodytų tik tiek, kad naujas kelias veikia, bet ne kad senasis buvo vakuumas. Vietinė pusė rodo maršrutizavimą, CI pusė — tikrą atkurtą bazę |
| ⚠️ **Nepilna saugykla atmetama PRIEŠ pirmą šalinimą** | `erasureReplayContract` | Mutacija M10 (patikra pašalinta) → krinta. Keturios nepilnos formos po vieną + kontrolė, kad pilna PRAEINA. Praleistas metodas reikštų dalinį ištrynimą su sėkmės kvitu — ta pati klasė kaip #183 |
| ⚠️ **Koordinatorius be tikslinės bazės kliento KRENTA** | `drCoordinatorContract` | Mutacija M11 (sargas pašalintas) → krinta. `DR_REPLAY_STORE_MISSING`: tylus grįžimas prie fasado būtų vakuumas be jokio ženklo |
| ⚠️ **E2E aplinka tikrinama VIETOJE, prieš nepasiekiamą bazę** | `drRestorePreconditions` | Kiekvienas DR sargas privalo kristi dėl RYŠIO, ne dėl konfigūracijos. Kontrolė: base64 raktas atpažįstamas kaip `BACKUP_KEY_INVALID`, ne ryšio gedimas — be jos patikra būtų visada žalia. ⚠️ Prireikė po TRIJŲ CI raundų, kurie krito ne ties elgesiu, o ties aplinka (`AUDIT_ID_SALT`, sesijos forma, base64 raktas). Aplinka imama iš to paties `drRestoreEnv.js`, kurį naudoja integracinis testas — kopija gintų nebe tą aplinką |
| ⚠️ **Gedimo sklidimas: replay klaida sustabdo VISKĄ po jos** | `drRestore.integration` (6b) | Klaida įleidžiama PO sėkmingo merge (blokuojantis `ERASURE_REPLAYED` be audito saugyklos). Tikrinama, kad sesijos NEREVOKUOTOS, job'ai NETERMINALIZUOTI, `DR_RECOVERY_COMPLETED` ir `POST_RESTORE_RECONCILED` NĖRA, o žyma liko ATVIRA. Auka — atskiras job'as, kad 7 žingsnis liktų tikras |
| ⚠️ **D5: antras paleidimas nekeičia BŪSENOS, ne tik skaičių** | `drRestore.integration` (9) | Lyginamas turinys: job'ų statusai ir fazės, rezultatų raktai, VISI žymų stulpeliai, sesijų `revoked_at`, kopijų horizontas. ⚠️ **Codex radinys (#288):** ankstesnė redakcija lygino tik `COUNT(*)`, tad perrašytas rezultatas ar pastumtas horizontas būtų praėję. Kontrolė: pjūvis privalo būti netuščias |
| ⚠️ **`already absent` NĖRA likutis** | `jobErasure`, `lifecycleService` | ⚠️ **Codex radinys (#288):** `storageRemoved: false` reiškia DVI būsenas — „nepašalinta" ir „jau nebuvo". Suplakus jas, melagingas kvitas (`storage=deleted`, kai nieko nebuvo) būtų pakeistas melagingu likučiu (`remaining: [source_audio]` prie sėkmingo statuso). Įvestas `storageAlreadyAbsent`, kategorijos skaito „ar artefakto nebėra", kvitas skiria `deleted`/`absent`/`none` |
| ⚠️ **Nepavykęs paleidimas ATSTATOMAS pakartojimu** | `drRestore.integration` (6b) | Atvira žyma cutover'į blokuoja (`DR_VERIFICATION_FAILED`), o pakartojimas su TUO PAČIU žurnalu ją uždaro. ⚠️ **Radinys iš CI:** pirmoji 6b redakcija paliko žymą atvirą ir 7 žingsnis krito — sargas buvo teisus, testas nepilnas. Iš to atsirado runbook'o eilutė: kitas žurnalas tos žymos neuždarys, nes jos jame nėra |
| ⚠️ **Senas cookie po atkūrimo NEBEAUTENTIKUOJA** | `drRestore.integration` | Tikrinamas `touch()` elgesys, ne tik `revoked_at` stulpelis: revokacija, kurios `touch()` nepaiso, praeitų būsenos patikrą ir neapsaugotų nieko |
| ⚠️ **Atidėto audio valymo skola registruojama, bet nėra nei nesėkmė, nei revive** | `erasureReplayContract`, `jobErasure` | Mutacijos M12 (`storageRemoved` vėl besąlyginis) ir M13 (registravimas pašalintas) → krinta. ⚠️ **TRYS redakcijos, ir dvi pirmosios klaidingos:** (1) tikrintas `outcome.audioCleanupPending` — laukas, kurio `eraseJob()` niekada nenustato (negyva šaka su garantiją skelbiančiu komentaru); (2) tikrintas `!storageRemoved` — bet nesant objekto `del()` grąžina `false`, tad ĮPRASTAS pakartotinis trynimas degtų kaip likutis (signalas, degantis normaliu atveju, nustoja būti signalu); (3) dabar imama tikroji `audio_cleanup_pending` vėliava iš atkurtos eilutės. Testų dublis, grąžinęs `undefined` vietoj `boolean`, dengė pradinį `eraseJob` defektą — žr. #266 trečią dalį |
| ⚠️ **DR moduliuose NĖRA nuosavo trynimo SQL** | `erasureExportContract` (tripwire) | Mutacijos M14 (`DELETE FROM jobs` į `erasureReplay`), M16 (į `postRestoreReconcile`) ir M17 (importų filtras praleidžia viską) → krinta. Komentarai nukerpami prieš palyginimą (#265), kontrolės abiem kryptimis. ⚠️ **Apimtis IŠVEDAMA iš `require` grafo, ne rankinio sąrašo (Codex, #288):** pirmoji redakcija turėjo penkių failų sąrašą, kuriame TRŪKO `postRestoreReconcile` — `DELETE FROM jobs` ten būtų praėjęs. Ta pati klasė kaip #231 postgres rinkinys ir `redis` masyvas (K1). Gylis — vienas: tranzityvi aibė įtrauktų saugyklas, kur trynimo SQL yra teisėtas. ⚠️ STATINĖ patikra (§9.2) |
| ⚠️ **Pasenusio žurnalo patvirtinimas PASIEKIA sargą** | `drCoordinatorContract` | ⚠️ **Įrodo PERDAVIMĄ, ne pilną praėjimą iki `verify`** (Codex, #288): testas naudoja nepasiekiamą bazę ir tvirtina, kad be patvirtinimo krenta `UNCONFIRMED`, o su teisingu — nebe. Pilnas teigiamas kelias iki `verify` gyvena `drRestore.integration` (10 žingsnis, `PRIVACY_MODE` + pasenęs žurnalas). Mutacija M15 (`patvirtinimas` vėl numetamas `paleisti()` viduje) → krinta. ⚠️ **Codex radinys (#288):** `paleisti()` NEPRIIMDAVO `patvirtinimas`, o CLI jį perduodavo — `PRIVACY_MODE` diegime teisėto atsigavimo su pasenusiu žurnalu NEBUVO IŠVIS. Nepagavo todėl, kad padengtos buvo tik NEIGIAMOS šakos ir tik `_uzfiksuotiOverride()` lygyje: teigiamas kelias per visą seką liko neįrodytas — „patikra be teigiamos kontrolės", tik viena pakopa aukščiau |
...
| ⚠️ **`inline` eilutė NETURI vientisumo metaduomenų** | `jobResultsExternalShape.integration` | ⚠️ **Codex radinys (#289):** invariantas buvo NESIMETRIŠKAS — `inline` šaka tikrino tik `payload` ir `storage_key`, tad external → inline perėjimas, išvalęs raktą, bet pamiršęs `bytes`/`checksum`, būdavo priimtas: pasenusi external būsena, atvaizduota kaip galiojanti inline eilutė. `bytes` ir `checksum` tikrinami ATSKIRAI. ⚠️ **ATŠAUKTA (§12.1, #157 PR-6):** frazė „+ mutacija `inline` šakai" buvo NETEISINGA. Ta mutacija (`:451`) nustato suvaržymą į tekstą, IDENTIŠKĄ `PILNA_FORMA` (`:362` ≡ `:454`), o `PILNA_FORMA` inline šaka yra SENOJI, nesimetriška — be `bytes IS NULL AND checksum IS NULL`. Kadangi kiekvienas ankstesnis `finally` jau atstatė būtent `PILNA_FORMA`, sargo eilutėje nebebuvo dar PRIEŠ tą mutaciją: ji nieko nepakeičia, o tvirtinimas praeina dėl anksčiau prarasto sargo, ne dėl mutacijos. **Pati GARANTIJA galioja** — `INSERT` sargai (`inline` + `bytes`, `inline` + `checksum`) vykdomi prieš migracijos suvaržymą ir yra teisingi; negalioja tik jos MUTACIJOS įrodymas. Taisymas — #305, 5 punktas |
...
| ⚠️ **Kiekviena `CHECK` dalis yra NEŠANTI** | `jobResultsExternalShape.integration` | Mutacijos VYKDOMOS teste: sargas pašalinamas `ALTER TABLE`, tikrinama, kad eilutė įsirašo, ir grąžinamas. Trys atskiros (`payload`, `bytes`, `checksum`) + reikšmių aibė atskirai nuo formos. ⚠️ **Kodėl to reikėjo, nors testas jau krito prieš migraciją:** raudonas raundas (CI 33854194027) parodė, kad visi keturi sargai krenta su `42703` (stulpelio nėra), ne su `23514` — t. y. testas jautė migracijos NEBUVIMĄ, o ne kiekvieną dalį atskirai. ⚠️ **APIMTIS SUSIAURINTA (§12.1, #157 PR-6):** antraštė „kiekviena" teigė daugiau, nei įrodyta. Galioja TRYS external šakos dalys (`payload`, `bytes`, `checksum`) — joms `PILNA_FORMA` `ELSE` šaka sutampa su migracija, tad mutacija ir atstatymas tikri. **NEGALIOJA ketvirtoji, `inline` metaduomenų dalis:** jos „mutacija" ir „pilna forma" yra tas pats tekstas, tad ji nemutuoja. Taisymas — #305, 5 punktas |
| ⚠️ **KONTROLĖ: mutacija tikrai atstatoma** (dalinė) | `jobResultsExternalShape.integration` | Be jos mutacijos rodytų tik tai, kad susilpninta forma praleidžia, bet ne kad pilna — atmeta. ⚠️ **PATIKSLINTA (§12.1, #157 PR-6):** „po kiekvienos mutacijos" buvo netikslu. Kontrolė vykdoma VIENĄ kartą (`:431`), PRIEŠ `inline` šakos mutaciją (`:451`), ir tikrina tik EXTERNAL eilutes. Todėl ji negali pastebėti, kad `PILNA_FORMA` atstato susilpnintą `inline` šaką — būtent dėl to gretimos eilutės klaida liko nepamatyta. Taisymas — #305, 5 punktas |
...
| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **Ribos priimtų reikšmių aibė telpa į `jsonb` aibę — REPREZENTATYVIŲ KANDIDATŲ aprėptis** | `jobResultsJsonbDomain.integration` | ⚠️ **Tai regresijos, ne visuotinė garantija (§12.1):** `jsonb` sritis begalinė, tad imamas po vieną atstovą kiekvienai ribos taisyklei ir po vieną teisėtą kaimyną greta. ⚠️ **Kryptis svarbi:** riba gali būti GRIEŽTESNĖ už PG (uniformizacijos kaina), bet niekada ŠVELNESNĖ — praleista reikšmė sulaužytų inline diegimą jau PO sėkmingo `put()`. Kontrolė: bent viena reikšmė praeina abi puses, bent viena atmetama ribos. ⚠️ **Testas iškart RADO pažeidimą (CI 33909895874): riba praleido neporinį surogatą, kurį PG atmeta (`22P02`).** Iki tol NUL atmetimas rėmėsi dokumentacija, ne matavimu (§14.1); dabar abu išmatuoti |
| ⚠️ **Neporinis surogatas atmetamas ties riba** | `artifactStoreContract`, `jobResultsJsonbDomain.integration` | ⚠️ **`fs` ir S3 to NEPARODYTŲ:** `JSON.stringify` jį užkoduoja kaip ASCII escape, tad kanoninėje eilutėje raw simbolio nėra ir round-trip nelūžta. Klasė matoma tik prieš PG. Reali kilmė — teksto pjūvis per emoji. **Kontrolė:** tekstas, kuriame LITERALIAI parašyta `\ud800`, privalo PRAEITI (aptikimas skaičiuoja nelyginį pasvirųjų brūkšnių skaičių) |
...
| ⚠️ **TAS PATS kontraktas galioja `s3` (S3-suderinama saugykla)** | `artifactStoreS3.integration` | Rinkinys paleidžiamas NEKEIČIAMAS prieš tikrą S3-suderinamą saugyklą. ⚠️ **TIEKĖJAS NEĮVARDIJAMAS (#405, P3):** išmatuota, kad seaweedfs, garage, adobe/s3mock ir localstack praeina rinkinį IDENTIŠKAI (25/25), tad tikrinamas kontraktas, ne serveris. S3 yra pirmas TIKRAI nutolęs backend'as: tinklo latencija, kita klaidų taksonomija, `ETag` semantika, brangus `head`. ⚠️ Rinkinys tris kartus tyliai perėmė `fs` savybes (kelio forma, sinchroniškumas, rakto gamyba) — kiekvieną kartą tai pasimatė tik prieš antrą backend'ą |
| ⚠️ **`get` kelias tikrinamas ATSKIRAI nuo `put`** | `artifactStoreS3.integration` | Checksum problema rašymo ir skaitymo pusėse pasireiškia SKIRTINGAI: `PutObject` lūžta dėl trūkstamos `Content-Md5`, o `GetObject` — dėl pasirašytos `x-amz-checksum-mode` antraštės, į kurią S3-suderinami tiekėjai atsako `SignatureDoesNotMatch`. Testas, dengiantis tik `put`, praleistų pusę klasės ir liktų žalias |
| ⚠️ **Checksum nustatymai realiai PATENKA į klientą** | `artifactStoreS3Config` | Tvirtinama, kad klientas neša `WHEN_REQUIRED`; pašalinus juos iš konstruktoriaus SDK grąžina `WHEN_SUPPORTED`, ir testas krenta BE tinklo. Kontrolė: SDK numatytoji reikšmė privalo SKIRTIS — kitaip tvirtinimas nieko neįrodytų apie mūsų konstruktorių |
| ⚠️ **INFORMACINIS matavimas, ne garantija** | `artifactStoreS3.integration` | ⚠️ **Išmatuota DUKART, prieš dvi skirtingas saugyklas:** MinIO `RELEASE.2025-04-22` (CI 33946366087) ir seaweedfs 4.47 (CI 36181198716) — abiem `rašymas=praėjo, skaitymas=praėjo`. Abi numatytuosius nustatymus jau palaiko, tad jų pašalinimas prieš jas nieko nesulaužo — vadinti tai mutacija reikštų teigti daugiau, nei ji daro. Testas lieka, nes pasakys, kada pririšta saugykla ims elgtis kitaip; enforcement gyvena `artifactStoreS3Config` |
| ⚠️ **Riba be reikalo nėra GRIEŽTESNĖ už `jsonb`** | `jobResultsJsonbDomain.integration`, `artifactStoreContract` | ⚠️ **Antra kryptis, ir joje BUVO defektas:** substring patikra atmesdavo tekstą, kuriame NUL escape parašytas LITERALIAI (programinio kodo transkripcija) — PG jį priima. Teisėtas rezultatas niekada nebūtų išsaugotas. Dabar „tikras escape, ne tekstas apie escape" yra VIENA taisyklė (`tikroEscapeSablonas`), iš kurios išvedami ir NUL, ir surogato šablonai |
| ⚠️ **Ne baigtinės reikšmės viršutiniame lygyje atmetamos** | `artifactStoreContract`, `artifactStoreErrors` | `NaN`/`±Infinity` praeidavo `null`/`undefined` sargą, o kanonizavimas juos paverčia `null` (išmatuota) — atkurdamas būtent tą „`completed` be rezultato" būseną, kurią sargas uždarė. **Viduje** esantis `NaN` NEATMETAMAS: jis virsta `null` vienodai visuose backend'uose (`NUOSTOLINGI` paritetas) |
| ⚠️ **Skaitymas taiko TĄ PAČIĄ reikšmių sritį kaip rašymas — VISUOSE trijuose backend’uose** | `artifactStoreCodec`, `artifactStoreContract` |⚠️ **Sritis, taikoma dviem iš trijų, nėra sritis:** inline grąžindavo `payload` tiesiai, tad tas pats netinkamas turinys viename diegime tapdavo `ARTIFACT_CORRUPT`, o kitame „galiojančiu rezultatu“. `jsonb` reikšmė `'null'` stulpeliui teisėta, rezultatui — ne. `put()` atmesdavo viršutinio lygio `null`, NUL ir neporinį surogatą, o `read()` juos PRIIMDAVO — atkurtas ar iš išorės pakeistas objektas galėjo grąžinti tai, ko riba niekada neįsileido. Literalus `null` atkuria „`completed` be rezultato" būseną, po kurios terminalus valymas ištrina šaltinio audio. Sritis gyvena vienoje funkcijoje (`patikrintiSriti`), kodavimas ir dekodavimas — dvi jos kryptys. Kontrolė: teisėtas turinys perskaitomas nepakitęs |
| ⚠️ **Netaisyklingas UTF-8 yra SUGADINIMAS, ne tyliai pakeisti duomenys** | `artifactStoreCodec` | `Buffer.toString("utf8")` blogus baitus pakeičia U+FFFD ir grąžina „sėkmę": `JSON.parse` pavyksta, o kvietėjas gauna PAKEISTUS vartotojo duomenis. Griežtas (`fatal`) dekoderis paverčia tai `ARTIFACT_CORRUPT` — tyli duomenų mutacija blogiau nei klaida |
| ⚠️ **Rakto segmentas ribojamas BAITAIS (255), ne simboliais** | `artifactStoreCodec` | `NAME_MAX` ext4/XFS/APFS yra 255 baitai; riba, matavusi tik bendrą ilgį, praleisdavo raktą, kurį `fs` atmesdavo žaliu `ENAMETOOLONG` — teisėtas kontrakte, neįmanomas vienoje implementacijoje. Aibė imama iš to backend'o, kuris gali mažiausia. `fs` papildomai verčia bet kokį `ENAMETOOLONG` į `ARTIFACT_KEY_INVALID`, nes riba nežino kiekvienos failų sistemos |
| ⚠️ **Artefakto turinys NEPATENKA į savininkui matomą klaidos lauką** | `artifactStoreMessages` | `JSON.parse` diagnostikoje Node cituoja nepavykusią vietą — t. y. transkripcijos fragmentą (asmenvardžiai, adresai, sveikatos informacija); `_classifyError()` jį grąžindavo nepakeistą į job'o klaidos įrašą, kurį savininkas mato per `GET /api/jobs/:id`. Dabar viešas tekstas gaminamas iš KODO, pilnas — į serverio logą. ⚠️ **Node versija keičia nuotėkio dydį, ne jo egzistavimą** (18: vienas simbolis, 22: iki dešimties), tad testuojama TAISYKLĖ, o ne fragmento nebuvimas |
| ⚠️ **SUGADINTAS ≠ NERASTAS** | `artifactStoreMessages`, `artifactStoreContract` | Netaisyklingas JSON esančiame objekte buvo ženklinamas `ARTIFACT_NOT_FOUND` — atkūrimas eitų į atsarginės kopijos paiešką, nors objektas guli vietoje ir reikia vientisumo tyrimo. Naujas kodas `ARTIFACT_CORRUPT`; parserio diagnostika lieka atskirame lauke serverio logui |
| ⚠️ **`verify()` priima metaduomenis TOKIUS, KOKIE ateina iš DB** | `artifactStoreVerifyMetadata.integration`, `artifactStoreContract` | ⚠️ **P1:** `job_results.bytes` yra `bigint`, o `node-postgres` jį grąžina EILUTE; griežtas `===` lygintų `"12"` su `12`, ir KIEKVIENAS DB paremtas artefaktas atrodytų sugadintas — 7.6 restore verifikacija taptų beverte būtent tada, kai ja remiamasi. Normalizuojama VIENOJE vietoje ir tik LAUKIAMA pusė. Tipas **išmatuojamas** prieš tikrą stulpelį, ne imituojamas; kontrolė: pakeitus turinį, tas pats verdiktas tampa `ok: false` |
| ⚠️ **`verify()` yra PILNA funkcija** | `artifactStoreContract` | Nesančio objekto verdiktas turi TĄ PATĮ laukų rinkinį kaip rastas, įskaitant `nepriklausomas` — kitaip PR-7 ataskaitoje atsirastų trečia, `undefined` būsena būtent tame lauke, pagal kurį ji skirsto. Forma gyvena vienoje vietoje (`nesancioVerdiktas`), o ne trijose implementacijose |
| ⚠️ **Objektas, dingęs tarp `head()` ir skaitymo, duoda „nėra", ne žalią klaidą** | `artifactStoreContract`, `artifactStoreFsBoundary` | Erasure kelias trina lygiagrečiai, tad langas realus. `ENOENT` čia reikštų nutrūkusią 7.6 ataskaitą vietoj eilutės „objekto nebėra" |
| ⚠️ **Backend'as DEKLARUOJA savo semantiką, o kontraktas ją tikrina** | `artifactStoreContract`, `artifactStoreInline.integration` | Tikrinti tik TIPĄ („`reference` yra `null` arba adresas") reiškia, kad regresija lieka žalia: inline, pradėjęs grąžinti `job_id` kaip nuorodą, praeitų vartus, o pirmas kvietėjas, kuris ją persistintų, gautų `23514`. Fixture deklaruoja `nuoroda` ir `nepriklausomas`; EXTERNAL pakopa išvedama iš `nuoroda`, ne atskiras jungiklis |
| ⚠️ **Inline `verify()` NĖRA nepriklausomas — ir tai tvirtinama** | `artifactStoreInline.integration` | Inline `nepriklausomas: false` tikrinamas EKSPLICITIŠKAI (ir rastam, ir nesančiam objektui). Regresija į `true` priverstų PR-7 ataskaitą skaičiuoti iš to paties `payload` išvestą sumą kaip nepriklausomą vientisumo įrodymą |
| ⚠️ **Logai neša TIK kodą** | `artifactStoreMessages` | `_classifyError()` rašydavo pilną `message` ir parserio diagnostiką į `log.error`; logger'io euristinė redakcija savavališkų vardų nešalina, tad centralizuoti logai išlaikydavo tai, ką viešas pranešimas paslėpė — tas pats nuotėkis pro kitas duris. Testas su sentinel eilute tikrina VIEŠĄ pranešimą, logą IR `cause` grandinę |
| ⚠️ **Starto patikra PRIVALOMA visiems backend’ams** | `artifactStoreSelection`, `artifactStoreContract`, `artifactStoreFsBoundary` | `fs` fail-fast neívykdavo, nes metodą turėjo tik S3, o factory laukė sąlygiškai — netinkamas `ARTIFACT_FS_ROOT` paaiškėdavo per pirmą operaciją, jau PO brangaus tiekėjo darbo. `patikrintiSaugykla()` dabar yra privalomas kontrakto metodas, o factory kviečia jį be sąlygų |
| ⚠️ **Valymo nesėkmė PRANEŠAMA, o ne nurijama** | `artifactStoreFsBoundary` | `rm(...).catch(() => {})` ištrynimo garantijų grandinėje yra tas pats, kas jokio valymo: jautrus nereferencuotas objektas likdavo, o apie tai nesužinodavo niekas. Dabar trynimas tikrinamas, katalogas po jo sinchronizuojamas (kitaip failas GRĪŽTA po avarijos), o nesėkmė gauna savo kodą `ARTIFACT_ORPHAN_LEFT` ir logo eilutę su raktu — bet be turinio. Gedimai įterpiami į `rename`, `fsync` ir `rm` |
| ⚠️ **S3 protokolo reikšmės validuojamos PRIEŠ konversiją** | `artifactStoreS3Protocol` | `Number(null)`, `Number("")` ir `Number(false)` duoda `0`: tikrinant po konversijos, netaisyklingas atsakymas virsdavo `{ exists: true, bytes: 0 }`. Priimamas tik SDK dokumentuotas `number`; versijavimui — tik `Status === undefined`; kūnui — tik `Symbol.asyncIterator` (`pipe()` nieko negarantuoja, nes vartotojai naudoja `for await`). Lentelės su teigiamais ir neigiamais atvejais |
| ⚠️ **Metaduomenų kelio FORMA vienoda visuose backend’uose** | `jobStoreBackendContract.integration`, `jobStoreHydrationFlag` | Kontraktas tikrino metodų aibę ir ELGESĮ, bet ne grąžinamą FORMĄ: PG `listByFlag()` nustojo grąžinti `result`, o memory/Redis jį grąžino toliau — ir rinkinys liko žalias. Laukų aibė lyginama TARP backend’ų (ne su ranka surašytu sąrašu, kuris pasentų su pirmu nauju lauku). **Mutacija:** grąžinus `result` viename backend’e — krenta |
| ⚠️ **Bendri stulpelių vardai nepatenka į rezultatą nealiasuoti** | `jobStoreHydration.integration` | `jobs` ir `job_results` dalijasi `storage_key` IR `created_at`. Pirmoji kolizija jau kainavo CI raundą (33984736988); `created_at` atveju ji būtų blogesnė — abi reikšmės timestamp’ai, tad joks tipo tvirtinimas nekristų. **Bendrų vardų sąrašas išvedamas iš `information_schema`**, ne surašomas: pridėjus stulpelį, sargas apie jį sužino pats. Plius tikslinis sargas — `job_results.created_at` užsėjamas kita reikšme, `job.createdAt` privalo nepasikeisti |
| ⚠️ **Saugykla parenkama pagal EILUTĘ, ne pagal konfigūraciją** | `jobStoreHydration.integration` | Autoritetas yra persistintas `result_storage_type`: diegimas, kuriame dalis rezultatų jau `s3`, o nauji rašomi į `fs`, yra NORMALI būsena po backend’o pakeitimo. Saugyklos raktuojamos pagal tipą; neregistruotas ar nežinomas tipas — aiški klaida, ne skaitymas iš to, kas po ranka (`s3` eilutė per `fs` saugyklą tuo pačiu raktu ir sutampančiu dydžiu duotų SVETIMĄ turinį). Testas registruoja DU tipus ir tikrina, kad kiekviena eilutė skaitoma per savo saugyklą |
| ⚠️ **Hidratacija susieta su AUTORIZUOTU snapshot’u** | `jobStoreHydration.integration` | Du nepriklausomi skaitymai (metaduomenų su `scope` ir turinio BE `scope`) paliko langą: atkūrimas ar administracinis kelias gali pakeisti to paties UUID savininką, ir kvietėjas gautų jau naujo savininko rezultatą. Dabar: sprendimas ir turinys iš TOS PAČIOS eilutės; external artefaktas skaitomas pagal SNAPSHOT’O raktą; po skaitymo savininkas ir `version` tikrinami dar kartą, o nesutapus turinys ATMETAMAS. Inline atveju pakartotinės patikros nėra ir nereikia — turinys atėjo ta pačia užklausa kaip savininkas. Testai ties draiverio riba: savininko pakeitimas ir versijos pokytis tarp skaitymų |
| ⚠️ **Registro TVARKA tikrinama per produkcinį kelią** | `externalCompletion.integration` | `attemptRegistry` vienetinis testas niekada nekviečia `finishAtomic()`: perkėlus `registruoti()` ŽEMIAU `put()`, jis liktų žalias — o būtent ta tvarka yra viso registro esmė. Dublis rašymo metu klausia DB, ar bandymo eilutė jau yra (`pending`) |
| ⚠️ **Lenktynės SINCHRONIZUOJAMOS, ne tikimasi** | `externalCompletion.integration` | `Promise.all()` negarantuoja lenktynių: nuoseklus vykdymas praeitų su visomis asercijomis, nieko neišbandęs. Barjeras ties saugyklos riba sulaiko abu bandymus, kol jiedu paruošė objektus, ir tik tada leidžia eiti į transakciją |
| ⚠️ **Būsenų aibė lyginama, ne grep’inama** | `attemptRegistry` | `committed` pašalinimas iš migracijos masyvo praeitų, nes žodis lieka komentare (§9.2). Aibė išrenkama iš migracijos apibrėžties ir lyginama TIKSLIAI — trūkstama ir perteklinė būsena abi yra defektai. **Mutacijos abiem kryptim: krenta** |
| ⚠️ **Job’as turi DAUGIAUSIA VIENĄ įsipareigotą bandymą** | `externalCompletion.integration` | Po remonto registre likdavo DU `committed` įrašai — senasis (kurio objekto nebėra) ir naujasis. Registras teigtų, kad naudojami du objektai, o šlavėjas (PR-5) senojo niekada neliestų: jis atrodytų reikalingas. Spragą rado testas („du lygiagretūs remontai“), ne konstrukcija, tad invariantas perkeltas į DB: dalinis unikalus indeksas `UNIQUE (job_id) WHERE busena = 'committed'` (migracija `1756400000000`) daro antrą įsipareigotą bandymą NEIŠREIŠKIAMĄ — tikrinama APLENKIANT registro API (tiesioginis `UPDATE` → `23505`). Indeksas dalinis, tad neatidedamas (`DEFERRABLE` reikalauja constraint'o, o constraint'as negali būti dalinis), ir perėjimas skaidomas į DU sakinius toje pačioje transakcijoje: pirma nuvertinimas, tada įsipareigojimas. Vienas `CASE` sakinys kristų ATSITIKTINAI — eilučių tvarka jame neapibrėžta |
| ⚠️ **Konfliktuojanti saugyklų registracija KRENTA konstrukcijos metu** | `artifactStoreRegistracija` | `rasymoSaugykla` buvo įrašoma pirma, o `artifactStores` ciklas ją TYLIAI perrašydavo: su `fs` root A rašymui ir root B skaitymui job'as būtų sėkmingai užbaigtas su NEPERSKAITOMU rezultatu (nuoroda į A, skaitymai per B). Fail-closed, ne „rašymo saugykla laimi": pastarasis paliktų klaidingą konfigūraciją veikiančią ir tylią. Tikrinama TAPATYBĖ, ne `backend` sutapimas — ta pati saugykla abiejose vietose yra normalus PR-7 prijungimas. Dengiami visi trys įėjimai, įskaitant `artifactStore` vienaskaitą |
| ⚠️ **Nesėkmė palieka VALOMĄ, ne laukiantį pėdsaką** | `externalCompletion.integration` | `paruostiExternalRasyma()` kviečiama PRIEŠ `finishAtomic()` `try` bloką, tad `head()` klaida po sėkmingo `put()` atmesdavo dar prieš priskyrimą — `isvalytiBandyma()` nebūdavo kviečiamas niekada. Klaida bendrinė, tad worker'is laiko ją ATKARTOJAMA: kiekvienas pakartojimas paliktų dar vieną transkripciją. Sinchroninis valymo kelias egzistavo ir buvo nepasiekiamas. Tikrinami DU vykdymai iš eilės: nulis susikaupusių objektų, nė viena eilutė nelieka `pending` |
| ⚠️ **Nutekėjęs `pg` klientas baigiasi ĮVARDYTU kritimu, ne 20 min timeout'u** | `pgGyvavimoCiklas.integration`, `resursuKruva` | Iki #380 negrąžintas pool'o klientas kabino `pool.end()` neribotai (`pg-pool@3.14.0`: `end()` išsisprendžia tik kai `_clients` tuščias, `index.js:140`, o `_remove` pasiekiamas tik per `idleListener` `:59` arba `release()` `:397/:404/:413`), ir CI job'as krito po 20 min kaip infrastruktūros gedimas. Dabar uždarymas ribotas, o klaida įvardija pool'ą, paimtų klientų skaičių ir priežastį |
| ⚠️ **Nutraukta jungtis NE valymo metu yra gedimas, ne fonas** | `pgGyvavimoCiklas.integration`, `resursuKruva` | Valymas pats nutraukia backend'us, tad jo sukeltos `'error'` klaidos nėra gedimas — bet identiškas pranešimas gali reikšti tikrą gedimą (testas, DB restartas). Atskiriama pagal ŽYMĘ, nustatytą prieš helper'io paties `pg_terminate_backend`, o NE pagal pranešimo regex: teksto filtras paslėptų tikrą atvejį |
| ⚠️ **Pakibęs testų failas nutraukiamas su vardu, likę failai vykdomi** | `run-tests.mjs` (`--tap-dir` kelias) | Vienas pakibęs failas anksčiau suvalgydavo visą job'ą. Dabar `spawnSync` gauna 120 s ribą (×3,6 nuo išmatuoto maksimumo 33 s), dalinė išvestis išsaugoma, prie jos prikabinama `FILE_TIMEOUT` žyma, o `ENOBUFS` atskiriamas nuo `ETIMEDOUT` |
| ⚠️ **Pavėlavęs rašytojas NEGALI įsipareigoti** | `isipareigojimoTvora.integration` | Perėjimas `pending → committed` amžiaus tvoros neturėjo: rašytojas, pradėjęs `put()` prieš valandą, įsipareigodavo PO to, kai šlavėjas jo objektą jau ištrynė — nuoroda rodydavo į nesantį adresą. Tvora gyvena SQL sakinyje su `clock_timestamp()`, ne `now()`: pastarasis yra `BEGIN` laikas, užfiksuotas dar prieš CAS ir užraktų laukimus toje pačioje transakcijoje, tad tvorą praleistų būtent tas segmentas, dėl kurio ji kuriama. `rowCount !== 1` meta ir ATŠAUKIA transakciją — `return false` paliktų įsipareigotą buvusio `committed` nuvertinimą ir perjungtą `job_results` nuorodą be gyvo bandymo |
| ⚠️ **Šlavėjo patikra SERIALIZUOJASI su commit'u** | `isipareigojimoTvora.integration` | Pakartotinė patikra prieš `delete()` skaitė eilutę be užrakto, tad rašytojas spėdavo įsipareigoti tarp patikros ir trynimo. Dabar imamas `FOR UPDATE ... NOWAIT`: `FOR KEY SHARE` netiktų — `isipareigoti()` `UPDATE` keičia ne raktinius stulpelius ir ima `FOR NO KEY UPDATE`, su kuriuo `FOR KEY SHARE` NEKONFLIKTUOJA; `SKIP LOCKED` netiktų — eilutė dingtų iš rezultato ir kvite atsirastų MELAGINGA priežastis „eilutės nebėra“. `55P03` klasifikuojama pagal `code` (ne tekstą) į `uzimtas`, visos kitos klaidos lieka `nepavyko`. Patikra vykdoma per `pool`, tad užraktas paleidžiamas PRIEŠ nuotolinį I/O — PR-4 D4 draudimas nepažeistas |
| ⚠️ **Netinkamas prikėlimo horizontas STABDO šlavimą** | `isipareigojimoTvora.integration`, `auditRetention` | `teigiamas()` tikrina ĮVESTIS, bet `horizonMs` skaičiuojamas po jo: `retry += baze * 2 ** i` su `i >= 1024` duoda `Infinity`, o `baze === 0` — `0 * Infinity === NaN` (išmatuota: `QUEUE_MAX_ATTEMPTS=1026`, `QUEUE_BACKOFF_MS=0`). `NaN >= MAX` yra `false`, tad D4 invariantas netenkinamas, ir gyva `pending` eilutė galėtų būti palaikyta šluotina. Šlavimas PRALEIDŽIAMAS su garsiu `warn`, ne pritempiamas `clamp`'u. Sargas gaudo `NaN`/`Infinity`/neigiamą; BAIGTINĖS per didelės reikšmės jis sąmoningai negaudo (riba priklausytų nuo `interval` vidinės reprezentacijos, kurios repo nefiksuoja) — ta šaka fail-closed kitu mechanizmu: sakinys krenta DB pusėje, kvietėjas gaudo, nešalinama nieko |
| ⚠️ **Žymų sąlygos posakis: be bind parametrų, alias VALIDUOJAMAS** | `erasureMarks` | Pirmas kartas repo, kai SQL tekstas keliauja tarp modulių. Kontraktas „be parametrų" buvo TIK doc komentare: parametras posakyje tyliai sujauktų kvietėjo `$1`, `$2` numeraciją, o klaida pasirodytų kitoje sakinio vietoje. Alias — vienintelė vieta, per kurią kvietėjo tekstas patenka į SQL, tad validacija tikrinama abiem kryptim (dešimt blogų atmetami, penki teisėti praeina). Testas rado tikrą spragą: `String(null)` yra `"null"`, kuris allowlist'ą PRAEIDAVO |
| ⚠️ **Per-row `storage_type`: keturi vartotojai, ne trys** | `lifecycleDeletion`, `artefactScanner`, `backupPolicy`, `artefactInventory` | Sąlyga 5 nurodė tris; inventorius rado ketvirtą — `artefactInventory` aprašai TEIGĖ fizinę vietą tekstu („jobo įraše"), o po #157 tai melas external eilutėms. `lifecycleService` turėjo DU priešingus defektus: melagingą teigiamą (raportuojama ištrinta, nors objektas liko) ir melagingą neigiamą (našlaičių kelyje `jobRemoved` visada `false`, tad artefaktai raportuojami likę net kai objektas ką tik pašalintas). Vienas predikatas — sąjunga „ar artefakto nebėra" — uždaro abu, nes klausimas suformuluotas teisingai |
| ⚠️ **Zondo GEDIMAS duoda `nepavyko`, ne `nebuvo`** | `sweepVerdiktai` | Iš keturių zondo būsenų ši vienintelė destruktyvi TYLĖJIMU: `nebuvo` uždaro registro eilutę kaip sėkmę, o eilutė yra vienintelis objekto adresas — laikinas saugyklos gedimas, palaikytas nebuvimu, padaro transkripciją nepasiekiamą galutinai. Tikrinamos abi poros pusės (zondas ir `head()`), neregistruotas `storage_type`, ir KONTROLĖ: `ENOENT` abiejose pusėse vis tiek duoda `nebuvo`, kitaip „viską verčiantis `nepavyko`" zondas praeitų |
| ⚠️ **`pazeidimas` KARANTINUOJAMAS — pranešama vieną kartą** | `sweepVerdiktai`, `registryErasure.integration` | Be žymos kiekvienas ciklas aptiktų tą patį objektą iš naujo ir vėl rašytų `log.error`; per savaitę triukšmas apie vieną failą virsta ignoravimu, o `pazeidimas` yra VIENINTELIS signalas apie pasikeitusią rakto schemą. Žyma turi pabaigą: eilutė nebešluojama, bet SKAIČIUOJAMA kiekvienoje suvestinėje, kol operatorius ją uždaro |
| ⚠️ **Šlavimo zondas: TRYS būsenos, vienas kelias** | `registryErasure.integration` | Iš `pending` eilutės neįmanoma pasakyti, kurioje `rename` pusėje nutrūko procesas: prieš jį yra tik laikinas vardas, po jo — tik galutinis, po cleanup — nė vieno. Verdiktas priimamas iš PORO, ne iš dviejų `if`: atskirai parašius, ketvirta būsena atsirastų kaip šalutinis efektas, o dvi šakos galėtų nesutarti dėl tos pačios būsenos |
| ⚠️ **Retencijos šlavėjas NETRINA svetimo job'o GYVO objekto** | `registryErasure.integration` (trys atvejai), `attemptRegistry` (kontraktas) | ⚠️ **VIENINTELIS #305 punktas, trinantis svetimus duomenis AUTOMATIŠKAI — be žmogaus, skirtingai nuo erasure.** Kandidatų užklausos tikrino tik `job_results`, tad job'o B `pending` bandymas (objektas ĮRAŠYTAS, nuoroda dar ne) A eilutės nestabdė: šlavėjas objektą ištrindavo, o B po to įsipareigodavo nuorodą į JAU NEEGZISTUOJANTĮ rezultatą. Nuosavybės sargas egzistavo tik erasure pusėje (`svetimiAdresai()`). ⚠️ **Predikatas klausia GYVYBINGUMO, ne nuosavybės** (pirmas Codex raundas): blokuoja BET KURIS gyvas bandymas tuo pačiu adresu, IŠSKYRUS pačią kandidatę (pagal `attempt_id`). Pirmoji redakcija klausė „ar savininkas kitas?" (`job_id <>`) ir praleido DU atvejus — to paties job'o gyvą `pending`, ir du pasibaigusius `pending`, blokavusius vienas kitą AMŽINAI (išorinis sakinys juos laikė šluotinais, vidinis — gyvais; dvi to paties lauko amžiaus semantikos). `abandoned` NEBLOKUOJA — to objekto nebereikia niekam. ⚠️ **Nuosavybė PERTIKRINAMA ties destruktyvia riba** (`arUzimtasGyvo()`), nes atranka mato snapshot'ą — PR-5 D šaknies recidyvas kitame kelyje; užrakto per I/O nėra sąmoningai (PR-4 D4), tad likutinis langas lieka ir yra užrašytas. Todėl retencijos ir erasure taisyklės NĖRA tos pačios (ana blokuoja visas būsenas), ir sąryšį — kad retencijos aibė yra erasure aibės POAIBIS — fiksuoja atskiras kontraktinis testas plius tripwire, neleidžiantis į erasure užklausą įrašyti būsenų filtro. ⚠️ **Praleidimas MATOMAS:** atskiras `svetimi` skaitiklis ir sava `log.warn` eilutė — `praleista` netinka, nes jo pranešimas įvardija `created_at` ateityje, ir suliejus jis meluotų. **Mutacija: pašalinus nuosavybės sąlygą → krenta pirmas testas; kontrolė („niekieno kito adresas ŠLUOJAMAS") gina, kad sargas nevirstų visada-„ne"** |
| ⚠️ **Abu adresai rasti = INVARIANTO PAŽEIDIMAS, nešalinama** | `registryErasure.integration` | Reikštų, kad `rename` neįvyko, o ankstesnis bandymas TUO PAČIU raktu paliko galutinį objektą — su attempt-unique raktais neįmanoma. Šlavėjas praneša ir netrina: pakeitus rakto schemą tai bus pirmas signalas, ir vienintelis, nes `list(prefix)` pagal A3 nėra |
| ⚠️ **Žymų saugykla ne `postgres` → šlavimas NEVYKDOMAS (žingsnio lygiu)** | `retentionSweeper` | Atminties režime `erasure_marks` tuščia, tad predikato antra šaka apsaugotų NULĮ nereferencuotų objektų, o pirmoji gina tik referencuotus — būtent tuos, kurių šlavėjas neliečia. Suvestinėje `resultAttempts: null`, ne `0`: nulis reiškia jog nieko nebuvo, o `null` reiškia jog nežinia, ar buvo |
| ⚠️ **Erasure trina PAGAL REGISTRĄ, ne pagal vieną nuorodą** | `registryErasure.integration` | `job_results.storage_key` rodo į vieną objektą — tą, kuris ir taip saugus, nes referencuotas. Pralaimėję ir nutrūkę bandymai paliko savo objektus attempt-unique adresais, ir be registro jie yra transkripcijos be jokios rodyklės. Testas tikrina ne tik ištrynimą, bet ir kad REGISTRAS buvo skaitymo šaltinis NEREFERENCUOTIEMS bandymams: pašalinus registro eilutę, objektas privalo IŠLIKTI — mutacijos ekvivalentas, įvykdytas duomenimis. ⚠️ Referencuotas objektas atrandamas ABIEM keliais (`busena: null` šaka grąžina jį ir be registro eilutės) — sąmoninga dviguba gynyba, ne spraga; platesnis teiginys būtų nepatikrintas |
| ⚠️ **Rezultato artefaktų šalinimo klaida yra KRITINĖ — DB įrašas lieka** | `jobErasure` | #157 DoD: dalinis object-storage gedimas negali būti raportuojamas kaip sėkmingas ištrynimas. Tvarka yra garantijos dalis: pašalinus job'o eilutę pirma, `job_results` dingtų per `ON DELETE CASCADE`, o su ja ir adresai — `deletionRetry` grįžtų prie pažymėto job'o ir nebeturėtų ko trinti. Saugykla be `deleteResultArtifacts()` (fasado `null`) irgi yra kritinė nesėkmė, ne no-op |
| ⚠️ **`.tmp` likutis PASIEKIAMAS per registrą** | `artifactStoreFsBoundary` | PR-2 `.tmp` klausimą NUKREIPĖ į PR-4 su pažadu „registras dengia ir šitą", bet vardas liko atsitiktinis: registre yra tik galutinis raktas, `list(prefix)` pagal A3 nėra, tad nutrūkus procesui tarp `writeFile` ir `rename` DB kryptimi orientuotas šlavėjas failo nerastų NIEKADA. Vardas dabar išvedamas iš rakto (`laikinasVardas()`), tad šlavėjas jį apskaičiuoja iš `storage_key`. Ne `<raktas>.tmp`: išmatuota, kad 255 baitų segmentas + `.tmp` = 259 baitai → `ENAMETOOLONG`, t. y. ta pati klasė, dėl kurios vardas pirmą kartą tapo atsitiktinis. Fiksuoto ilgio santrauka tenkina abu reikalavimus |
| ⚠️ **Registro `23505` NEPASIEKIAMAS produkciniu keliu** | `externalCompletion.integration` | Naujas dalinis unikalus indeksas įveda gedimo režimą, kurio repo neapdoroja: `23505` apdorojamas VIENOJE vietoje ir tik `jobs_idempotency` suvaržymui, tad registro pažeidimas sklistų kaip žalia DB klaida, o worker'is ją laikytų atkartojama ir perleistų visą transkripciją. Įrodyta ELGESIU, ne argumentu: zondas antra jungtimi promote metu gauna `55P03` (eilutė užrakinta), o kito job'o eilutę paima laisvai. Vadinasi du promote tam pačiam job'ui negali sutapti, ir indeksas yra SARGAS, ne veikimo kelias. Antra pusė: lygiagretus remontas tikrinamas per `allSettled` — DB klaida gaudoma atvirai |
| ⚠️ **Pre-check gamina STEBĖJIMUS, ne verdiktus** | `externalCompletion.integration` | Prieš užraktą priimtas sprendimas negali būti autoritetas po jo: tarp pre-check ir užrakto kitas vykdytojas gali remontą jau atlikti, ir mūsų „remontas“ perrašytų svetimą nuorodą, palikdamas nereferencuotą objektą. Pre-check fiksuoja tik „šitas raktas buvo tuščias“; remontą patvirtina transakcija, jei eilutė po užraktu vis dar rodo į tą patį raktą. **Scenarijus, kurio mutacijų lentelė nedengė:** du lygiagretūs remontai to paties dingusio objekto → vienas įsipareigotas įrašas, jokių nereferencuotų objektų |
| ⚠️ **`head()` klausia PERSISTINTO backend’o, ne aktyvaus** | `externalCompletion.integration` | #245 invariantas („persistinti metaduomenys autoritetingi, runtime konfigūracija — ne“) pažeistas trečioje vietoje. Mišrioje DB `s3` eilutės objektas, tikrinamas per `fs` saugyklą, atrodytų dingęs — ir sukeltų remontą, kurio niekas neprašė |
| ⚠️ **Kvitas ir įrašomi baitai — TA PATI serializacija (kontraktinis lygmuo)** | `artifactStoreContract`, `externalCompletion.integration` | PR-2 uždarė šią klasę `inlineStore` VIDUJE; completion kelias ją pakartojo sluoksniu aukščiau — kvitas iš `result`, o `put()` kanonizuodavo tą patį kintantį objektą antrą kartą. Taisytas PARAŠAS, ne kvietimas: `put()` priima ir jau paruoštą reprezentaciją (`paruostiReiksme()` idempotentiška, ženklas — simbolis, ne laukų spėjimas). Tikrinama abiem lygmenimis: kontrakte (saugykla pradinės reikšmės NEBESKAITO) ir per `finishAtomic()` (checksum kolona aprašo tai, kas guli saugykloje) |
| ⚠️ **Struktūrinis atmetimas SUSTABDO retry grandinę** | `artifactUnrecoverable.integration` | PR-2 padarė `neatkartojama` ženklą, bet niekas jo neskaitė — savybė be ją paisančio kelio yra dokumentacija (buvo užrašyta kaip `UNVERIFIED`). Dabar completion kelias vynioja tokias klaidas į `UnrecoverableError` su `cause`, kaip `assertResultWithinLimits` precedentas. ⚠️ **Matuojamas ELGESYS:** job'as baigiasi `failed` po VIENO vykdymo, `attemptsMade` lieka 1 — o ne lauko buvimas. Kontrolė: paprasta klaida vis dar kartojama (kitaip „retry sustabdytas" reikštų neveikiantį mechanizmą). ⚠️ **§12.1 KOREKCIJA (#298): eilutė sakė „job'as su `Date` rezultate" — `Date` nebeatmetamas NIEKUR** (žr. eilutę žemiau), tad testas naudoja NUL. ⚠️ **Grandinė uždaroma tik IKI FASADO RIBOS:** klaidos kilmė sintetinė, nes šio testo worker'is eina per atmintį, o struktūriniai atmetimai gyvena external kelyje. ⚠️ **Eilutė sakė „likutis eina su barjeru" — barjeras ATIDARYTAS (#155), o likutis neišnyko:** jam reikia ne barjero, o šio testo perrašymo prieš tikrą PostgreSQL + saugyklą worker'io kelyje. Žyma, laukusi įvykio, dabar laukia DARBO |
| ⚠️ **§9.1 mutacijos ĮVYKDYTOS, ne suplanuotos** | `externalCompletion.integration` | Trys mutacijos paleistos CI su tikra PostgreSQL (laikina šaka, po to pašalinta): **pre-check pašalintas** → krenta „pakartojimas … `put()` NEKVIEČIAMAS“ ir lygiagretumo testas (34052512327); **cleanup pašalintas** → krenta „`RESULT_CONFLICT`, o pralaimėjęs objektas IŠVALOMAS“ ir skirtingų rezultatų lenktynės (34052718738); **registro būsena neatnaujinama** → krenta „registras uždarytas“ ir ABI lenktynės (34052933537). ⚠️ Vietoje jos neįvykdomos (reikia PostgreSQL), o „turėtų pagauti“ be paleidimo yra §14.1 silpna evidencija |
| ⚠️ **Lygybės paritetas: external verdiktas = inline verdiktas** | `artifactRoundTrip`, `externalCompletion.integration` | Ta pati porų aibė (`helpers/rezultatuPoros.js`) paleidžiama abiem keliais; nesutapimas fiksuojamas kaip KRITIMAS, ne diagnostika. Checksum yra fast-path, o semantiką sprendžia `kanoninisRezultatas()` — jei jie nesutaria, teisėtas retry gautų `RESULT_CONFLICT` |
| ⚠️ **Bandymas REGISTRUOJAMAS prieš `put()`** | `externalCompletion.integration`, `attemptRegistry` | Su attempt-unique raktu kiekvienas kritęs bandymas palieka unikaliai pavadintą objektą, kurio nerodo nė viena `job_results` eilutė — o erasure eina per `storage_key`. Registras tą klasę pašalina konstrukciškai: kiekvienas mūsų parašytas objektas turi savininką ir adresą DB pusėje. ⚠️ **Kryptis sąmoninga:** geriau eilutė be objekto (šiukšlė registre), nei objektas be eilutės (transkripcija, kurios nepasiekia nei erasure, nei A3 skenavimas) |
| ⚠️ **Pakartotinis `finish()` external atveju NEKVIEČIA `put()`** | `externalCompletion.integration` | Su attempt-unique raktu „tas pats raktas, tad perrašymas nekenkia“ nebeegzistuoja: be pre-check kiekvienas pakartojimas sukurtų NAUJĄ objektą. Tikrinama skaitikliu: pakartojimas — `0` rašymų, `version` nedidėja, nuoroda neperrašoma |
| ⚠️ **Pre-check nėra verdiktas** | `externalCompletion.integration` | Sutapęs `checksum` praleidžia TIK `put()`; „tas pats rezultatas“ sprendžia DB pusėje matoma eilutės būsena transakcijoje. Taisyklė gyvena `common.js` (`idempotentiskasAtsakymasIsMetaduomenu`) — antros lygybės taisyklės nėra, o checksum yra tos pačios `kanoninisRezultatas()` išvesties santrauka |
| ⚠️ **No-op neskelbiamas nepatikrinus, ar objektas dar YRA** | `externalCompletion.integration` | Sutapęs checksum sako tik tiek, kad METADUOMENYS sutampa; objektas galėjo dingti. Neišlaikius `head()` patikros, tai nebe pakartojimas, o REMONTAS: rašomas naujas `attemptId` objektas ir perjungiama nuoroda. ⚠️ Riba: `head()` be checksum’o sugadinto, bet to paties dydžio objekto neaptiks — ta klasė lieka PR-7 restore verifikacijai |
| ⚠️ **Cleanup liečia TIK savo bandymą** | `externalCompletion.integration` | Su attempt-unique raktu pralaimėjęs FIZIŠKAI neturi kaip paliesti laimėtojo objekto — sąlyginė patikra tampa nereikalinga ne todėl, kad ją praleidžiame, o todėl, kad nebėra ką ja spręsti. Testas tikrina abi puses: pralaimėjusio objekto nebėra, laimėtojo — vietoje, o registro eilutė lieka `abandoned` (įrodymas retencijai) |
| ⚠️ **DR replay ištrina, nors artefaktas sugadintas** | `erasureReplayContract`, `restoredJobStoreForwarding` | `erasureReplay` po atkūrimo įvykdo BDAR ištrynimą, kurio reikalauja žyma — jei jį blokuotų sugadintas artefaktas, garantija turėtų skylę BŪTENT atkūrimo scenarijuje, kur artefaktai ir būna nevientisi. ⚠️ **Šaknis buvo PARAŠO NUSIAURINIMAS:** `restoredJobStore` adapteris rašė `get: (jobId) => store.get(jobId)` ir tyliai numesdavo antrąjį argumentą — taip mirtų ir bet kuris būsimas parametras. Metodai dabar GENERUOJAMI iš `BUTINI`, tad rankomis neparašytas metodas negali susiaurinti to, ko neaprašo. ⚠️ Adapterio sargas tikrina metodų VARDUS, ne parašus (§12.1 — užrašyta prie jo), o `Function.length` čia nepadėtų: `get(id, opts = {})` turi `length === 1`, kaip ir `(jobId) => ...` |
| ⚠️ **Admin override ir ištrynimo PATIKRA nehidratuoja** | `adminJobService`, `lifecycleE2E`, `jobStoreHydration.integration` | `ADMIN_DELETE_OVERRIDE` egzistuoja BŪTENT sugedusiems job’ams — tai paskutinė instancija, ir hidratuodama ji lūžtų PIRMA: job’as su sugadintu artefaktu taptų neištrinamas ABIEM keliais (savininką politika nukreipia į override, o override krenta ties hidratacija). Ta pati klasė — `artefactScanner` `JOB_RECORD` strategija: ji klausia, ar ĮRAŠAS yra, ir hidratuodama lūžtų ties tais job’ais, dėl kurių patikra ir daroma. Ne saugumo, o prieinamumo ir BDAR klausimas: neištrinama transkripcija |
| ⚠️ **`system.get()` reikalauja EKSPLICITINIO `hydrate`** | `jobStoreHydrationFlag` | Aibė nustoja būti surašoma: PR-3 metu keturios paieškos davė keturis nepilnus sąrašus, ir visos rėmėsi sintaksiniu raktu — adapteris `store` perpakuoja nauju vardu, tad paieška pagal vardą dingsta kartu su vardu. Apvertus numatytąją reikšmę, praleistus kvietėjus parodo TESTAI, ne atmintis. ⚠️ **Išmatuota: apvertimas atskleidė NULĮ naujų gamybos kvietėjų** — visi septyni jau buvo lentelėje; tai žinoma dabar, po struktūrinės patikros, o ne tada, kai sąrašas buvo skelbiamas baigtu. Viešas `jobStore.get()` nepakitęs: ten sprendžia operacija |
| ⚠️ **Hidratacija yra OPERACIJOS sprendimas** | `jobAccessHydration`, `jobs.route`, `jobStoreHydration.integration` | `DELETE` rezultato nenaudoja, bet už jį mokėjo — ir krisdavo ties hidratacija BŪTENT tada, kai objektas sugadintas, t. y. tuo atveju, kuriam ištrynimo kelias ir reikalingas. ⚠️ **Grandinė prasideda maršrute:** `resolveJobAccess()` naudoja visi interaktyvūs endpoint’ai, tad saugyklos metodų peržiūros nepakako. Vėliavą parenka OPERACIJA (`jobAccessPolicy.reikiaRezultato()`), ne kvietėjas. Matuojama dviem susietomis dalimis: maršrutas prašo `hydrate: false` (`jobs.route`), o `hydrate: false` reiškia NULĮ kreipinių į saugyklą (`jobStoreHydration.integration`) — atminties backend’as artefaktų saugyklos neturi, tad skaitiklio per maršrutą būti negali |
| ⚠️ **Nuosavybė sprendžiama PRIEŠ hidrataciją** | `jobStoreHydration.integration` | Po PR-3 `get()` gali eiti į išorinę saugyklą, tad autorizacija po jos reikštų tris dalykus: amplifikaciją (svetimas priverčia iki `MAX_RESULT_BYTES` I/O užklausai, kuri turėjo baigtis `403`), informacijos nutekėjimą per klaidas (`ARTIFACT_CORRUPT` vietoj vienodo `FORBIDDEN`) ir tai, kad savininkas nebepasiekia ištrynimo, kai artefaktas sugadintas. Tikrinama skaitikliu: atmestai užklausai — `0` |
| ⚠️ **Metaduomenų keliai NEHIDRATUOJA rezultato** | `jobStoreHydration.integration`, `jobStoreHydrationFlag` | Autorizacija, sąrašai, sweep ir `countActiveJobs()` naudoja tik būsenas, bet `SELECT_JOB` jiems deserializuodavo visą `payload` — su 20 MiB riba tai keli GiB per praėjimą. Tikrinama SKAITIKLIU pačioje saugykloje (`read` IR `verify`, nes `fs` `verify()` perskaito visą objektą; `head` skaičiuojamas atskirai ir pažeidimu nėra): metaduomenų kelias — `0`, hidratuotas — `1`. Antroji pusė yra kontrolė, be kurios patikra virstų visada-„taip“ |
| ⚠️ **Nehidratuotas job’as `result` lauko NETURI** | `jobStoreHydration.integration`, `jobStoreHydrationFlag` | `result: null` reikštų „rezultato nėra“ (`common.js` `rezultatoNera()`) — melas apie job’ą, kurio rezultatas yra; be to `applyPatch()` sprendžia pagal `"result" in job`, tad `null` reikštų nurodymą jį IŠTRINTI. Forma vienoda visuose trijuose backend’uose |
| ⚠️ **Hidratacija RIBOTA: dviguba riba** | `jobStoreHydration.integration`, `artifactStoreBoundedRead` | Pigus atmetimas pagal persistintą `bytes` (į saugyklą nesikreipiama; skaitiklis privalo likti `0`) IR kietas stabdis skaitiklyje skaitymo metu. ⚠️ **Vien persistinto `bytes` nepakanka:** objektas saugykloje gali būti perrašytas iki didesnio, ir pasenusi maža reikšmė patikrą praeitų. Viršytas `MAX_RESULT_BYTES` → `ResultLimitError`; neatitikimas deklaruotam dydžiui (didesnis ARBA mažesnis) → `ARTIFACT_CORRUPT` |
| ⚠️ **External nuoroda be saugyklos KRENTA uždarai** | `jobStoreHydration.integration` | Grąžinti `null` reikštų „`completed` be rezultato“ — būseną, po kurios terminalus valymas ištrina šaltinio audio. Kontrolė: inline eilutė hidratuojama nepaliečiant saugyklos |
| ⚠️ **Ištrynimas tikrina versijavimą IŠ NAUJO** | `artifactStoreS3Protocol` | Versijavimas yra KINTAMA tiekėjo būsena: kibiras, startavęs kaip neversijuotas, gali būti įjungtas bet kurią minutę, o įsimintas verdiktas galioja visą proceso gyvenimą — `delete()` toliau patvirtindavo ištrynimą, nors ankstesnė transkripcijos versija lieka pasiekiama. Patikra vyksta PRIEŠ `DeleteObject`, tad versijuotame kibire nesukuriamas net delete marker’is; aptikus nesaugią būseną, įsimintas verdiktas išvalomas ir kitiems keliams |
| ⚠️ **Starto patikra ZONDUOJA rašymą** | `artifactStoreFsBoundary` | `stat`/`realpath` pavyksta ir tik skaitymui prijungtai šakniai (`0555`), tad startas skelbdavo backend’ą paruoštą, o pirmas `put()` krisdavo jau PO brangaus transkribavimo. Zondo vardas prasideda tašku, kurio raktų allowlist’as NELEIDŽIA — susidūrimas su artefaktu neįmanomas, ne mažai tikėtinas; `wx` yra antra linija. Sėkmingas zondas nepalieka nieko. **Gedimas įterpiamas, ne modeliuojamas `chmod`** (CI gali veikti kaip `root`) |
| ⚠️ **Inline `verify()` sugadinimą klasifikuoja kaip `read()`** | `artifactStoreCodec` | `verify()` leisdavo prasprūsti `ARTIFACT_VALUE_UNSUPPORTED` („naujo rezultato išsaugoti negalima“), nors skaitoma tai, kas JAU guli lentelėje, ir `read()` tą pačią eilutę vadina `ARTIFACT_CORRUPT`. Dviguba semantika siųstų remontą tikrinti tiekėjo rezultato vietoj eilutės |
| ⚠️ **S3 saugykla NENAUDOJAMA, kol politika nepatikrinta** | `artifactStoreS3Protocol` | Versijavimo patikra egzistavo tik kaip eksportuotas metodas — jos NIEKAS nekvietė, tad reklamuotas fail-closed startas neįvykdavo niekada, o `delete()` galėjo patvirtinti ištrynimą kibire, kur ankstesnė transkripcijos versija lieka pasiekiama. Dabar jos laukia ir factory (startas), ir KIEKVIENA operacija (konstruktoriaus apėjimas jos neapeina). Testas tikrina, kad versijuotame kibire nė viena objekto komanda nepasiekia saugyklos |
| ⚠️ **Nežinoma versijavimo būsena failina UŽDARAI** | `artifactStoreS3Protocol` | Anksčiau atmetami buvo tik `Enabled`/`Suspended`, o trūkstamas atsakymas, `null` ar būsimas `Status` praeidavo kaip „neversijuota" — fail-open ten, kur sprendžiama, ar ištrynimas gali būti PATVIRTINTAS. Priimama tik dokumentuota neversijuoto kibiro forma. Kontrolė: ta forma praeina |
| ⚠️ **Tiekėjo atsakymas nėra kontraktas, kol nepatikrintas** | `artifactStoreS3Protocol` | `HeadObject` be `ContentLength` duodavo `bytes: NaN` su `exists: true`; `GetObject` be kūno lūždavo vėliau žaliu `TypeError` iš `for await`. Abu dabar — `ARTIFACT_STORAGE_PROTOCOL`, atskiras nuo „nėra" ir nuo „sugadinta": sugedo SAUGYKLA, ne artefaktas |
| ⚠️ **`verify()` nebuferina viso objekto — NEI `s3`, NEI `fs`** | `artifactStoreS3Protocol`, `artifactStoreFsBoundary` | Objektas, pakeistas į daug didesnį už persistintą `bytes`, išsemtų atkūrimo procesą būtent tame kelyje, kuris sugadinimą ir turi aptikti. Baitai skaičiuojami ir maiša atnaujinama gabalais, skaitymas nutraukiamas peržengus patikimą dydį (lūkestis arba `MAX_RESULT_BYTES`); verdiktas — fail-closed `ok: false` be teigiamos sumos |
| ⚠️ **ĮJUNGIMO VARDAS YRA VIENAS, IR SENAS NEBEVEIKIA** | `s3Guard` | **GALIOJA ŠIANDIEN (#405).** Vienintelis įjungimo vardas — `REQUIRE_S3=1`; kartu su juo privalomas `S3_ENDPOINT`, kitaip sargas META klaidą. ⚠️ **`REQUIRE_MINIO=1` SĄMONINGAI NEBEĮJUNGIA sargo, ir tai išmatuota:** `REQUIRE_S3=1` be `S3_ENDPOINT` → `throw`, exit 1; `REQUIRE_MINIO=1` → praeina neįjungdamas nieko. Vadinasi **bet kuris senas dokumentas ar CI kintamasis su `REQUIRE_MINIO` = TYLUS PRALEIDIMAS**, ne sargyba. Alias'as pašalintas sąmoningai: dviejų priimamų vardų kaina — CI, kuris atrodo apsaugotas ir nėra |
| ⚠️ *ISTORIJA (#290), NEBEGALIOJA* | ~~minioGuard~~ (istorinis sargas, failo nebėra) | *Iki #405 sargas vadinosi `minioGuard` ir priėmė DU vardus (`REQUIRE_MINIO` autoritetingas, `REQUIRE_S3` irgi priimamas), nes planas dokumentavo vieną, o implementacija tikrino kitą — dokumentuota komanda praleisdavo vienintelį S3 rinkinį ir baigdavosi sėkme. Alias'as panaikintas #405 kartu su tiekėjo vardu; **šiandien galioja eilutė viršuje**.* |
| ⚠️ **Negaliojanti `fs` šaknis yra KONFIGŪRACIJOS klaida, ne dingę duomenys** | `artifactStoreFsBoundary` | Šaknis, kuri yra failas, duodavo `ENOTDIR`, ir operacijos jį laikydavo „objekto nėra": `head` → `null`, `read` → `ARTIFACT_NOT_FOUND`. Remontas nueidavo atkūrimo keliu, nors trūksta ne artefakto, o saugyklos. Tikrinamos VISOS operacijos (matrica išvedama iš paviršiaus). Failų sistemos šaknis (`/`) atmetama konstruktoriuje |
| ⚠️ **Trūkstama šaknis sukuriama `0700` ir jos įrašas ĮTVIRTINAMAS** | `artifactStoreFsBoundary` | `mkdir` grąžina sėkmę, kai įrašas dar page cache; be tėvo `fsync` maitinimo dingimas po `put()` pasiimtų visą naujai sukurtą artefaktų šaknį |
| ⚠️ **Artefaktai kuriami `0600`, ne pagal `umask`** | `artifactStoreFsBoundary` | Be eksplicitinio režimo Node naudoja `0o666 & ~umask`; su įprastu `022` transkripcija lieka `0644` — perskaitoma bet kurios vietinės paskyros ar sidecar'o, pasiekiančio tą volume. Teisės dedamos LAIKINAM failui (`rename` jas išsaugo), tad turinys nė milisekundės nebūna platesnis |
| ⚠️ **Katalogas nėra objektas NĖ VIENOJE operacijoje** | `artifactStoreFsBoundary`, `artifactStoreContract` | `delete("results")` mesdavo žalią `EISDIR`, nors `head` tam pačiam raktui sako „nėra", o `inline` ir S3 grąžina `false` — tas pats įėjimas duodavo tris skirtingus atsakymus |
| ⚠️ **Symlink'as pastebimas PRIEŠ `mkdir`** | `artifactStoreFsBoundary` | Riba tikrino tik patį taikinį: kai `results` yra symlink'as, o `results/job/` dar nėra, `realpath` grąžindavo `ENOENT`, ir `mkdir` sukurdavo katalogus UŽ šaknies dar prieš raktui būnant atmestam. Dabar sprendžiamas arčiausias ESANTIS protėvis; testas tikrina, kad už šaknies neliko NIEKO |
| ⚠️ **`fs` riba taikoma per VARTUS, ne per operaciją** | `artifactStoreFsBoundary`, `artifactStoreContract` | `realpath` patikrą darė tik `head` ir `put`; `read`/`readStream`/`verify` buvo apsaugoti ATSITIKTINAI (jie kviečia `head`), o `delete` symlink'ą per katalogą praleisdavo ir ištrindavo failą UŽ šaknies. Dabar `leksinisKelias()` už resolverio nekviečiamas niekur — operacija be resolverio kelio neturi. **Mutacijos:** resolverio pašalinimas iš `delete` ir iš `head` → krinta |
| ⚠️ **Raktų matrica IŠVEDAMA iš saugyklos paviršiaus** | `artifactStoreContract`, `artifactStoreFsBoundary` | Ranka surašytas operacijų sąrašas jau buvo praleidęs `readStream`. **Mutacija:** pridėjus operaciją be matricos eilutės, rinkinys sustoja su jos vardu. Vienintelė ranka rašoma dalis (`BE_RAKTO`) klysta SAUGIA kryptimi — pamiršus, testas krenta, o ne praleidžia |
| ⚠️ **Laikino failo vardas nepriklauso nuo rakto ilgio** | `artifactStoreFsBoundary` | Riba priima 512 simbolių raktą, failų sistema — 255 baitus vienam vardui; `<raktas>.<uuid>.tmp` sufiksas raktą nuo 214 simbolių paversdavo `ENAMETOOLONG` (išmatuota). Krisdavo dėl MŪSŲ pasirinkimo, ne dėl sistemos ribos |
| ⚠️ **Gedimas PO `rename` nepalieka artefakto be savininko** | `artifactStoreFsBoundary` | `put()` meta → kvietėjas nuorodos nepersistina → objektas lieka, o DB krypties inventorius (A3) jo neranda pagal apibrėžimą. Šviežias objektas šalinamas; **buvęs — ne** (turinys po `rename` jau pakeistas, atstatyti nebėra iš ko). Abi kryptys turi testą |
| ⚠️ **Kvitas aprašo TAI, KAS ĮRAŠYTA** | `artifactStoreContract`, `artifactStoreInlineWrite`, `artifactStoreInline.integration` | Riba paskaičiuoja `bytes`/`checksum` VIENĄ kartą, ir implementacija persistina būtent ribos paruoštą turinį. Antra serializacija (inline `put()` kvietė `JSON.stringify` iš naujo) paliko langą, kuriame reikšmė spėja pasikeisti — tada `checksum` aprašo ne tai, kas įrašyta, o būtent juo remiasi 7.6 vientisumo patikra. Tikrinama nestabilia reikšme; ribos skaitymų skaičius IŠMATUOJAMAS, ne įkoduojamas |
| ⚠️ **`inline` yra teisėtas režimas, BET NE FALLBACK** | `artifactStoreSelection` | Diegimas be external konfigūracijos startuoja `inline` — tai normalu. Bet pasirinkus `fs`/`s3` su netinkama konfigūracija, grįžimas į `inline` DRAUDŽIAMAS: dalis rezultatų atsidurtų kitoje saugykloje, nei mano operatorius, ir tai paaiškėtų tik tada, kai jų prireiktų. Tikrinamas KIEKVIENAS būtinas kintamasis atskirai; kontrolė: su pilna konfigūracija backend'as parenkamas |
| ⚠️ **Vieta nekeičiama NUMANANT** | `artifactStoreSelection` | Likęs nuo bandymų `ARTIFACT_S3_BUCKET` NEPERJUNGIA backend'o — kitaip rezultatų saugojimo vieta tyliai pasikeistų, ir dalis jų atsidurtų ten, kur niekas neieškos |
| ⚠️ **404 NESUPLAKAMAS su nesančiu objektu** | `artifactStoreS3Config` | ⚠️ **Codex radinys (#290):** 404 grąžina ir neegzistuojantis KIBIRAS, ir blogas maršrutas. Suplakus juos, konfigūracijos gedimas nueitų MISSING-OBJECT keliu — tuo pačiu, kurį #157 apibrėžia kaip fail-closed remonto signalą, ir orphan patikra imtų „taisyti" tai, kas nesugedę. Tikrinami keturi ne-objekto atvejai |
| ⚠️ **Versijuotas kibiras NELEIDŽIA startuoti** | `artifactStoreS3Config` | ⚠️ **Codex P1 (#290):** `DeleteObject` versijuotame kibire palieka ankstesnę versiją, o `delete()` grąžina `true` — PATVIRTINTAS ištrynimas su išlikusia transkripcija. Fail-closed startas pasirinktas vietoj visų versijų šalinimo: pastarasis reikalautų `ListObjectVersions`, padarytų `delete()` neapibrėžtos trukmės ir vis tiek priklausytų nuo lifecycle politikos, kurios nevaldome. Kontrolė: neversijuotas praeina |
| ⚠️ **Praleidimas yra GEDIMAS, ne tyla** | `s3Guard`, `verify-postgres-suite-ran.mjs s3` | `REQUIRE_S3=1` plius per-failo „rinkinys tikrai vykdytas" sargas. ⚠️ **Čia tai svarbiau nei kitur:** checksum konfigūracija, dėl kurios testas egzistuoja, prieš AWS veiktų IR BE JOS — tik S3-compatible saugykla parodo skirtumą, tad praleistas rinkinys reikštų, kad vienintelis ją tikrinantis kelias tyliai dingo |
| ⚠️ **TAS PATS kontraktas galioja `inline`** | `artifactStoreInline.integration` | Rinkinys paleidžiamas NEKEIČIAMAS prieš tikrą PostgreSQL. ⚠️ **Čia pirmą kartą susitinka ribos predikatas ir tikras `jsonb`:** `validation.js` stabilumo patikra yra inline kelio MODELIS, o šis testas — pats kelias. Jei modelis kur nors klysta, kanoninė tapatybė po įrašymo/perskaitymo nesutaps ir round-trip scenarijus kris |
| ⚠️ **`inline`: `reference` yra `null`, `verify` — `nepriklausomas: false`** | `artifactStoreInline.integration` | `storage_key` inline eilutėje privalo būti `NULL` (PR-1 invariantas), tad grąžinus adresą kaip nuorodą pirmas kvietėjas gautų `23514`. `bytes`/`checksum` išvedami iš perskaitytos reikšmės — palyginimas lygina reikšmę SU SAVIMI, ir vėliava tai pasako garsiai |
...
| ⚠️ **Inline reprezentacijos stabilumas** | `artifactStoreContract`, `jobResultsJsonbDomain.integration` | ⚠️ **Peržiūros radinys, patvirtintas matavimu:** `kanonizuoti()` perrenka tik NUOSAVUS raktus, o `payload` keliauja per `JSON.stringify`, kviečiantį PROTOTIPE esantį `toJSON`. `Date` kanoniškai virsta `{}` (visos datos tapatingos!), o po inline round-trip'o — ISO eilute → `RESULT_CONFLICT` teisėtam retry. `fs`/S3 to neparodytų. ⚠️ **§12.1 KOREKCIJA (#298): ŠIS APRAŠYMAS BUVO DIAGNOZĖ, NE GARANTIJA.** Divergencija buvo MODELIO, ne reikšmės, problema, ir ji pašalinta iš priežasties (`kanonizuoti()` dabar kviečia `toJSON`). `Date` ties riba nebeatmetamas — jis perkeltas į `NUOSTOLINGI`: forma keičiasi, bet VIENODAI visuose backend'uose. Sargas lieka ir apimtis susiaurėjo iki klasės, kurios `kanonizuoti()` išspręsti negali — nedeterministinis `toJSON` |
...
| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **Tvarka: registras → `put()` → `verify()` → atominis perjungimas** | `artifactMigration.integration` | Ta pati tvarka kaip produkciniame completion kelyje, ir dėl tų pačių priežasčių. ⚠️ **PATAISYTA (§12.1):** eilutė tebeteigė `head()`, nors kodas jau naudoja `verify()` — ir nurodė mutaciją („`head()` grąžina neteisingą dydį"), kuri NEGALI paveikti elgesio, kurį eilutė teigia ginanti. `verify()` prieš perjungimą yra SĄLYGA 7 („vienintelė kopija"): `head()` grąžina tik baitus, tad sugadintas **to paties ilgio** objektas ją praeitų, o kitas `UPDATE` sunaikintų vienintelę galiojančią inline kopiją. **Mutacija dabar gadina TURINĮ:** objektas perrašomas tuo pačiu ilgiu, aplenkiant saugyklą → eilutė privalo likti `inline`, `payload` nepaliestas, objektas pašalintas. Kontrolė ties pačiu sugadinimu tikrina, kad ILGIS nepakito — kitaip įrodymas būtų apie kitą klasę (CI 34360090645) |
...
| ⚠️ **Rezultatai rašomi TEN, kur nurodė operatorius — arba startas krenta; tylaus grįžimo į `inline` nėra** | `jobStoreArtefaktuPrijungimas`, `jobStoreArtefaktuPrijungimas.integration` | Iki PR-7 prijungimo serveris `parinktiBackenda()` nekvietė IŠ VISO (vienintelis produkcinis kvietėjas buvo `scripts/migrate-artifacts.mjs`), tad diegimas su `ARTIFACT_STORE_BACKEND=s3` ir trūkstamu raktu startuodavo ir rašydavo `inline` — tiksliai tas grįžimas, kurį `backendSelection.js` draudžia žodžiais: „dalis rezultatų atsidurtų kitoje saugykloje, nei mano operatorius, ir tai paaiškėtų tik tada, kai jų prireiktų". **Mutacija:** `parinktiBackenda()` klaida gaudoma ir grąžinamos tuščios parinktys (tylus grįžimas) — krenta. ⚠️ **Tikrinama ir antra gedimo pusė:** kritęs startas privalo UŽDARYTI pool'ą; matuojama `pg_stat_activity`, ne kodo forma, nes procesas, kuris vis tiek nepakils, kitaip laikytų jungtis iki pabaigos. ⚠️ **Tai NĖRA 10 sąlyga** (startas su neprieinama DB ir ATIDARYTU barjeru) — čia sustabdo PR-2 kontraktas, galiojantis nuo #290, o barjeras lieka `false` |
| ⚠️ **`inline` saugykla NIEKADA nepaduodama kaip rašymo saugykla** | `jobStoreArtefaktuPrijungimas` | `inlineStore.backend` yra `"inline"`, o rašymo kelias iš to lauko gamina `storage_type` KARTU su `storage_key`; `job_results_storage_shape` inline šakai reikalauja `storage_key IS NULL`. Paduota inline saugykla ne „nieko nekeistų" — ji duotų `23514` KIEKVIENAM užbaigimui diegime, kuris #157 dar nenaudoja, ir pasimatytų ne starte, o per pirmą `finish()`. **Mutacija:** `inline` šakoje grąžinama inline saugykla — krenta. ⚠️ **Testas BE DB sąmoningai:** taisyklė nuo DB nepriklauso, o klasė per plati, kad lauktų CI raundo. Kad tokia eilutė tikrai atmetama, jau išmatuota `jobResultsShapeDomain.integration` (60/60 dalinių perėjimų atmesti `23514`) |
| ⚠️ **Stebėtojo verdiktas po prijungimo pavirsta žaliu — išmatuota, ne teigiama** | `jobStoreArtefaktuPrijungimas.integration` | ⚠️ **Tai stebėtojo (`prijungimoBusena`) PIRMAS realus panaudojimas.** „Prieš" ir „po" matuojami VIENAME teste, TA PAČIA funkcija ir prieš TĄ PAČIĄ bazę: be paduotos saugyklos verdiktas yra `rasymas_neprijungtas`, po `initializePostgres()` — `ok: true` su `rasymoBackend: 'fs'`. Du atskiri testai apie dvi skirtingas sąlygas pokytį leistų paaiškinti bet kuria iš jų. Tikrinamas ir `inline` kelias: saugykla NEPADUODAMA, verdiktas žalias, elgesys nepakitęs |
...
## #157 (PR-7) — restore verifikacija ir per-row sprendimas

| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **`finish(failed)` ant užbaigto external job'o duoda `JobPhaseError`, ne vidinę klaidą** | `finishExternalFazes.integration` | ⚠️ **Rasta MATUOJANT PRIEŠ RAŠANT, ne recenzuojant.** Non-inline sargas gyvena `job.status === COMPLETED` šakoje ir pasiglemžia GYVAVIMO CIKLO klausimą: `finish(failed)` ant užbaigto job'o su external eilute gaudavo vidinę klaidą apie „lygybės autoritetą", nors lygybės niekas neklausė. Inline kelyje tas pats perėjimas duoda `JobPhaseError`, tad sargas kūrė backend-priklausomą elgesį. **Mutacija:** grąžinus seną sargą → krenta |
| ⚠️ **External eilutė laikoma TURINČIA rezultatą — `COMPLETED_WITHOUT_RESULT` čia būtų melas** | `finishExternalFazes.integration` | `sviezias.result = eilute.payload`, o external eilutėje `payload` PRIVALOMAI yra `NULL` (formos `CHECK`), tad kelias po sargu būtų atsakęs „rezultato nėra" apie job'ą, kurio rezultatas guli saugykloje. ⚠️ **Ir tai ne kosmetika:** `COMPLETED_WITHOUT_RESULT` dokumentuotas kaip REMONTUOTINA būsena, po kurios kvietėjas gali perrašyti rezultatą — toks perrašymas external eilutę perjungtų į inline ir paliktų objektą NAŠLAIČIU. Pažodinis sargo trynimas būtų atidaręs orphan'ų kelią per remonto semantiką. Teisingas atsakymas — `RESULT_CONFLICT`, paritetas su inline. **Mutacija:** grąžinus `eilute.payload` → krenta |
| ⚠️ **Sargas lieka TIKSLIAI vienam atvejui: nėra rašymo saugyklos** | `finishExternalFazes.integration` | External eilutė diegime be rašymo saugyklos: palyginti nėra kuo ir įrašyti nėra kur, tad klaida teisinga. Bet pranešimas privalo įvardyti TIKRĄ priežastį (saugykla neprijungta), ne `storage_type`. ⚠️ **Apsauga NEPRARASTA, o PERKELTA:** tą pačią būseną #329 stebėtojas rodo `doctor` ir `/api/health/deep` išvestyje — **prieš** naudojimą, ne jo metu. **Mutacija:** pašalinus likusį sargą → krenta |
| ⚠️ **KONTROLĖ: inline idempotencija nepakito** | `finishExternalFazes.integration` | Be jos trys krentantys testai būtų suderinami su taisymu, kuris sulaužo idempotenciją apskritai: tas pats rezultatas privalo likti no-op, kitas — `RESULT_CONFLICT` |
| ⚠️ **Restore ataskaita skirsto pagal `nepriklausomas`, NE pagal `ok`** | `artifactRestoreVerify`, `artifactRestoreIntegrity.integration` | ⚠️ **Inline `ok: true` yra TIKRAS, bet TUŠČIAS:** `verify()` inline eilutei gali tik perskaičiuoti sumą iš to paties `payload`, iš kurio ją ir gamintų — lygina reikšmę su savimi. Po migracijos DB bus MIŠRI, ir inline eilučių joje bus dauguma, tad ataskaita, skaičiuojanti `ok`, rodytų beveik 100 % ir būtų **melas**: pratybos praeitų per lengvai. ⚠️ **Laukas, kurio niekas neskaito, nėra garantija** — lygiai kaip `neatkartojama` be `UnrecoverableError` (PR-4 pamoka); `nepriklausomas` egzistuoja nuo PR-2, o šis modulis pirmas pagal jį SPRENDŽIA. **Mutacijos:** skirstoma pagal `ok` → krenta; inline skaičiuojamas kaip patikrinta → krenta. Integraciniame patikrinama mišri bazė: 1 patikrinta + 1 nepatikrinama, ne „2 ok" |
| ⚠️ **`nepriklausomas: false` iš EXTERNAL saugyklos turi SAVO verdiktą** | `artifactRestoreVerify` | Suplakus jį su `nepatikrinama_inline`, backend'as, praradęs nepriklausomą patikrą, ataskaitoje atrodytų kaip teisėta inline eilutė — regresija pasislėptų po normalia būsena. Tai kontrakto pažeidimas, ir jis skaičiuojamas kaip NESĖKMĖ. **Mutacija:** verdiktas pakeičiamas į `nepatikrinama_inline` → krenta |
| ⚠️ **Laukiama vientisumo reikšmė ateina IŠ DB, niekada neperskaičiuojama iš tikrinamo objekto** | `artifactRestoreVerify`, `artifactRestoreIntegrity.integration` | Perskaičiavus ją iš to paties objekto, verifikacija lygintų objektą su savimi — tas pats tuščias `ok: true`, tik be inline pateisinimo. ⚠️ **Įrodoma priešinga kryptimi:** integraciniame sugadinamas ne objektas, o DB `checksum`; vientisas objektas privalo KRISTI. Jei laukiama būtų skaičiuojama iš objekto, testas nieko nepastebėtų. **Mutacija:** `laukiama = {}` → krenta. ⚠️ `bigint` per `node-postgres` grįžta EILUTE — normalizuojama vieną kartą, kitaip `41 !== "41"` kiekvienai eilutei |
| ⚠️ **Sugadintas TO PATIES ILGIO objektas pagaunamas — `head()` nepakanka** | `artifactRestoreIntegrity.integration` | ⚠️ **Sugadinimas išlaiko ILGĮ sąmoningai.** Pakeitus dydį, kristų `bytes` palyginimas, ir testas įrodytų SILPNESNĘ savybę: kad `head()` pakaktų. Kontrolė ties pačiu sugadinimu tikrina, kad ilgis nepakito ir sha256 pakito. Objektas gadinamas failų sistemoje, APLENKIANT saugyklą — kitaip tikrintume saugyklos rašymo kelią, ne vientisumo patikrą. ⚠️ **Kaina užrašoma:** `verify()` perskaito VISĄ objektą (`fs`/S3 checksum'o metaduomenyse neturi), tad procedūra nėra metadata kelias — ji skirta atkūrimo pratyboms, ne startui |
| ⚠️ **Neregistruotas `storage_type` bazėje — NESĖKMĖ, ne praleidimas** | `artifactRestoreVerify`, `artifactRestoreIntegrity.integration` | Praleidus, ataskaita tylėtų apie eilutes, kurių perskaityti NEĮMANOMA, ir atkūrimas būtų paskelbtas sėkmingu su nepasiekiamais rezultatais. **Mutacija:** verdiktas pakeičiamas į `nepatikrinama_inline` → krenta. Tas pats sprendimas kaip `postgresStore` skaitymo kelyje |
| ⚠️ **Per-row sprendimas: eilutė, ne konfigūracija** | `artifactRestoreIntegrity.integration`, `backupPolicy` | Konfigūracija sako, kur rašoma ŠIANDIEN; eilutė sako, kur guli JAU ESANTIS rezultatas — mišrioje DB tai skirtingi atsakymai. ⚠️ **Išmatuota, kad `backupPolicy.TABLE_BY_TYPE` produkcinių kvietėjų NETURI** (tik testai ir žmonės), tad „perkelti sprendimą į per-row" neturėjo ko perkelti: PR-7 sukūrė per-row kelią ten, kur sprendimas realiai priimamas, ir paliko žemėlapį atsakantį tik savo klausimą („kurią LENTELĘ liečia tipas"). Antras turinio požymis žemėlapyje būtų DVI vietos, atsakančios tą patį, ir viena jų — konfigūracija |

## #298 — kanoninė rezultato tapatybė

| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **Kanoninė tapatybė MODELIUOJA saugyklą, o ne siaurina įvestį** | `kanonineTapatybe`, `artifactStoreContract` (`NUOSTOLINGI`) | ⚠️ **#298: `kanonizuoti()` nekviesdavo `toJSON`, nors visos trys saugyklos serializuoja per `JSON.stringify`, kuris jį KVIEČIA.** Prieš rašymą kanoninė forma buvo `{"d":{}}`, po skaitymo — `{"d":"1970-..."}`, tad teisėtas pakartojimas gaudavo `RESULT_CONFLICT` — melagingą konfliktą apie savo patį. Išmatuota prieš TIKRĄ Redis (CI 34106486710); Redis yra aktyvus kelias, PostgreSQL tuo metu buvo už barjero (nuo #155 — pasirenkamas, o išvada nesikeičia: klausimas apie GRYNĄ funkciją, kurią visi trys keliai kviečia vienodai). ⚠️ **Taisyta priežastis, ne pasekmė:** ne atmesti reikšmes, kurių modelis neapdoroja, o suderinti modelį su saugykla. `toJSON` kviečiamas VIENĄ kartą lygyje ir su RAKTU — tikrinama TIKRUOJU `JSON.stringify` kaip etalonu, ne prielaida apie jį. ⚠️ **#298 užrašytas predikatas („objektas, turintis `toJSON`") IŠMATUOTAS KAIP PER PLATUS:** literale `toJSON` yra nuosavas, tad round-trip'ą pergyvena; laužo tik `toJSON` PROTOTIPE. **Penkios mutacijos, kiekviena krenta:** `toJSON` nekviečiamas; kviečiamas rekursiškai; raktas neperduodamas; funkcijos paliekamos objekte; funkcija masyve praleidžiama vietoj `null` (indeksų poslinkis = kitas rezultatas) |
| ⚠️ **CUTOVER: #298 nepakeitė nė vienos JAU ĮRAŠYTOS tapatybės** | `kanonineTapatybe` | `job_results.checksum` ir `job_result_attempts.checksum` yra UŽŠALDYTOS kanoninės formos — apskaičiuotos rašymo metu ir nebeperskaičiuojamos. Pakitusi tapatybė reikštų, kad teisėtas pakartojimas PO diegimo eina remonto arba `RESULT_CONFLICT` keliu. ⚠️ **Atsakymas STRUKTŪRINIS, ne statistinis:** saugykloje gulintys baitai YRA kanoninė eilutė (`paruostiReiksme()` grąžina `kanonine`), o iš `JSON.parse` gauta reikšmė prototipo `toJSON` neturi niekada — kode nėra nė vieno `JSON.parse` reviver'io (patikrinta). Tikrinama abiem kryptim: kanoninė forma perskaityta atgal duoda TĄ PAČIĄ eilutę, ir aštuonios reikšmės be prototipo `toJSON` turi PAŽODŽIUI užšaldytas eilutes — antra `kanonizuoti()` implementacija testų pusėje būtų kopija, nustojanti sutapti tyliai (#305) |

## #155 — aktyvavimo barjeras ir eksplicitinis pasirinkimas

| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **`postgres` renkamas TIK eksplicitiškai — `DATABASE_URL` neperjungia tyliai** | `jobStoreBackendSelection`, `postRestoreReconcileContract` | ⚠️ **Politikos pakeitimas, ne taisymas (#155).** Iki barjero atidarymo `DATABASE_URL` buvimas PATS pasirinkdavo `postgres`, tik pasirinkimas būdavo nuleidžiamas atgal. Atidarius be šio pakeitimo, KIEKVIENAS diegimas su `DATABASE_URL` — įskaitant nustačiusius jį sesijoms (7.3), auditui ar migracijoms — būtų persijungęs tyliai, o Redis metaduomenys NĖRA migruojami: job'ai taptų nematomi. ⚠️ **Užrašyta kaip PORA:** `POLITIKA (1/2)` rodo, kas tapo leistina (eksplicitinis kelias), `(2/2)` — kas atsirado vietoj jos (numanomo nebėra); atskirai nė vienas nepasako politikos. **Išmatuota:** `DATABASE_URL` → `memory`; `+REDIS_URL` → `redis`; `JOB_STORE_BACKEND=postgres` → `postgres`. Nė vienas esamas diegimas nepersijungia savaime |
| ⚠️ **Operatorius mato PRIEŽASTĮ, kodėl job'ų autoritetas nėra PostgreSQL** | `postRestoreReconcileContract`, `jobStoreBackendInfo` | ⚠️ **Barjeras buvo NETIESIOGINIS laidas.** `#280 P1` tikrino `jobai.barjeras === true` — kol priežastis galėjo būti TIK barjeras, vėliavos pakako. Atidarius ji tapo `false` nepraradus nė vienos ribos: asercija būtų likusi žalia pakeitus ją į `false`, o komentaras toliau tvirtintų, kad priežastis matoma. Dabar `nustatytiAsis()` perduoda `priezastis`, kurią `resolveBackendChoice()` jau skaičiuoja. ⚠️ **Reikalinga BŪTENT prieš cutover** — vienintelį kartą, kai klaida negrįžtama. ⚠️ **Sanitizacija tikrinama testu:** `priezastis` neša kintamųjų VARDUS arba literalą `numatyta`; testas paduoda DSN su slaptažodžiu ir reikalauja, kad nė viena dalis neatsirastų. **Mutacijos:** priežastis nebeperduodama → krenta; priežastis neša DSN → krenta |
| ⚠️ **„PostgreSQL sukonfigūruotas, bet job'ai atmintyje" matoma PRIEŠ restartą** | `jobStoreBackendInfo` | Po eksplicitinio pasirinkimo įvedimo ši būsena tapo DAŽNA: kiekvienas diegimas, nustatęs `DATABASE_URL` sesijoms ar auditui, ją turi. ⚠️ **`ok: true`, ne `degraded`** — procesas daro tiksliai tai, ko konfigūracija prašo; raudona varnelė reikštų gedimą ten, kur jo nėra, ir po kelių kartų operatorius jos nebeskaitytų. Bet be jos jis sužinotų tik PO restarto, kai job'ai dingtų. ⚠️ **Kontrolė dviem kryptim:** be `DATABASE_URL` eilutės NĖRA (kitaip būtų triukšmas), o `PGHOST` duoda TĄ PAČIĄ eilutę — `arNurodytaPostgres()` priima abu, ir testas, tikrinęs tik `DATABASE_URL`, praleistų `PG*` diegimus (7.4e). **Trys mutacijos, kiekviena krenta:** eilutės nėra; pažymima kaip gedimas; DSN patenka į tekstą |
| ⚠️ **Pasirinkus PostgreSQL, neprieinama DB NUTRAUKIA startą — nenusileidžia į atmintį** | `workflowIntegrity` (kad žingsnis TEBĖRA su visomis trimis asercijomis) · CI žingsnis „Fail-closed startas" (backend job'as) — pats matavimas | ⚠️ **Ne unit testas sąmoningai.** `_initializePostgresForTests` tikrino funkciją, kurios produkcinis startas tiesiogiai nekviečia; tarp jos ir tikro `node server.js` guli visa parinkimo politika, kurią šis PR ir pakeitė. ⚠️ **EXIT KODO NEPAKANKA — išmatuota:** su uždarytu barjeru tas pats startas krenta kodu 1 dėl kitos priežasties, tad kodą tikrinanti patikra būtų žalia abiem atvejais. ⚠️ **EILUTĖ IŠ PRADŽIŲ BUVO BE REPO-VIDINIO ĮRODYMO, IR TAI RADO NE PERŽIŪRA, O `check-matrix-rows`:** ji rodė TIK į `ci.yml`, tad žingsnį ištrynus ar jo asercijas nuėmus nekristų niekas, o matrica toliau skelbtų dengimą. `workflowIntegrity` testas gina liudytojo BUVIMĄ (matavimą ir toliau atlieka pats žingsnis — kitaip jo atlikti negalima). **Mutacijos:** `initializePostgres()` `catch` bloke `throw` pakeitus į `store = memoryStore; return store` → žingsnis krenta; iš `ci.yml` pašalinus priežasties `grep` → krenta `workflowIntegrity` (išmatuota). ⚠️ **Ir ta mutacija atskleidė TREČIĄ sluoksnį:** procesas vis tiek nekyla — jį sustabdo ištrynimo žymų lentelės patikra, irgi fail-closed. Todėl asercija, tikrinanti tik „job store neinicijuotas", mutacijos NEBŪTŲ pagavusi; pagauna būtent priežasties asercija |
| ⚠️ **Diagnostika rodo TIKRĄJĄ job'ų saugyklą, ne išvestinę iš `persistentStorage`** | `privacyConfig` („diagnostika įvardija PostgreSQL…", „diagnostika NEKRENTA dėl prieštaringos konfigūracijos") | ⚠️ **Formulė nepasikeitė — susitraukė aibė, kuriai ji teisinga.** `persistentStorage ? "redis" : "memory"` buvo teisinga PAGAL KONSTRUKCIJĄ, kol PostgreSQL neparinkdavo niekas; atidarius barjerą diegimas su `JOB_STORE_BACKEND=postgres` ėmė gauti `jobState: "redis"` — laukas, kurio vienintelė paskirtis yra pasakyti, KUR duomenys gyvena. **Nė vienas testas nekrito.** ⚠️ Antras testas gina tai, kad taisymas neįneštų NAUJO kritimo kelio į diagnostiką: `selectBackend()` prieštaringai aplinkai meta, o į diagnostiką ateinama BŪTENT su gedimu — grąžinamas įvardytas nežinojimas. **Mutacijos:** senoji formulė → krenta; `try/catch` nuimtas → krenta |
| ⚠️ **Konfigūracijos klaida įvardija tikrą priežastį IR veiksmą, kuris ją pašalina** | `privacyConfig` („klaida įvardija EKSPLICITINĮ PASIRINKIMĄ ir jo veiksmą") | Žinutė „PostgreSQL dar NEAKTYVUOTA (#155 aktyvavimo barjeras)" po atidarymo tapo melu, o senoji asercija `/aktyvavimo barjeras/` ją LAIKĖ vietoje. ⚠️ **Testas, kuris tikrina tekstą, yra ir to teksto sargas, ir jo kalėjimas** — pasenusį jis gina lygiai taip pat uoliai kaip teisingą. Dabar tikrinama priežastis, veiksmas (`JOB_STORE_BACKEND=postgres`) ir `doesNotMatch` ant senojo teksto. **Mutacija:** grąžinta senoji žinutė → krenta |
| ⚠️ **Operatorius mato priežastį ir KLAIDOS momentu, ne tik sėkmės** | `postRestoreReconcileContract` („D7 sargas…"), `postRestoreReconcile.integration` (#280 P1) | `RECONCILE_BACKEND_NOT_POSTGRES` žinutė turėjo `" dėl 7.2a barjero"` šaką; atidarius ji tapo nepasiekiama, ir operatorius iškart po atkūrimo matydavo tik „job'ai: memory" — be kodėl, tuo vieninteliu momentu, kai klaida negrįžtama. ⚠️ **Vėliava dingo TYLIAI: žinutės netikrino nė vienas testas.** Dabar tikrinamas ir tekstas (`job'ai: memory — numatyta`), ir tai, kad jame nebeliko barjero. Integracinė pusė tikrina, kad priežastis pereina per TIKRĄ komandą prieš tikrą bazę — ne tik kad gryna funkcija ją apskaičiuoja |

---

## #245 (#155) — viena PostgreSQL jungties formos semantika

⚠️ **Kodėl tai saugumo, o ne konfigūracijos eilutė.** Klausimas „į kurią bazę
jungiamės" po incidento yra tas pats klausimas kaip „kur nuėjo audito įrašai".
Iki #245 repo turėjo tris nepriklausomus atsakymus, o `PGSSLMODE` ir `PGOPTIONS`
galėjo pilną `DATABASE_URL` perrašyti — TLS režimą ir `search_path` schemą —
nė vienam sargui to nematant.

| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **Dviprasmiška jungties konfigūracija STABDO startą — visuose keliuose, ne tik audito** | `pgConnectionSemantics` („STARTAS FAILINA: dviprasmybė sustabdo pool'ą…", „…`validateConfig` praneša…") | ⚠️ **Pirmoji #245 redakcija šito NEDARĖ, ir tai rado ne peržiūra, o paveikto paviršiaus išvedimas.** Išmatuota: `{ DATABASE_URL, PGOPTIONS }` **be** `AUDIT_BACKEND=postgres` duodavo **0** konfigūracijos klaidų, o `deletionTombstones` pasirinkdavo `postgres` ir kildavo — o tai NUMATYTOJI konfigūracija, nes žymos PostgreSQL renkasi automatiškai. Antras tos pačios šaknies pasireiškimas: `workers/index.js` `validateConfig()` **nekviečia** iš viso, tad taisymas tik ten būtų palikęs defektą kitame adapteryje. Sargas perkeltas į `pgJungtiesNustatymai()` VIDŲ — patikrinta `grep "new Pool(\|new Client("`: penki konstravimo taškai repo, visi per šią funkciją. **Mutacijos:** chokepoint sargas pašalintas → **3 kritimai**; centrinė `validateConfig` ataskaita pašalinta → **1 kritimas** |
| ⚠️ **Slaptažodis nepatenka į klaidas, logus, diagnostiką ar testų snapshot'us** | `pgConnectionSemantics` („D2: KREDENCIALŲ konfliktas failina, bet slaptažodis niekur nepatenka") | ⚠️ **Rizika struktūrinė, ne hipotetinė.** `jungtiesTapatybe()` istoriškai suka apie `host`/`port`/`database`/`options`, o #245 reikšmingas klases praplėtė iki `user`/`password`. Natūralus „expected vs actual" diagnostinis objektas slaptažodį įtrauktų SAVAIME. Todėl palyginimo rinkinys ir rodoma tapatybė yra **du skirtingi objektai**, o klaida įvardija tik **klasę** (`taikinys`/`kredencialai`/`saugumas`/`sesija`). Tikrinami **penki kanalai**: `message`, `stack`, pilna serializacija (`Object.getOwnPropertyNames`), `dviprasmybesTekstas()`, `tapatybesTekstas()`, plius chokepoint pranešimas — atskirai DSN slaptažodžiui (eina per `parseDsn`) ir `PGPASSWORD` (eina per `val()`). **Mutacija:** rodomą tapatybę praplėtus kredencialais → krinta |
| ⚠️ **Konfliktas sprendžiamas pagal EFEKTĄ, ne pagal env kintamojo BUVIMĄ** | `pgConnectionSemantics` („D2: konfliktas sprendžiamas pagal EFEKTYVIĄ semantiką…"), `auditStoreFields`, `postRestoreReconcileContract` | ⚠️ **Ankstesnio sprendimo ATŠAUKIMAS.** `DATABASE_URL && PGHOST` buvo neteisingas abiem kryptimis: su pilnu DSN `PGHOST` `pg` semantikai neturi jokios įtakos (startas krisdavo be reikalo, o `PG*` diegime persistencijos įjungti buvo NEĮMANOMA), o `PGSSLMODE`/`PGOPTIONS`/`PGCLIENT_ENCODING` pilną DSN **perrašo** ir sargas jų nematė — `-csearch_path=…` reiškia kitą schemą, t. y. tiksliai tą „auditas kitoje vietoje" atvejį, kurio taisyklė ir siekė neleisti. **Mutacijos:** grąžintas senas predikatas → **4 kritimai**; efektyvių parametrų palyginimas pakeistas statiniu `PG*` sąrašu → **8 kritimai** |
| ⚠️ **DR tapatumo garantija po atlaisvinimo NESUSILPNĖJO** | `pgConnectionSemantics` („D2: DR kelias NESUSILPNĖJA"), `postRestoreReconcileContract` („#280 IV") | Sargo atlaisvinimas ir apsaugos praradimas atrodo vienodai: abiem atvejais senas `assert` apsiverčia. Garantiją laiko ne sargas, o **palyginimas** — su pilnu DSN efektyvus taikinys yra DSN taikinys. Tikrinamos **trys** kryptys: sutampantis `--target` su neveiksniu `PGHOST` praeina; kita bazė krinta `RECONCILE_TARGET_MISMATCH`; `PGHOST` reikšmė **netampa** galiojančiu taikiniu. **Mutacija:** statinis `PG*` sąrašas → krinta |
| ⚠️ **Diagnostika zonduoja TĄ PATĮ taikinį, kurį naudos aplikacija** | `pgConnectionSemantics` („DIAGNOSTIKA: `postgresReachability` jungiasi prie `env` taikinio…", „DIAGNOSTIKA: konflikto tekstas ATITINKA startą…") | ⚠️ **Struktūrinė patikra čia NEPAKANKA:** `pg` numatytai skaito `process.env`, tad klaidingas kelias baigtųsi TA PAČIA timeout klaida ir testas praeitų žalias. Todėl paleidžiami **du** laido protokolo serveriai (`tests/helpers/fakePostgres.js`) — vienas per `process.env`, kitas per perduotą `env` — ir liudytojas yra tai, **kuris priėmė jungtį**. Iki #245 `startupChecks` turėjo ketvirtą savo `PG*` išskleidimą: `make doctor` galėjo rodyti žalią varnelę bazei, į kurią servisas nerašo. **Mutacija:** diagnostika ir pool'ai vėl tik per DSN → **2 kritimai** (įsk. šį — svetimas serveris priima jungtį) |
| ⚠️ **`pg` aplinkos paviršius fiksuotas; nežinomas laukas yra fail-closed, ne praleidimas** | `pgConnectionSemantics` („TRIPWIRE: `pg` aplinkos paviršius nepasikeitė…", „FAIL-CLOSED: nežinomas `pg` laukas REALIAI patenka į `kita:`…") | ⚠️ **Rankinis `PG*` sąrašas šioje užduotyje suklydo TRIS kartus:** pirmoji redakcija praleido `PGSSLNEGOTIATION` ir `PGREPLICATION`, o tripwire rado dar `PGBINARY`. Todėl aibė **išvedama iš `pg`** — `Proxy` seklys ant `process.env`, zonduojant DVIEM DSN formomis (pilnu ir tuščiu; vien pilnas duoda 8 raktus vietoj 13, nes `val()` trumpina kelią). `pg` priklausomybė yra `^8.x`, o `pg/lib/connection-parameters` forma semver'io nedengia: be tripwire minoras, pridėjęs kintamąjį, sulaužytų startą **diegime, ne CI'e**. Fail-closed šaka **realiai vykdoma** — per `require.cache` `pg` klasė pakeičiama palikuone su nauju lauku. **Mutacija:** nežinomo lauko atsargą (`kita:<laukas>`) pakeitus tyliu `continue` → krinta |
| ⚠️ **`process.env` po semantikos skaičiavimo atstatoma BITAS Į BITĄ, įskaitant klaidos kelią** | `pgConnectionSemantics` („APLINKA: `process.env` atstatoma BITAS Į BITĄ…") | `pg` `val()` skaito globalų `process.env`, tad kito `env` semantikai apskaičiuoti globalą tenka laikinai pakeisti. Korektiškumas rėmėsi savybe, kurios NIEKAS nefiksavo: atstatymu per nuorodą. Perrašius į atstatymą po raktą, raktui, kurio nebuvo, `= undefined` įrašytų eilutę `"undefined"`, ir kita `pg` interpretacija būtų kitokia — tyliai. Tikrinamas identitetas, raktų aibė, reikšmės, tuščia eilutė, `"undefined"` nebuvimas ir **metanti** `fn`. **Mutacija:** atstatymas po raktą su `= undefined` → **2 kritimai** |
| ⚠️ **Jungties formos suvienodinimas NEPERJUNGIA nė vieno komponento į PostgreSQL** | `pgConnectionSemantics` („D3: `PG*`-only leidžia EKSPLICITINĮ postgres…", „D4: `sessionStore`…"), `jobStoreBackendSelection` | Sargas, kuris kartu pakeistų backend politiką, būtų tyli persistencijos regresija priešinga kryptimi. `PG*` buvimas savaime nieko neperjungia; `sessionStore` be eksplicitinio pasirinkimo lieka atmintyje; aktyvavimo barjeras nepaliestas. ⚠️ **Ankstesnė barjero patikra buvo TAUTOLOGIJA** — lygino `POSTGRES_AKTYVAVIMAS_LEISTAS` su tuo pačiu moduliu, tad praeidavo esant bet kokiai reikšmei. Dabar fiksuojama reikšmė IR pasekmė (`barjeras: false`). **Mutacijos:** `POSTGRES_AKTYVAVIMAS_LEISTAS = false` → **2 kritimai**; vienas pool'as nukreiptas į kitą bazę → **2 kritimai** |
...
⚠️ **KUR ATEINA ĮRODYMAS.** Dalis šių garantijų reikalauja TIKROS PostgreSQL, ir
vietinėje aplinkoje jos NEVYKDOMOS (egzemplioriaus nėra; provizionavimas
eksplicitiškai uždraustas). Tokios eilutės pažymėtos **[CI <run id>]**: įrodymas
ateina iš nurodyto paleidimo, kur veikia `postgres:16-alpine` servisas, o
žingsnis nustato IR `DATABASE_URL`, IR `REQUIRE_POSTGRES=1` - be pastarojo tylus
praleidimas būtų įmanomas, ir žalias job'as nieko neįrodytų.

⚠️ Statusas keistas TIK toms eilutėms, kurių testai CI loge yra `ok` - ne todėl,
kad job'as žalias (AGENTS.md §14). Run'e `33044931167` audito testų: **25 `ok`,
1 `not ok`** (biudžeto reikšmių drift'as teste, ištaisytas `e618b4c`),
**0 praleistų**.
...
| `AUDIT_BACKEND=postgres` be `AUDIT_ID_SALT` NEPAKYLA — atsitiktinė druska padarytų GDPR ištrynimą neveiksmingą po restarto | `startupChecks`, `auditPersistence.integration` **[CI 33044931167]** | Leidus atsitiktinį fallback → `removeBySubjectIdentifier()` senų įrašų neberastų ir grąžintų „ištrinta 0" |
| `AUDIT_ID_SALT_ID` privaloma; `hash_key_id` yra operatoriaus etiketė, NEIŠVESTA iš druskos | `auditPersistence.integration` **[CI 33044931167]** | Išvedus etiketę iš druskos → ji taptų orakulu druskos spėjimams tikrinti |
| ⚠️ **7.4d ATŠAUKĖ 7.4b SARGĄ:** `PRIVACY_MODE=true` × `postgres` LEIDŽIAMAS – `resolveAuditBackend()` derinio nebeatmeta, o startas išvalo `audit_log` ir garsiai įspėja KIEKVIENĄ kartą (žr. #213 skyrių) | LEIDIMAS: `auditRetention` (grynas `resolveAuditBackend()`, be DB) · PURGE IR ĮSPĖJIMAS: **[PG NOT RUN]** `auditPersistence.integration` | Grąžinus sargą → krinta `auditRetention` (patikrinta mutacija), ir visas #213 `PRIVACY_MODE` kontraktas vėl nepasiekiamas: instancija nepakyla, o persistentinių eilučių per vėliavą ištrinti nebėra kaip. Pašalinus starto įspėjimą → grįžta būtent tas tylus scenarijus, dėl kurio 7.4b sargą įvedė: migruota, sukonfigūruota ir amžinai tuščia lentelė atrodo kaip veikianti sistema |
...
| Determinuota skaitymo tvarka: VIENOJE TRANSAKCIJOJE įrašytos eilutės skaitomos įrašymo tvarka (`seq`, ne `timestamp`, ne `id`) | `auditStoreBackendContract.integration` (pg adapteris) **[CI 33044931167]** | Pakeitus `ORDER BY seq` į `ORDER BY timestamp` → `now()` vienoje transakcijoje duoda vienodus laikus ir tvarka tampa neapibrėžta |
...
| `timestamp` autoritetas yra DB (`DEFAULT now()`); aplikacijos laikas į lentelę nepatenka | `auditPersistence.integration` **[CI 33044931167]** | Perdavus `timestamp` iš aplikacijos → sugedęs NTP vienoje replikoje sumaišytų viso audito tvarką |
| APPEND-ONLY: tiesioginis SQL `UPDATE` atmetamas trigerio, įskaitant `meta` keitimą | `auditPersistence.integration` **[CI 33044931167]** | Pašalinus trigerį → įrašą apie neteisėtą veiksmą galima pataisyti per `psql` |
| Nukritęs append-only trigeris NUTRAUKIA startą (jis NĖRA `CHECK`, tad `REQUIRED_AUDIT_CONSTRAINTS` jo nemato) | `auditPersistence.integration` **[CI 33044931167]** | Pašalinus atskirą trigerio patikrą → DB su nukritusiu trigeriu startuotų sėkmingai, o auditas taptų redaguojamas |
| `DELETE` DB lygmenyje NERIBOJAMAS — sąmoningas sprendimas, be jo GDPR ištrynimas neįmanomas | `auditPersistence.integration` **[CI 33044931167]** | Atėmus `DELETE` grantą → `removeBySubjectIdentifier()` nustotų veikti |
| `REQUIRED_AUDIT_CONSTRAINTS` sąrašas PILNAS — išvedamas iš šviežiai migruotos DB, ne surašomas ranka | `auditPersistence.integration` **[CI 33044931167]** | Pridėjus `CHECK` migracijoje ir ne į sąrašą → starto barjeras praleistų DB be to invarianto |
| Neleistina `result` ir neatitinkantis `event` atmetami CHECK invariantų; DB šablonas SUTAMPA su runtime `EVENT_PATTERN` | `auditPersistence.integration` **[CI 33044931167]**, `auditStoreFields` (tripwire) | Įrašius šabloną migracijoje literalu → du autoritetai išsiskirtų tyliai, ir runtime priimtų įvykį, kurio DB nebepriima |
| RAW: plikojo job ID NĖRA nė viename stulpelyje ar `meta` lauke (`to_jsonb(t)` visai eilutei, per PRODUKCINĮ kelią) | `auditPersistence.integration` **[CI 33044931167]** | Įrašius `jobId` vietoj `subjectId` → sentinel randamas RAW eilutėje |
| RAW: transkripcijos ir prompt'o turinio nėra net tada, kai jis perduodamas TYČIA | `auditPersistence.integration` **[CI 33044931167]** | Pašalinus allowlist → sentinel patenka į `meta` JSONB |
| Auditas išlieka po instancijos sunaikinimo; dvi instancijos toje pačioje DB mato viena kitos įrašus ir ištrynimus | `auditPersistence.integration` **[CI 33044931167]** | Tai neįmanoma atmintyje iš principo - būtent dėl to 7.4b ir egzistuoja |
| Pool'o gyvavimo ciklas: `shutdown()` uždaro jungtis, ir DB pusėje (`pg_stat_activity`) jų nelieka | `auditPersistence.integration` **[CI 33044931167]** | Pašalinus `pool.end()` → po kelių restartų išnaudojamas `max_connections` |
| Laiko biudžetas: `statement_timeout` (0.7×T) + pool riba (0.2×T) TELPA į `AUDIT_WRITE_TIMEOUT_MS`, ir DB realiai NUTRAUKIA ilgą užklausą | `auditPersistence.integration` **[CI 33044931167]**, `startupChecks` (invariantas) | Sulyginus `statement_timeout` su fasado langu → fasadas visada suveiktų pirmas, DB nespėtų nutraukti, ir vėlyvas rašymas taptų neišvengiamas |
| Išsekęs pool'as duoda klaidą per ribotą laiką, o ne kabo neribotai | `auditPersistence.integration` **[CI 33044931167]** | Pašalinus `connectionTimeoutMillis` → užklausa liktų eilėje ir įsirašytų JAU PO to, kai kvietėjui pasakyta „nepavyko" |
| Produkcinis kelias su TIKRA DB klaida nesukelia `unhandledRejection`; blokuojantis įvykis atmeta veiksmą, neblokuojantis - ne | `auditPersistence.integration` **[CI 33044931167]** | Pašalinus `await` → detektorius suveikia (atmintyje ši šaka realiai nebuvo vykdoma, nes `record()` nekrisdavo) |
...
| Nauji produkciniai `auditLog.getAll()` kvietėjai DRAUDŽIAMI — whitelist TUŠČIAS, nes šiandien nė vienas produkcinis kelias neriboto skaitymo nedaro | `auditStoreFields` (tripwire, AGENTS.md §9.2) | Pridėjus `getAll()` bet kuriame produkciniame kataloge → krinta; patikrinta mutacija. Whitelist tikrinamas ir atvirkščiai: atsilikęs įrašas irgi krinta |
| ⚠️ **7.4d PAKEITĖ:** startas įspėja tik apie `AUDIT_MAX_ENTRIES` netaikymą, ir tik jei jis nustatytas – retencija veikia (žr. #213 skyrių) | `auditStoreFields` (turinys), `auditPersistence.integration` **[CI 33044931167]** (kad realiai loginama, ir kad startas TĘSIASI) | Pašalinus `log.warn` → krinta PG testas; sutrumpinus tekstą iki bendro „retencija ribota" → krinta turinio patikra, nes dingsta `7.4d` ir `docs/audit-storage.md` nuorodos |
| Auditas išlieka per PILNĄ `init → shutdown → init` ciklą, o GDPR ištrynimas PO restarto randa PRIEŠ restartą sukurtą įrašą | `auditPersistence.integration` **[CI 33044931167]** | Leidus atsitiktinę druską → antras paleidimas skaičiuotų kitą pseudonimą, `removeBySubjectIdentifier()` grąžintų „ištrinta 0" tyliai. ⚠️ KONTEINERIO restartas NEPATIKRINTAS — reikia Docker, kurio kelti negalima |
| WORKER'IAI inicijuoja audito saugyklą ir uždaro jos pool'ą — su `postgres` worker'io įvykiai pasiekia DB, o nepasiekiama audito DB NELEIDŽIA worker'iui pakilti | `auditStoreFields` | Pašalinus `auditStore.init()` arba nuėmus `await` (klaida ignoruojama) → krinta; abi mutacijos patikrintos |
| Append-only trigeris tikrinamas TOJE PAČIOJE schemoje ir privalo būti REALIAI APSAUGANTIS: `tgenabled` ∈ {`O`, `A`}. `D` (išjungtas) ir **`R` (replica-only)** atmetami | `auditPersistence.integration` **[CI 33044931167]** | `ENABLE REPLICA TRIGGER` palieka trigerį atrodantį įjungtu, bet jis suveikia tik `session_replication_role = 'replica'` - aplikacijos sesijos veikia kaip `origin`, tad `UPDATE` praeina. Testas tikrina ABI puses: kad startas atmeta IR kad tokioje DB `UPDATE` realiai praeitų. Be `pg_namespace` tiktų to paties vardo trigeris kitoje schemoje (AGENTS.md §16) |
| `audit_log_seq_unique` yra STARTO barjero dalis — tikrinamas atskirai, nes `contype = 'u'`, o `CHECK` užklausa jo nemato | `auditPersistence.integration` **[CI 33044931167]** | Numetus constraint'ą → tiesioginis INSERT gali pakartoti `seq`, ir `ORDER BY seq` - deklaruotas tvarkos autoritetas - tampa neapibrėžtas; startas privalo nutrūkti |
| Puslapis IR bendras kiekis gaunami VIENAME momentiniame vaizde (CTE + `LEFT JOIN LATERAL`) | `auditStoreBackendContract.integration` | Grąžinus atskirą `COUNT(*)` tuščiam puslapiui → lygiagretus audito rašymas tarp dviejų užklausų duotų `entries: []` su `total: 1` net esant `offset: 0` |
| Neribotas `clear()` persistentiniame režime VYKDOMAI uždraustas už testų ribų | `auditPersistence.integration` **[CI 33044931167]** | Pašalinus sargybą → atsitiktinis produkcinis kvietėjas ištrintų VISĄ audito pėdsaką, nepaisant dokumentuotos „tik subjektu apribotas trynimas" ribos |
| ⚠️ **RIBA:** `ON CONFLICT (id)` idempotencija yra SAUGYKLOS lygio savybė, ne end-to-end garantija — `record()` kiekvienam kvietimui generuoja naują UUID, o `rasytiAudita()` pakartojimo kilpos neturi, tad produkcinis kelias šios šakos nepasiekia. Stabilų `id` per pakartojimo ribą turės nešti [7.5b] | `auditStoreBackendContract.integration` | Teigti „pakartotas rašymas nesukuria antros eilutės" apie visą sistemą būtų stipriau, nei kodas daro (AGENTS.md §12.1) |
| `null` NERAŠOMAS į `meta` JSONB, bet skaitant atkuriamas kaip `null` — raktų aibė ir API kontraktas nesikeičia | `auditStoreFields`, `auditStoreBackendContract.integration` | Grąžinus `null` rašymą → kiekviena eilutė neštų visus 21 raktą neribotai augančioje lentelėje; pašalinus `iEilute()` atkūrimą → krinta round-trip ir backend'ų paritetas. `0` ir `""` privalo išlikti |
| Atmintyje saugoma eilutė UŽŠALDOMA — numatytasis backend'as neturi būti silpnesnis už persistentinį | `auditStoreFields`, `auditLog` | Pašalinus `Object.freeze()` → `saved.event = ...` pataisytų JAU ĮRAŠYTĄ įrašą, o postgres pusėje tą patį stabdo trigeris |
| Retencijos kontraktas SUTAMPA visuose operatoriaus dokumentuose: `.env.example`, README ir runbook'as įvardija priklausomybę nuo `AUDIT_BACKEND`, ne tik skaičių | `operationalProcedures` | Grąžinus „Audito retencija — 30 d." be backend'o sąlygos → krinta; patikrinta mutacija. Operatorius kitaip manytų, kad ištrynimo langas užtikrintas |
| DB laiko biudžeto invariantas tikrinamas TIK `postgres` režimui — atmintyje `AUDIT_WRITE_TIMEOUT_MS=3` yra teisėta reikšmė | `auditStoreFields` | Padarius patikrą besąlyginę → atminties diegimas nebestartuoja dėl DB invarianto, kurio jo konfigūracijoje nėra; patikrinta mutacija |
| `init(env)` perduota `AUDIT_ID_SALT` yra autoritetas IR pseudonimizacijai — vienas raktas `subject_id` ir `hash_key_id` | `auditStoreFields` | Grąžinus `process.env` kaip vienintelį šaltinį → `hash_key_id` iš injektuotos konfigūracijos, o `subject_id` iš sugeneruotos druskos; po TIKRO restarto `removeBySubjectIdentifier()` senų eilučių neberastų. Restarto testas tai UŽDENGDAVO, nes procesui lokali druska išgyvena `shutdown()` |
| `meta` privalo būti JSON OBJEKTAS (`audit_log_meta_is_object`), o skaitymas nepalaikomą formą normalizuoja, ne krinta | `auditStoreFields`, `auditPersistence.integration` **[CI 33044931167]** | JSONB priima skaliarus; be sargybos `laukas in meta` mestų `TypeError`, ir VISAS `GET /api/audit` puslapis su ta eilute grąžintų 500 — viena nuodinga eilutė padarytų žurnalą neperskaitomą |
| Įvykių šablonas migracijoje UŽŠALDYTAS literalu, o dabartinį `EVENT_PATTERN` privalo padengti bent viena migracija; startas lygina TIKRĄ `pg_get_constraintdef` su runtime šablonu | `auditStoreFields`, `auditPersistence.integration` **[CI 33044931167]** | Pakeitus `EVENT_PATTERN` be naujos migracijos → krinta (patikrinta). Grąžinus `require` migracijoje → krinta: šviežia DB gautų naują constraint'ą, atnaujinta liktų su senu, ir abi startuotų priimdamos SKIRTINGAS aibes |
| Append-only tikrinamas ELGSENA: startas atsukamoje transakcijoje įrašo zondą ir bando `UPDATE` | `auditPersistence.integration` **[CI 33044931167]** | Metaduomenų (`tgname`+`tgenabled`) patikra praleistų to paties vardo trigerį ant kitos operacijos arba su pakeista funkcija — ji įrodo, kad kažkas parašyta, ne kad veikia (AGENTS.md §9.2) |
| Serverio riba suveikia PIRMA: `statement_timeout` (0.55×T) < `query_timeout` (0.7×T), pool 0.15×T | `auditStoreFields`, `auditPersistence.integration` **[CI 33044931167]** | Sulyginus abi → `pg` klientas atmeta pirmas ir serverio užklausos NENUTRAUKIA; INSERT, spėjęs įsirašyti, praneštas kaip nepavykęs, o `suRiba()` vėlyvos sėkmės logas ir skaitiklis lieka nepasiekti |
| `AUDIT_BACKEND=postgres` priima IR `DATABASE_URL`, IR `PG*` — dokumentuotas Compose diegimas naudoja `PG*` sąmoningai (slaptažodžiai su URI simboliais) | `auditStoreFields` | Reikalavus tik `DATABASE_URL` → dokumentuotame diegime persistentinis auditas NEĮSIJUNGTŲ, o backend ir worker'iai kristų starte konfigūracijoje, kuri realiai teisinga |
| Injektuoti `PG*` PERSIUNČIAMI į pool'ą, o su `DATABASE_URL` — sąmoningai NE | `auditStoreFields` | Pašalinus persiuntimą → `init(customEnv)` su `PGHOST` tik objekte praeitų backend'o parinkimą, bet pool'as jungtųsi prie GLOBALIOS arba numatytosios DB: auditas rašytųsi ne ten, kur operatorius nurodė. ⚠️ `process.env` tripwire šito NEPAGAUNA — antrasis skaitytojas yra pati `pg` biblioteka |
| `DATABASE_URL` IR `PGHOST` kartu NUTRAUKIA startą — konfliktas, ne pirmenybė | `auditStoreFields` | Pašalinus patikrą → pool'as tyliai renkasi `DATABASE_URL`, servisas skelbia readiness, o auditas rašomas į KITĄ DB nei nurodo Compose `PG*`. Repo tai jau deklaravo, bet tik minkštame self-check'e PO `listen()` |
| Zondas įrodymu laiko TIK `restrict_violation` (`23001`); bet kuri kita klaida persiunčiama, o po zondo pool'as patikrinamas atskira užklausa | `auditPersistence.integration` **[CI 33044931167]** (zondas vykdomas kiekviename init'e) | Grąžinus tuščią `catch` → nutrūkusi jungtis ar `statement_timeout` būtų palaikyti trigerio patvirtinimu, ir startas pavyktų su NEPATIKRINTA audito DB |
| VISŲ `CHECK` invariantų tikrinami APIBRĖŽIMAI, ne tik vardai | `auditPersistence.integration` **[CI 33044931167]** | Perkūrus `audit_log_meta_is_object` kaip `CHECK (true)` → vardo patikra praeitų, o skaliarinis `meta` vėl taptų persistinamas |
| Dokumentacija ir `.env.example` įvardija `PG*` kaip PALAIKOMĄ formą ir konfliktą kaip klaidą | `auditStoreFields` | Be to operatorius su Compose `PG*` arba be reikalo konstruotų URL (slaptažodžio rezervuoti simboliai pakeistų URI reikšmę), arba manytų konfigūraciją neteisinga (AGENTS.md §12.1) |
| **VISI TRYS** produkciniai pool'ai (`jobStore`, `sessionStore`, `auditStore`) turi `error` klausytoją — neveiklios jungties klaida NENUŽUDO proceso | `auditStoreFields` | Pašalinus klausytoją bet kuriame iš trijų → krinta; patikrinta kiekvienam atskirai. Be jo `EventEmitter` neapdorotą `error` meta, Node nutraukia HTTP serverį arba worker'į, ir joks audito klaidų apdorojimas nebesuveikia — proceso nebelieka |
| Audito moduliai NESKAITO `process.env` už `KONFIG_RAKTAI` autoriteto ribų; sąrašas tikrinamas ABIEM kryptim | `auditStoreFields` (tripwire) | Pridėjus naują neregistruotą `process.env.AUDIT_*` skaitymą → krinta (patikrinta). Trys #211 peržiūros radiniai buvo tos pačios šaknies simptomai, pastebėti po vieną; ši patikra neleidžia atsirasti ketvirtam |
| `init(env)` yra VIENAS autoritetas `AUDIT_ID_SALT`, `PRIVACY_MODE` ir `AUDIT_WRITE_TIMEOUT_MS` — visi trys skaitomi iš tos pačios konfigūracijos | `auditStoreFields` | Grąžinus `process.env` kaip vienintelį šaltinį: `PRIVACY_MODE` → saugykla praneša apie sėkmingą startą ir lieka TYLIAI tuščia; `AUDIT_WRITE_TIMEOUT_MS` → fasadas praneša nesėkmę anksčiau, nei DB spėja nutraukti užklausą, ir vėlyvo rašymo langas grįžta. Abi patikrintos mutacija |
| `audit_log_seq_unique` tikrinamas pagal APIBRĖŽIMĄ (`UNIQUE (seq)`), ne vardą | `auditPersistence.integration` **[CI 33044931167]** | Nuklydusi DB su to paties vardo constraint'u ant kito stulpelio praeitų vardo patikrą, o `seq` dublikatai liktų teisėti |
...
| ⚠️ **7.4d PAKEITĖ ŠIĄ RIBĄ:** `AUDIT_RETENTION_DAYS` galioja ABIEM režimams – `postgres` eilutės šalinamos centralizuotu sweep'u; `AUDIT_MAX_ENTRIES` lieka **tik `memory`** ir retencijos taisykle NETAMPA | `auditRetention`, `docs/audit-storage.md` §9 · **[PG NOT RUN]** `auditPersistence.integration` | Perkėlus `AUDIT_MAX_ENTRIES` į DB → įrašas dingtų ne dėl termino, o dėl to, kad po jo atėjo pakankamai naujų: tylus naikinimas be politikos. Palikus seną teiginį („postgres režime įrašai NEŠALINAMI") → dokumentacija būtų silpnesnė už kodą (AGENTS.md §12.1), ir operatorius pridėtų antrą trynimo mechanizmą ant tos pačios lentelės |
...
| `AUDIT_ID_SALT_ID` privalomas TIK `postgres` režime; `memory` su druska be ID PRAEINA, bet ĮSPĖJA | `auditRotation`, `auditKeyRing` | Pašalinus įspėjimą → operatorius apie būsimą startup FAIL sužinotų tik migracijos metu; reikalavus ID visur → lūžtų esami atminties diegimai be jokios naudos (`hash_key_id` atmintyje nerašomas). Patikrinta mutacija |
...
| VIENU MOMENTU (viena transakcija) įrašytos eilutės su VIENODU `timestamp` puslapiuojamos teisingai | `auditStoreBackendContract.integration` (memory + pg) | `ORDER BY timestamp` → tvarka neapibrėžta ir puslapiavimas duoda dublikatus. ⚠️ `Date.now()` mock'as netiktų: postgres pusėje laiko autoritetas yra DB, tad vienodus laikus sukuria transakcija. Atminties adapteris dabar irgi stamps'ina vieną momentą — anksčiau jis tikrino silpnesnę sąlygą nei postgres |
| Generacijų skenavimo KLAIDA metama, o ne verčiama į `[]`; tuščia lentelė lieka teisėtu `[]` | `auditStoreFields` (**elgsenos** testas su krentančiu pool'u, ne šaltinio paieška) | Įdėjus `catch { return [] }` → našlaičių patikra praeitų tuščiai (tyli GDPR spraga), o kiekio taisyklė nuspręstų, kad VISI istoriniai raktai nebereikalingi (klaidingas FAIL). Patikrinta mutacija DVIEJOSE rijimo vietose: funkcijos viduje ir kvietimo vietoje |
...
| ⚠️ Retencijos langas yra TIKSLIOS `N × 24 h`, ne kalendorinės dienos — DST zonoje abu backend'ai duoda TĄ PAČIĄ ribą | `auditRetention` (**tripwire**: SQL forma + prašomas valandų skaičius lyginamas su atminties etalonu) · **[PG NOT RUN]** `auditPersistence.integration` (langas su LYGIAI VIENU perėjimu) | Grąžinus `interval 'N days'` → krinta abu `auditRetention` ribos testai; praleidus `× 24` daugiklį → krinta tie patys (abi mutacijos patikrintos). `timestamptz` aritmetikoje dienų laukas yra kalendorinis: DST zonoje diena yra 23 arba 25 h, tad postgres NEGRĮŽTAMAI ištrintų eilutes valanda anksčiau, nei leidžia memory kontraktas — du kartus per metus ir be jokio matomo požymio |
...
| Produkcijoje `/api/health` grąžina TIK `status` — jokio backend'o, DB ar audito lauko | `auditReadiness.route` | Naujas laukas, pridėtas neapgalvotai, krinta čia. Tikrinama raktų AIBĖ, ne konkretaus žodžio nebuvimas — `HEALTH_DETAILS` logika turi tris šakas plius `x-audit-key` išimtį |
| `/api/ready` komponentai yra loginės BŪSENOS, ne infrastruktūros vardai ar generacijų ID | `auditReadiness.route` | Readiness produkcijoje detalių neslepia (orkestruotojui reikia žinoti, kuri priklausomybė krito) — todėl tuo labiau negali nešti vardų |
| CI kviečia `verify-postgres-suite-ran.mjs` ATSKIRU žingsniu su išsaugotu TAP | `.github/workflows/ci.yml` | Skriptas, kurio niekas nekviečia, garantijos neduoda. Nuo jos priklauso 7.4b–7.4e uždarymas: `npm run test:postgres` grąžina 0 ir tada, kai viskas praleista |
| Runbook'as turi EKSPLICITINĮ perkrovimo žingsnį po našlaičių eilučių išvalymo | `docs/backup-runbook.md` §4 | Be jo operatorius išvalytų eilutes ir matytų 503, nesuprasdamas kodėl — snapshot semantika §16 dokumentuota, bet operatorius skaito runbook'ą |
...
| ⚠️ **Barjero pasiekiamumas tikrinamas per AUDITO jungtį readiness metu** | `auditReadiness.route` | `deletionTombstones.probe()` tikrina lentelę per SAVO pool'ą, o barjeras skaito ją per audito jungtį. Be atskiro zondo abu esami zondai lieka teigiami, o pirmas rašymas duoda `42P01` → CHECK FAILED → prisijungimas nustoja veikti VYKDYMO metu. Mutacija: pašalinus zondą iš `probeRuntimeReadiness()` → krinta 1 |
| Barjero zondas tikrina STULPELIUS ir ima juos iš ŽYMŲ autoriteto | `auditReadiness.route` | `SELECT 1 FROM erasure_marks` pavyksta ir tada, kai diegimas nutrūko po lentelės sukūrimo, bet prieš vėlesnę migraciją. Ketvirta stulpelių sąrašo kopija išsiskirtų; testas tikrina, kad užklausoje yra `deletionTombstones` `STULPELIAI` |
| ⚠️ **ATMINTINIS auditas duoda `auditBarrierReachable: true`** — diegimai be DB lieka paruošti | `auditReadiness.route` | Grąžinus `false`, `/api/ready` taptų 503 KIEKVIENAM mock diegimui (`docker-compose.demo.yml`, quickstart), o „nepasiekiamo barjero" testas liktų žalias: jis tikrina `false` KARTU su `auditStoreReachable: true`. Mutacija: `memoryStore.probeBarrier()` → `false` → krinta |
| Nepasiekiamas barjeras duoda ATSKIRĄ readiness priežastį, ne „auditas neveikia" | `auditReadiness.route` | Sujungus su `auditStoreReachable`, operatorius ieškotų audito gedimo ten, kur trūksta tik migracijos. Testas reikalauja `auditBarrierReachable: false` KARTU su `auditStoreReachable: true` |
| ⚠️ **`PG*` konfigūracija realiai pasiekia `erasure_marks`** | **[PG NOT RUN]** `auditErasureFinality.integration` | `ci.yml` PostgreSQL žingsnis nustato `DATABASE_URL`, tad `PGHOST` šaka - §5 P1 pataisa - prieš tikrą DB nebuvo vykdoma. Testas išskaido tą patį URL į `PG*` ir jungiasi BE `DATABASE_URL`. Mutacija: grąžinus `env.DATABASE_URL ? ...` atranką → `pasirinktiBackend` grąžina `memory` ir testas krinta |
...
| ⚠️ **Startas nutrūksta IR tada, kai serveris jungtį PRIIMA, bet neatsako** — ne tik kai atsisako jungtis | `startPoolTimeouts` (laido protokolo rankos paspaudimas užbaigiamas, tada tyla; **be tikros DB**) | Pašalinus `query_timeout` iš `jobPoolNustatymai()` → testas **KABO**, ir jo sava 15 s riba paverčia tai kritimu (išmatuota). `connectionTimeoutMillis` čia nepadeda: `pg` jį nuvalo ties `ReadyForQuery`. CI žingsnis „Fail-closed startas" šio režimo NEDENGIA — jis naudoja uždarytą portą |
| ⚠️ **Ta pati riba galioja ištrynimo žymų patikrai** — sluoksniui, kuris fail-closed elgesį realiai ir užtikrina | `startPoolTimeouts` · ADR „PATIKSLINIMAS: du gedimo režimai" | Pašalinus `query_timeout` iš `zymuPoolNustatymai()` → `deletionTombstones.init()` kabo (išmatuota). ADR prielaidos 5 matavimas parodė, kad procesą sustabdo BŪTENT ši patikra; kabantis sluoksnis duoda ne fail-closed, o fail-never |
| ⚠️ **RIBA:** `/api/ready` NEZONDUOJA job metaduomenų saugyklos — `readiness.jobStore` yra starto skląstis | ADR „EKSPOZICIJA, KURIOS ŠIS PR NEUŽDARO" | Kiekvienas kitas persistuojantis komponentas zonduojamas, o naujai autoritetingas — ne. Su `JOB_STORE_BACKEND=postgres` `/api/ready` grąžins 200 net sugedus `jobs`/`job_results`. Įrašyta, o ne nutylėta: atidarymo PR negali įjungti kelio, kurio niekas nestebi |

| ⚠️ **Gyvas startas reikalauja VISŲ keturių lentelių ir jų invariantų**, ne dviejų | `jobStoreBackendSelection` (tripwire: sąrašas IŠVEDAMAS, ne literalas) · **[PG NOT RUN]** `migrations.integration` (`DROP TABLE` → startas nutrūksta) | Grąžinus literalą `["jobs","job_results"]` → krinta tripwire (išmatuota). Dalinai migruota bazė praeidavo startą, o krisdavo per external completion (`job_result_attempts`) arba BDAR ištrynimą (`artifact_migration_progress`) — JAU PRIĖMUS SRAUTĄ |
...
**20/20 praėjo** prieš tikrą PostgreSQL ties `5e72c8b`
([run 33329316969](https://github.com/forevercornix/stenograma/actions/runs/33329316969)).

⚠️ **NUORODA PRIVALO SEKTI MEDĮ.** Ankstesnė šio skyriaus versija rėmėsi
paleidimu ties `94fb334`, kuriame buvo 18 testų – o medyje jau buvo 19 (pridėta
`executor_lost` migracija ir jos testas). Senesnis paleidimas negali įrodyti
naujesnių eilučių: pridėjus [PG] eilutę, cituojamas paleidimas atnaujinamas
kartu, arba eilutė lieka nepatikrinta.
...
| Garantija | Testai | Mutacijos įrodymas |
|---|---|---|
| ⚠️ **Pretenzija PRIEŠ destruktyvų I/O:** `mark()` grąžina `claimed`, ir tik įrašiusysis vykdo ištrynimą | `lifecycleDeletion` (`PRETENZIJA`, `202`), `adminJobService` (`svetima žyma`), `deletionResilience` (maršrutas → 202) | Ignoravus `claimed` → krinta 2 testai (patikrinta mutacija). Abi replikos kartotų tą patį eilės, saugyklos ir audito trynimą, o viena grąžintų 404 ten, kur kita 204 |
| ⚠️ **202 įrodymas yra NEPALIESTI DUOMENYS, ne statusas** | `lifecycleDeletion`, `deletionResilience` | Testas be `jobStore.system.get()` patikros praeitų ir tada, kai duomenys jau ištrinti – „susilaikėm" ir „ištrynėm ir pameluojam" atrodytų vienodai |
| ⚠️ **NEPAIMTĄ autorizuotą pakartojimą (`claim_token IS NULL`) pasiima VIENAS kvietėjas** | `lifecycleDeletion` (abi pusės), `auditAsyncCutover` (pilnas atkūrimo kelias) | ⚠️ **Ankstesnė šios eilutės formuluotė buvo klaidinga ir prieštaravo gretimai garantijai:** ji teigė, kad `pending` su `attempts > 0` NĖRA svetimas – universaliai. Po to, kai viena replika pretenziją paima, `attempts > 0` išlieka, bet `claim_token` nebėra `NULL`, ir kiekviena vėlesnė replika PRIVALO gauti 202. Palikta ji galėjo nukreipti būsimą pakeitimą atgal į dubliuotą destruktyvų darbą |
| ⚠️ **`deletion_failed` NEKARTOJAMAS automatiškai** – naują bandymą autorizuoja operatorius (`ERASURE_MARK_RETRIED`) | `lifecycleDeletion`, `adminJobService`, `deletionResilience` (503) | Pašalinus išankstinę patikrą → krinta 1 testas. ⚠️ Statusas liktų teisingas (jį pagauna vėlesnis sėkmės išvedimas), bet darbas būtų pakartotas – skirtumą mato TIK likęs `jobStore` įrašas. Pirma testo versija to netikrino ir mutaciją praleido |
| ⚠️ **Sėkmė išvedama iš GRĄŽINTOS žymos, ne iš to, kad `complete()` nemetė** | `adminJobService` (su `complete` stub'u), `auditAsyncCutover` | Ignoravus grąžinamą reikšmę → krinta 1 testas. Gynybinis sluoksnis natūraliai nepasiekiamas, tad testuojamas sąmoningai: netestuotas gynybinis sluoksnis lygus jo nebuvimui |
| ⚠️ **Mesta klaida palieka `deletion_failed`, ne `deletion_pending`** | `auditAsyncCutover` (`ŽYMOS TVARKA`) | Pašalinus → krinta 1 testas. `pending` be vykdytojo reikštų, kad kiekvienas vėlesnis `DELETE` amžinai atsako 202, o ištrynimas neįvyksta niekada. Tai pakeitė #210 kontraktą, ir testas migruotas SĄMONINGAI, ne apeitas |
| ⚠️ **Užstrigusi `pending` žyma turi išeitį: `release` → `retry` → įvykdytas ištrynimas** | `lifecycleDeletion` (pilna grandinė + neigiama pusė), **[PG CI]** `erasureMarks.integration` (CHECK) | `release` nukreipus į `deleted` → krinta 1 (patikrinta mutacija): tai būtų `force-resolve` semantika, t. y. teiginys apie duomenis, kurio niekas netikrino, ir klausimas užsidarytų negrįžtamai. `executor_lost` pakeitus į `retryable` → krinta 1: dingtų skirtumas tarp „bandymas įvyko ir nepavyko" ir „nežinoma, ar bandymas įvyko" |
| ⚠️ **`release` NEGALIMAS iš `deleted` ir `deletion_failed`** | `lifecycleDeletion` | Nuėmus sargą → krinta 1 (patikrinta mutacija). `release` taptų būdu perrašyti nesėkmės kategoriją - suklastoti įrašą apie tai, kas nutiko |
| ⚠️ **Užšaldytos aibės gali TIK plėstis; naujausia migracija = `states.js`** | `erasureMarks` (visos keturios aibės, be užkoduoto migracijos vardo) | Pridėjus reikšmę be migracijos → krinta 1. Ankstesnė testo versija lygino su VIENU užkoduotu failu; aibė nuo tada plėtėsi du kartus, ir trečias praplėtimas būtų pralindęs tyliai |
| ⚠️ **Vienas vykdytojas BE IŠIMČIŲ: pretenzija yra būsena (`claim_token`), ne akimirka** | `lifecycleDeletion` (autorizuotas pakartojimas, **vėliau atėjusi replika**, žetono valymas), **[PG CI]** `erasureMarks.integration` (du pool'ai) | Nuėmus žetono sąlygą, jo valymą arba pačią pretenziją → krinta po 4 testus (visos trys mutacijos patikrintos). ⚠️ Ankstesnė `attempts === 0` išimtis autorizuotam pakartojimui pretenzijos netaikė - visos replikos vykdydavo lygiagrečiai, t. y. DoD punktas buvo pažeistas ten, kur atrodė uždarytas |
| ⚠️ **`updated_at` compare-and-swap NETINKA - ir testas tai fiksuoja** | `lifecycleDeletion` (vėliau atėjusios replikos scenarijus) | CAS atskiria tik vienu metu skaičiusius: vėliau atėjusi replika perskaito po-pretenzijos reikšmę ir ja pasiremia. ⚠️ Antra priežastis, kurią atrado pats testas: atmintyje `updatedAt` yra milisekundės, tad `retry` ir pretenzija pataiko į tą pačią reikšmę - CAS praleistų net vienu metu atėjusius, ir tyliai |
| ⚠️ **Retencija palieka barjerą (`retention_policy`) ir gerbia svetimą pretenziją** | `lifecycleDeletion` (abi pusės) | Grąžinus bendrą `sweepExpired()` → krinta 2 (patikrinta mutacija). ⚠️ Pirmoji sargo versija leido priežiūros `sweepExpired()`, kai batch'as nepilnas - ir ji ištrindavo BŪTENT tuos job'us, kuriuos ciklas praleido dėl svetimos pretenzijos. Tą pagavo testas, ne peržiūra |
| ⚠️ **Žymėto jobo AUDIO neatkuriamas iš kopijos** | `backupRestore` (tikra kopija su audio, tikrinamas FAILAS saugykloje) | Nuėmus filtrą → krinta 1 (patikrinta mutacija). Įrašo sargas saugojo tik metaduomenis, o patys asmens duomenys grįždavo originaliu saugyklos raktu |
| ⚠️ **Sumažintas `BACKUP_RETENTION_DAYS` NESUTRUMPINA barjero** | `lifecycleDeletion` (aukščiausias vanduo + kryptis) | Nuėmus horizonto įskaitymą → krinta 1 (patikrinta mutacija). Jau išleista 90 d. kopija pergyventų žymą, ir atkūrimas iš jos ištrynimą atstatytų |
| ⚠️ **Admin override žymoje fiksuojamas OPERATORIUS, ne savininko prašymas** | `adminJobService` | Nuėmus `actorKind`/`reason` → krinta 1. ⚠️ Pirmoji mutacija IŠGYVENO: testo nebuvo, nes žymos kategorijų niekas netikrino - tik našlaičių kelyje |
| ⚠️ **Foninis sweeper'is NEKARTOJA `deletion_failed` žymėtų jobų, ir praleidžia GARSIAI** | `deletionRetryPersistence` (abi pusės: praleidžia žymėtą, kartoja nežymėtą ir `pending`) | Grąžinus automatinį kartojimą → krinta 1 (patikrinta mutacija). Sweeper'iui pavykus, jobas dingtų, o žyma liktų `deletion_failed` amžinai - vartotojas gautų 503 dėl duomenų, kurių nebėra. ⚠️ Atskiras `unresolved` skaitiklis būtinas: vien `attempted: 0` sutaptų su „dar ne laiku" ir neatskirtų praleidimo nuo atidėjimo |
| ⚠️ **Ta pati barjero būsena duoda tą patį HTTP kodą ir savininkui, ir administratoriui** | `rbac.route` (202 per admin override + nepaliesti duomenys) | Grąžinus administracinei šakai savą 503 kopiją → krinta 1 (patikrinta mutacija). Operatorius, matantis 503, ieškotų incidento, kurio nėra |
| ⚠️ **Operatoriaus komandoje kita vėliava NĖRA `--actor` reikšmė** | `deletionResilience` (realus proceso paleidimas) | Nuėmus sargą → krinta 1 (patikrinta mutacija). `release <id> --actor --note X` įrašytų barjero pakeitimą su aktoriumi `--note`: auditas atrodytų pilnas, o jame būtų apsirikimas |
| ⚠️ **Našlaičio 503 atsakyme NĖRA `errors`** (#19: jokių failų kelių, saugyklos ar Redis raktų) | `deletionResilience` (atvaizdavimo vienetas) | Grąžinus `deletion: result.outcome` → krinta 1 testas. Savininko kelias taisyklės laikėsi visada; administracinis buvo išimtis (AGENTS.md §16) |
| ⚠️ **P0:** `isDeleted()` asinchroninis, ir VISI vykdymo keliai jį `await`ina | `deletionEnforcement` (elgsena ABIEM kryptim + tripwire su `/await\s+tombstones\.isDeleted\(/`) | Pašalinus `await` iš `jobRunner` → krinta DU testai (patikrinta mutacija): elgsenos („be žymos jobas NEPRALEIDŽIAMAS") ir zondo. Be `await` `Promise` yra truthy → KIEKVIENAS job'as praleidžiamas kaip ištrintas, apdorojimas sustoja, o senasis zondas, ieškojęs vien paminėjimo, liktų žalias |
| ⚠️ **Priešinga kryptis:** be žymos job'as NĖRA praleidžiamas | `deletionEnforcement` (`_runInline` su registruotu procesoriumi) | Be šito „viskas blokuojama" praeitų kaip sėkmė — pozityvus testas vienas negali atskirti veikiančio barjero nuo visiškai užstrigusios sistemos |
| Barjeras aktyvus nuo `deletion_pending`; `deletion_failed` jo NENUIMA; tik žymos NEBUVIMAS leidžia | `erasureMarks` | Leidus `failed` praleisti → nepavykęs trynimas prikurtų dar daugiau to, ką kaip tik bandom pašalinti |
| `deleted` GALUTINĖ: vėlyva nesėkmė jos neperrašo | `erasureMarks` (matrica), **[PG NOT RUN]** `erasureMarks.integration` (RAW SQL) | Postgres pusėje garantija gyvena `UPDATE ... WHERE status = ANY($2)` FORMOJE — `deleted` neatsiranda nė viename `from` sąraše. Įrašius ją → vėluojantis nesėkmingas bandymas panaikintų patvirtintą ištrynimą |
| ⚠️ `deletion_failed → deleted` UŽDARYTAS; retry eina per `deletion_pending` | `erasureMarks` (matrica), `lifecycleDeletion` (teigiama IR neigiama pusė) | Grąžinus tiesioginį perėjimą → `deleted` pasiekiamas be jokio įrodymo, kad antras bandymas vyko; du skirtingi kvietėjai jį pasiektų iš tos pačios būsenos |
| `reason` ir `actor_kind` yra allowlist'ai, tikrinami RAŠANT | `erasureMarks`, **[PG NOT RUN]** `erasureMarks.integration` (DB `CHECK`) | Nežinomos reikšmės normalizavimas paverstų allowlist'ą dekoracija: kvietėjas manytų įrašęs viena, lentelėje atsirastų kita |
| ⚠️ **PRIVATUMAS:** žymoje NĖRA asmens identifikatorių — tik aktoriaus KATEGORIJA | `erasureMarks` (RAW sentinel + identifikatorių atmetimas) | `erasure_marks` pergyvena jobą IR nėra išbraukiama iš kopijų. Plikas `ownerId` joje taptų asmens duomenimis lentelėje, kurios paskirtis — įrodyti, kad asmens duomenys pašalinti. Tikslus aktorius lieka audito kvite |
| **[PG NOT RUN]** FK į `jobs` NĖRA: `jobs` eilutės ištrynimas NEPAŠALINA žymos | `erasureMarks.integration` (RAW SQL), `erasureMarks` (tripwire prieš `references`) | Su `ON DELETE CASCADE` žyma dingtų būtent tuo momentu, kai tampa reikalinga — vėluojanti eilės žinutė nebeturėtų kas ją atmestų |
| **[PG NOT RUN]** `marked_at` nekeičiamas DB trigeriu; `completed_at` ir būsena privalo sutapti | `erasureMarks.integration` (RAW SQL abiem kryptim) | `deletion_failed` su `completed_at` reikštų „nepavyko, bet štai kada pavyko" — būseną, kuria remiasi ištrynimo kvitas |
| **[PG NOT RUN]** Dvi instancijos, DU POOL'AI: destruktyvų perėjimą įvykdo TIK VIENA, abi mato tą pačią galutinę būseną | `erasureMarks.integration` | Procesui lokalus `Map` (`lifecycleService.inFlight`) čia negelbėtų — jis galioja viename procese, o #183 reikalauja koordinacijos tarp replikų |
| **[PG NOT RUN]** ⚠️ Advisory lock'as NELAIKOMAS per išorinį I/O | `erasureMarks.integration` (`pg_locks` patikra, kol „I/O" kabo barjere) | Realizacija, apgaubianti visą ištrynimą viena ilga transakcija, praeitų paprastą lenktynių testą ir produkcijoje išsemtų pool'ą. Transakcijos ribos sutampa su claim'o ribomis STRUKTŪRIŠKAI: `eraseJob()` kviečiamas jau grįžus iš modulio |
| **[PG NOT RUN]** Kritimas ties `pending`: barjeras išgyvena, lock'as neužstringa, retry tęsia | `erasureMarks.integration` (jungties uždarymas = „procesas mirė") | Su SESIJINIU lock'u nutekėjusi jungtis užrakintų `job_id` iki pool'o perkrovimo; transakcinis atlaisvinamas `COMMIT`/`ROLLBACK` metu |
| **[PG NOT RUN]** 7.4e TOCTOU: `assertNotBarred(client, jobId)` veikia KVIETĖJO transakcijoje | `erasureMarks.integration` (lygiagretus `mark()` LAUKIA, kol kvietėjas commit'ina) | `SELECT ... FOR SHARE` to neduotų: neegzistuojanti eilutė nieko neužrakina, ir lygiagretus žymėjimas įsiterptų tarp patikros ir rašymo |
| Prikėlimo horizontas IŠVEDAMAS iš faktinės eilių konfigūracijos, o vienetai normalizuojami PRIEŠ `Math.max` | `revivalHorizons` | Rankinis horizontų sąrašas teste liktų žalias pakeitus konfigūraciją. `86400` (sekundžių para) prieš `600000` (ms) neapdorotoje `Math.max` duotų dešimt minučių vietoj paros — tyliai, nes abu yra skaičiai |
| ⚠️ Neapskaičiuojamas horizonto dydis META, o ne virsta nuliu | `revivalHorizons`, `lifecycleDeletion` (fail-safe) | `parseInt("abc") → NaN`; palaikius jį nuliu, klaidinga konfigūracija TYLIAI sutrumpintų žymos retenciją. Metimas paverčia tai fail-safe atveju: žymos NEŠALINAMOS |
| ⚠️ Per-job `delay` riba VYKDOMA `enqueue()` metu, ne dokumentuojama | `revivalHorizons` (elgsena + tripwire, kad `queue.add(` neegzistuoja už `enqueue()` ribų) | Be ribos job'as su `delay: 30 dienų` atkeliautų po to, kai žyma teisėtai pašalinta, ir ištrintam ID vėl būtų sukurti artefaktai. Dokumentuota riba nesustabdo nė vieno producer'io |
| `deletion_pending` ir `deletion_failed` NESENSTA; `deleted` šalinama tik po `max(prikėlimas, kopijos) + atsarga` | `lifecycleDeletion`, `erasureMarks`, **[PG NOT RUN]** `erasureMarks.integration` | Iki 7.5a pasibaigusi žyma NUSTODAVO blokuoti — nuolat nepavykstantis ištrynimas „išspręsdavo" pats save laikrodžiu, o jautrūs duomenys galėjo tebeegzistuoti |
| `DELETION_TOMBSTONE_TTL_HOURS` gali terminą TIK pailginti | `erasureMarks` | Palikus jį autoritetu, operatorius nustatytų reikšmę žemiau prikėlimo horizonto ir tyliai sulaužytų garantiją |
| Žymų valymas eina per ESAMĄ `retentionSweeper`, ne per savo timer'į | `auditRetention`, `retentionSweeper` integracija (`tombstones=` `RETENTION_PURGE` įraše) | Antras timer'is reikštų antrą konfigūraciją, antrą laikrodį, o įrašas rodytų tik vieno jų darbą — ta pati taisyklė, kurią 7.4d pritaikė auditui |
| **[PG NOT RUN]** Atkūrimas žymų NELIEČIA: po kopijos sukurta žyma išlieka | `erasureMarks.integration` (elgsena + tripwire prieš `erasure_marks` minėjimą `restoreService`) | Jei atkūrimas liestų lentelę, po kopijos ištrinti job'ai grįžtų BE tombstone — kopija atšauktų GDPR ištrynimą |
...
**Override'ų registras.** Priežastis privalo gyventi REPO, ne tik commit'o žinutėje:
sargas turi dokumentuotą apėjimą, ir auditui commit'o žinutė yra sunkiau randama nei
įrašas šalia paties override.

| Kada | Testas | Priežastis | Ar tai tikras šalinimas? |
|---|---|---|---|
| #157 PR-5 (CI `34144363714`) | „KONTRAKTAS: visi trys backend'ai deklaruoja TĄ PAČIĄ **17** metodų aibę" | Skaičius testo VARDE darė tapatybę nestabilią: kiekvienas kontrakto praplėtimas keitė vardą, ir sargas tai matė kaip trynimą. Skaičius perkeltas į tvirtinimą, vardas — be jo | **Ne.** Testas liko, tvirtinimas ir apimtis nepakito; pasikeitė tik vardas. Po šio pakeitimo kontrakto plėtimas sargo nebežadina |
| #157 PR-6 (CI [`34370538544`](https://github.com/forevercornix/stenograma/actions/runs/34370538544) → [`34371358431`](https://github.com/forevercornix/stenograma/actions/runs/34371358431)) | Penki viršutinio lygio `#157 PR-6:` testai `artifactMigration.integration` faile | Testai PERKELTI į bendrą `helpers/artifactMigrationScenarios.js`, o antraštės gavo backend'o žymę (`[fs]` / `[s3]`). Sargui tai šalinimas plius pridėjimas PAGAL KONSTRUKCIJĄ — jis sąmoningai nedaro fuzzy pervadinimo spėjimo (žr. antraštės „NEDARO fuzzy pervadinimo spėjimo"). ⚠️ **Rinkinys iškeltas ne dėl tvarkos, o todėl, kad tie patys scenarijai turi veikti prieš ANTRĄ backend'ą** — ir pirmas `s3` raundas iškart rado septynias `'fs'` prielaidas, kurių `ArtifactStore` kontraktas nepagauna. | **Ne.** Aprėptis PADVIGUBĖJO: 16 → **32** testai (kiekvienas identitetas virto dviem, `[fs]` ir `[s3]`). Abu skaičius rodo tas pats raundas: pirmas `34370538544` (26 pass / 6 fail iš 32), po taisymo `34371358431` (32/32). |
| #155 barjeras (šis PR) | Penki `jobStoreBackendSelection.test.js` testai: `parinkimas: DATABASE_URL > REDIS_URL > memory`, `BARJERAS: DATABASE_URL vienas NEPERJUNGIA srauto į PostgreSQL`, `BARJERAS: DATABASE_URL + REDIS_URL palieka esamą Redis elgesį NEPAKITUSĮ`, `BARJERAS: eksplicitinis JOB_STORE_BACKEND=postgres yra KLAIDA, ne įspėjimas`, `BARJERAS: memory ir redis pasirinkimai praeina nepakitę` | Pavadinimai teigė TAISYKLĘ, kuri šiuo PR pasikeitė. `BARJERAS: ... yra KLAIDA` po atidarymo būtų buvęs tiesiog neteisingas, o likę trys vadino barjeru tai, ką dabar lemia eksplicitinis pasirinkimas. Testas, kurio vardas meluoja apie taisyklę, blogesnis už jo nebuvimą: jis skaitomas kaip dokumentacija. Nauji vardai neša taisyklę (`EKSPLICITINIS PASIRINKIMAS:`, `POLITIKA (1/2)`, `POLITIKA (2/2)`, `ATIDARYTAS BARJERAS:`) | **Ne.** Aprėptis PADIDĖJO: 5 → **6** identitetai. `yra KLAIDA` buvo invertuota asercija ir virto PORA — `(1/2)` rodo, kas tapo leistina, `(2/2)` — kas atsirado vietoj jos; atskirai nė vienas politikos nepasako. Nė vienas tvirtinimas nedingo, visi gavo `GINA:` eilutę |
| #155 barjeras (šis PR, antras raundas) | Du `privacyConfig.test.js` testai, TUOMETINIAIS vardais: `vien DATABASE_URL NEDUODA persistencijos, kol galioja aktyvavimo barjeras`, `PERSISTENT_STORAGE=true su DATABASE_URL: klaida įvardija BARJERĄ, ne trūkstamą URL` (abu pervadinti — priežastis iš vardo išimta) | Abu liko ŽALI atidarius barjerą — ir būtent todėl buvo rasti ne CI, o inventoriumi. Pirmas tikrino teisingą faktą su neteisinga priežastimi vardo viduje; antras tikrino `/aktyvavimo barjeras/` produkcinėje žinutėje, t. y. ĮTVIRTINO tekstą, kuris po atidarymo tapo melu (operatorius buvo siunčiamas kelti Redis, nors užtenka `JOB_STORE_BACKEND=postgres`) | **Ne.** Aprėptis 2 → **4**: pridėta apvirtusi pusė (`eksplicitinis JOB_STORE_BACKEND=postgres DUODA persistenciją`) ir diagnostikos `jobState` pora. Antrasis testas dabar tikrina ir PRIEŽASTĮ, ir VEIKSMĄ, ir turi `doesNotMatch` ant pasenusio teksto |
