...
| `AUTH_USERS` tikrinamas ir RUNTIME: pašalintas ar pažemintas vartotojas nutraukia sesiją be restarto | `sessionAuthFailClosed.route`, `sessionStoreBackendContract.integration` | Pašalinus `patikrintiTapatybe()` iš `touch()` → sena rolė toliau autorizuoja; įrašas nebepažymimas revokuotu |
