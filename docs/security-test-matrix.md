...
| ⚠️ Užstrigusi žyma TURI IŠEITĮ, ir ji palieka pėdsaką | `erasureMarks` (retry tik iš `failed`; force-resolve rašo auditą PRIEŠ veiksmą) | Barjeras nuo `pending` plius nesenėjimas be išeities užrakintų job'ą neribotam laikui. Kritus auditui žyma privalo LIKTI neterminalė: kitaip barjerą būtų galima nuimti be įrašo, kas jį nuėmė |
