...
| ⚠️ **Struktūrinis atmetimas SUSTABDO retry grandinę** | `artifactUnrecoverable.integration` | PR-2 padarė `neatkartojama` ženklą, bet niekas jo neskaitė — savybė be ją paisančio kelio yra dokumentacija (buvo užrašyta kaip `UNVERIFIED`). Dabar completion kelias vynioja tokias klaidas į `UnrecoverableError` su `cause`, kaip `assertResultWithinLimits` precedentas. ⚠️ **Matuojamas ELGESYS:** job’as su `Date` rezultate baigiasi `failed` po VIENO vykdymo, `attemptsMade` lieka 1 — o ne lauko buvimas. Kontrolė: paprasta klaida vis dar kartojama (kitaip „retry sustabdytas“ reikštų neveikiantį mechanizmą) |
