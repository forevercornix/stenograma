...
| ⚠️ **Eilutės backend’as privalo sutapti su saugyklos** | `jobStoreHydration.integration` | Pilnas dispatch pagal `storage_type` — PR-4/PR-6 tema, o kol skaitoma iš vienos injekuotos saugyklos, `s3` eilutė per `fs` saugyklą tuo pačiu raktu ir su sutampančiu dydžiu hidratuotų SVETIMĄ turinį, ir niekas apie tai nesužinotų. Fail-closed kainuoja vieną `if` |
