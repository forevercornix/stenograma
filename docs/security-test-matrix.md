...
| ⚠️ **`pending` su `attempts > 0` NĖRA svetimas** – operatoriaus autorizuotas pakartojimas privalo būti vykdomas | `lifecycleDeletion` (abi pusės), `auditAsyncCutover` (pilnas atkūrimo kelias) | Pašalinus sąlygą → krinta 2 testai (patikrinta mutacija). Autorizuotas pakartojimas amžinai gautų „jau vykdoma", ir pati pretenzija uždarytų vienintelę operatoriaus išeitį |
