...
| ⚠️ **Checksum nustatymai yra TIKRINAMA sąlyga, ne fonas** | `artifactStoreS3.integration` (mutacija) | Mutacija: klientas be `requestChecksumCalculation`/`responseChecksumValidation` paleidžiamas prieš tą pačią MinIO, ir rezultatas UŽRAŠOMAS išvestyje. ⚠️ **Testas toleruoja ir sėkmę:** naujesnės MinIO versijos numatytuosius jau palaiko, ir tada mutacija nieko nesulaužo — tai INFORMACIJA, ne gedimas (§14.1). Kontrolė: mūsų konfigūracija tą patį kelią praeina |
