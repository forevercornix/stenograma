...
| ⚠️ **TAS PATS kontraktas galioja `s3` (MinIO)** | `artifactStoreS3.integration` | Rinkinys paleidžiamas NEKEIČIAMAS prieš tikrą S3-compatible saugyklą. S3 yra pirmas TIKRAI nutolęs backend'as: tinklo latencija, kita klaidų taksonomija, `ETag` semantika, brangus `head`. ⚠️ Rinkinys tris kartus tyliai perėmė `fs` savybes (kelio forma, sinchroniškumas, rakto gamyba) — kiekvieną kartą tai pasimatė tik prieš antrą backend'ą |
| ⚠️ **`get` kelias tikrinamas ATSKIRAI nuo `put`** | `artifactStoreS3.integration` | Checksum problema rašymo ir skaitymo pusėse pasireiškia SKIRTINGAI: `PutObject` lūžta dėl trūkstamos `Content-Md5`, o `GetObject` — dėl pasirašytos `x-amz-checksum-mode` antraštės, į kurią S3-compatible tiekėjai atsako `SignatureDoesNotMatch`. Testas, dengiantis tik `put`, praleistų pusę klasės ir liktų žalias |
...
| ⚠️ **INFORMACINIS matavimas, ne garantija** | `artifactStoreS3.integration` | ⚠️ **Išmatuota (CI 33946366087): `rašymas=praėjo, skaitymas=praėjo`.** Pririšta MinIO versija numatytuosius nustatymus jau palaiko, tad jų pašalinimas prieš ją nieko nesulaužo — vadinti tai mutacija reikštų teigti daugiau, nei ji daro. Testas lieka, nes pasakys, kada pririšta versija pasikeis; enforcement gyvena `artifactStoreS3Config` |
