...
| Eksplicitinis įvykio vardas, neatitinkantis `EVENT_PATTERN` (pvz. `login_success`), ATMETAMAS kaip `AUDIT_EVENT_MALFORMED`, o ne tyliai išvedamas iš kitų laukų | `auditAsyncCutover` | Pašalinus sargybą → `normalizeEvent()` išveda `PROCESSING_COMPLETED`, ir autentikacijos įvykis su rašybos klaida paveldi NEBLOKUOJANČIĄ semantiką be jokio signalo |
