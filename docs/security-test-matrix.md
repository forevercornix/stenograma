...
| `AUDIT_BACKEND=postgres` startas ĮSPĖJA, kad retencija neveikia ir lentelė augs neribotai — matomumas, ne blokavimas | `auditStoreFields` (turinys), `auditPersistence.integration` **[NOT RUN]** (kad realiai loginama, ir kad startas TĘSIASI) | Pašalinus `log.warn` → krinta PG testas; sutrumpinus tekstą iki bendro „retencija ribota" → krinta turinio patikra, nes dingsta `7.4d` ir `docs/audit-storage.md` nuorodos |
| Auditas išlieka per PILNĄ `init → shutdown → init` ciklą, o GDPR ištrynimas PO restarto randa PRIEŠ restartą sukurtą įrašą | `auditPersistence.integration` **[NOT RUN]** | Leidus atsitiktinę druską → antras paleidimas skaičiuotų kitą pseudonimą, `removeBySubjectIdentifier()` grąžintų „ištrinta 0" tyliai. ⚠️ KONTEINERIO restartas NEPATIKRINTAS — reikia Docker, kurio kelti negalima |
...
| Append-only trigeris tikrinamas TOJE PAČIOJE schemoje ir privalo būti REALIAI APSAUGANTIS: `tgenabled` ∈ {`O`, `A`}. `D` (išjungtas) ir **`R` (replica-only)** atmetami | `auditPersistence.integration` **[NOT RUN]** | `ENABLE REPLICA TRIGGER` palieka trigerį atrodantį įjungtu, bet jis suveikia tik `session_replication_role = 'replica'` - aplikacijos sesijos veikia kaip `origin`, tad `UPDATE` praeina. Testas tikrina ABI puses: kad startas atmeta IR kad tokioje DB `UPDATE` realiai praeitų. Be `pg_namespace` tiktų to paties vardo trigeris kitoje schemoje (AGENTS.md §16) |
| `audit_log_seq_unique` yra STARTO barjero dalis — tikrinamas atskirai, nes `contype = 'u'`, o `CHECK` užklausa jo nemato | `auditPersistence.integration` **[NOT RUN]** | Numetus constraint'ą → tiesioginis INSERT gali pakartoti `seq`, ir `ORDER BY seq` - deklaruotas tvarkos autoritetas - tampa neapibrėžtas; startas privalo nutrūkti |
...
| Neribotas `clear()` persistentiniame režime VYKDOMAI uždraustas už testų ribų | `auditPersistence.integration` **[NOT RUN]** | Pašalinus sargybą → atsitiktinis produkcinis kvietėjas ištrintų VISĄ audito pėdsaką, nepaisant dokumentuotos „tik subjektu apribotas trynimas" ribos |
...
| `audit_log_seq_unique` tikrinamas pagal APIBRĖŽIMĄ (`UNIQUE (seq)`), ne vardą | `auditPersistence.integration` **[NOT RUN]** | Nuklydusi DB su to paties vardo constraint'u ant kito stulpelio praeitų vardo patikrą, o `seq` dublikatai liktų teisėti |
