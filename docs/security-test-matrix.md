...
| `AUDIT_BACKEND=postgres` startas ĮSPĖJA, kad retencija neveikia ir lentelė augs neribotai — matomumas, ne blokavimas | `auditStoreFields` (turinys), `auditPersistence.integration` **[CI 33044931167]** (kad realiai loginama, ir kad startas TĘSIASI) | Pašalinus `log.warn` → krinta PG testas; sutrumpinus tekstą iki bendro „retencija ribota" → krinta turinio patikra, nes dingsta `7.4d` ir `docs/audit-storage.md` nuorodos |
