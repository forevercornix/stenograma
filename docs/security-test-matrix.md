...
| Generacijų skenavimo KLAIDA nutraukia startą, o ne praeina tuščiai | `auditStoreFields` | Įdėjus `catch { return [] }` → našlaičių patikra praeitų tuščiai (tyli GDPR spraga), o kiekio taisyklė nuspręstų, kad VISI istoriniai raktai nebereikalingi (klaidingas FAIL). Patikrinta mutacija |
