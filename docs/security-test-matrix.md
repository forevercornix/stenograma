...
| LAIKINAS MATAVIMAS (bus pašalintas) | `zzMatavimasRedis.integration` | Redis round-trip ir kanoninė tapatybė — matavimas #157 PR-4 raundui |
| ⚠️ **Job’as turi DAUGIAUSIA VIENĄ įsipareigotą bandymą** | `externalCompletion.integration` | Po remonto registre likdavo DU `committed` įrašai — senasis (kurio objekto nebėra) ir naujasis. Registras teigtų, kad naudojami du objektai, o šlavėjas (PR-5) senojo niekada neliestų: jis atrodytų reikalingas. Perėjimas daromas VIENU sakiniu toje pačioje transakcijoje kaip nuorodos įrašymas, kitaip liktų momentas su dviem arba nė vienu įsipareigotu |
