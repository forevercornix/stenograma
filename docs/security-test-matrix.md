...
| Append-only trigeris tikrinamas TOJE PAČIOJE schemoje ir privalo būti ĮJUNGTAS (`tgenabled <> 'D'`) | `auditPersistence.integration` **[NOT RUN]** | `ALTER TABLE ... DISABLE TRIGGER` palieka `pg_trigger` eilutę: be `tgenabled` patikros startas paskelbtų barjerą veikiančiu, o `UPDATE` praeitų. Be `pg_namespace` tiktų to paties vardo trigeris kitoje schemoje (AGENTS.md §16) |
