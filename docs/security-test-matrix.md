...
| ⚠️ **Replay šalina iš ATKURTOS bazės, ne iš fasado** | `erasureReplayContract`, `drRestore.integration` (6a) | Mutacijos M8 (`store` ignoruojamas skaitant) ir M9 (`eraseJob` gauna fasadą) → krinta. ⚠️ **Radinį atidengė kritęs E2E, ne peržiūra ir ne mutacija:** 7.2a barjeras job'ų autoritetu palieka atmintį, tad replay per fasadą atkurtų eilučių NELIESTŲ — ir vis tiek uždarytų žymą bei įrašytų kvitą |
