...
### Griežtumas priklauso nuo to, ar išvestis remiasi šaltiniu

| Operacija | Išvestis | Šaltinio patikra |
|---|---|---|
| `startPhase()` | grafo briauna nuo dabartinės fazės | **griežta** |
| `restart()` | visada `processing/validating` tipo grafe | tipas ir fazė |
| `finish()` | visada `{status, phase: null, progress: null}` | **tik statuso perėjimo** |
| `reportProgress()` | progresas dabartinėje fazėje | griežta, bet grąžina `null` |

⚠️ **Ką `finish()` tikrina ir ko netikrina.**

| Tikrina | Netikrina |
|---|---|
| ar terminalus **statuso** perėjimas leidžiamas (`queued → completed` draudžiamas) | ar `phase` nuosekli su `status` |
| ar job'as jau nėra terminalus | ar `phase` priklauso tipo grafui |
| | ar `type` apskritai žinomas |

Tai sąmoninga: klaidos apdorojimas turi galėti terminalizuoti ir **dalinai
sugadintą** `processing` įrašą, neprarasdamas pirminės klaidos.

Priežastis — `finish()` kviečiamas būtent klaidos kelyje (`jobRunner.js:291`).
Reikalaudamas nuoseklios `phase`/`progress` būsenos, jis sugadintam įrašui
mesdavo: pirminė providerio klaida dingdavo, o job'as likdavo `processing`
amžinai.

> **Terminalus perėjimas negali nepavykti dėl NENUOSEKLIOS `phase`/`progress`
> būsenos.** Neleistinas statuso perėjimas (`queued → completed`) nepavyksta ir
> toliau — tai kita taisyklė.

**Praktinė pasekmė legacy įrašams.** `processing + phase=null` (iš prieš #154)
per `finish()` praeina, per `restart()` — ne. Tai teisinga: atstatymas nuo
nežinomos fazės neapibrėžtas, o išėjimas visada apibrėžtas.
...
`GET /api/jobs/:id` ir `GET /api/transcribe-jobs/:id` grąžina **tik** `current`
ir `total`. Papildomi laukai, patekę į įrašą (atkurta kopija, būsimos schemos
metaduomenys), į atsakymą nepatenka.

`progress` ir `progressKnown` skaičiuojami **kartu**: jei progresas netinkamas
arba pasenęs, abu tampa `null`/`false`. Klientas niekada negauna uždrausto
`progressKnown: true` su `progress: null` derinio, net jei įraše jis yra.

Legacy skaitinis `progress` (iš prieš #154) normalizuojamas į `null` — jis
neverčiamas į `{current, total}`, nes `total` nežinomas, o spėti reikštų
išgalvoti duomenis.
...
<!-- MEMORY-CAS:required -->
⚠️ **Tai reikalinga IR memory backend'ui.** Ankstesnė šio dokumento redakcija
tvirtino priešingai, remdamasi tuo, kad `get` ir `update` vyksta be `await`
tarp jų. Tai buvo neteisinga: fasadas daro `await store.get(id)`, ir tas `await` atveria langą.
...
> ⚠️ **Rankiniu būdu palaikomas žemėlapis, ne pilnas failų sąrašas.** Testas
> tikrina, kad žinomas sluoksnis nedingtų ir kad keliai egzistuotų — bet naujo
> sluoksnio atsiradimo jis nepastebi.

| Sluoksnis | Failas |
|---|---|
| State machine ir įrašo konsistencija | `backend/utils/jobPhase.js` |
| Store fasadas (fazių metodai, ištrynimo žymos) | `backend/utils/jobStore/index.js` |
| Atominis progresas: memory | `backend/utils/jobStore/memoryStore.js` |
| Atominis progresas: Redis (Lua CAS) | `backend/utils/jobStore/redisStore.js` |
| Pipeline įvykiai | `backend/queues/processors.js` |
| Fazių deklaravimas servisuose | `backend/services/transcriptionService.js`, `backend/services/protocolService.js` |
...
⚠️ **Backend'ų ekvivalentumas.** `reportProgressAtomic()` realizuotas ABIEJUOSE
backend'uose, ir jų elgesys turi sutapti — tai tikrina
`tests/jobStoreBackendContract.integration.test.js`. Backend'ai jau buvo
išsiskyrę (sugadintas įrašas, įdėti metaduomenys, skaitinės eilutės), tad
pridedant naują saugyklą (#155 `postgresStore`) šis testas privalo gauti trečią
adapterį.
