...
const { poolasTestui } = require("./helpers/resourceStack");
...
/**
 * ⚠️ GYVAVIMO CIKLAS PERKELTAS Į `helpers/resourceStack` (#380 R2).
 *
 * Iki tol čia gyveno `naujasPoolas`/`uzdarytiPoola`/`nutekejimoTvirtinimas` — antras
 * mechanizmas šalia #180 `resourceStack`. #380 juos sujungė: registras liko ten, kur
 * turi ŠEŠIS vartotojus, o riba ir nutekėjimo apskaita tapo jo dalimi. Šiame faile
 * lieka tik tai, kas yra #351 sargas, o ne gyvavimo ciklas: `ribotas`, `atblokuotiPer`.
 */
...
test(
  ...
  async (t) => {
     ...
     */
    await perkurtiDb();
    const { pool } = poolasTestui(t, { dsn: DB_URL });
...
test(
  ...
  async (t) => {
    ...
    const { pool, kruva } = poolasTestui(t, { dsn: DB_URL });
    ...
    try {
       ...
       */
      const nutekejo8a = kruva.matuotiNutekejima(pool, "8a: rašytojas laiko eilutę");
    ...
    const nutekejo8b = kruva.matuotiNutekejima(pool, "8b: lentelės nebėra");
...
test(
  ...
  async (t) => {
    ...
    const nutekejo9 = kruva.matuotiNutekejima(pool, "9: šluotinas kandidatas su `delete()` kabliu");
