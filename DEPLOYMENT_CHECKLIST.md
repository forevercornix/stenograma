...
| **Readiness nepatvirtina worker'io** (`/api/ready`) | `/api/ready` BullMQ režime tikrina Redis pasiekiamumą, BET NE ar worker konteineris gyvas. Jei worker išjungtas/nulūžęs, o Redis veikia, ready=true, ir jobai bus priimami bei liks `queued`. **Production sprendimas:** worker heartbeat - worker'is kas 10-15s atnaujina Redis raktą su TTL (`stenograma:worker:transcription:heartbeat`), o `/api/ready` grąžina 503, jei raktas pasenęs. Portfolio MVP tai priimtinas žinomas apribojimas. |
