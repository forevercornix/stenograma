...
#    GitHub sąsajoje bus prarasti.
#
# Tekstai imami iš SUBISSUES-155.md. Idempotentiškas: pakartotinis paleidimas
# nekuria dublikatų.
#
# ⚠️ „VIENINTELIS ŠALTINIS" GALIOJA SAVO APIMTYJE, NE PLAČIAU.
#
#    Spec yra vienintelis šaltinis TEKSTUI, KURĮ GENERUOJA ŠIS SKRIPTAS. Sukurtas
#    ir aktyvus issue toliau gyvena savo gyvenimą: jo body keičiasi per peržiūras,
#    ir būtent jis yra dabartinių reikalavimų bei DoD autoritetas. Pasenusi buvo
#    ne pati frazė, o jos NUMANOMA sritis.
#
# ------------------------------------------------------------------------------
# SUBISSUES-155.md is the source of truth for the [7.x] sub-issue text
# generated by this script. Once an issue is created and active, its GitHub
# issue body is authoritative for its current requirements and Definition of Done.
#
# WARNING: --update overwrites the GitHub issue body from SUBISSUES-155.md.
# It does not preserve changes made directly to an active issue after creation.
# Use --update only deliberately after verifying that overwriting the current
# issue body is intended.
#
# This is a replace, not a sync: there is no merge or reconciliation step.
# ------------------------------------------------------------------------------
